// ============================================================================
// bruno-corredor-card — Corredor como 8o TILE da faixa de comodos (Home V3).
//
// Clone estrutural fiel de bruno-quarto-casal-card.js (a mesma base dos demais
// tiles), com as entidades do Corredor. Motivo de existir: o comando do
// Corredor perdeu o lugar quando a faixa de acoes rapidas foi reavaliada, e
// entre as tres alternativas estudadas (partir o tile do Lavabo, mover para a
// rail, ou virar tile proprio) a escolhida foi a que PRESERVA a homogeneidade
// da serie — regra que sustenta o visual de tiles + filetes.
//
// O clique unico vem de graca: o tap do tile ja e toggle da luz do comodo, a
// mesma mecanica dos outros sete.
//
// PARTICULARIDADES em relacao aos demais comodos:
//   - Sem sensor de temperatura/umidade: o bloco `metric` se oculta sozinho
//     (a classe has-metric so entra quando ha temperatura).
//   - Sem grupo de luz: o circuito e um switch unico (light.corredor_switch_1),
//     usado tanto como room_group quanto como room_toggle.
//   - Sem sensor `*_active`: a contagem de luzes cai no fallback por membros,
//     que resolve corretamente para um circuito unico.
//   - Asset proprio em par off/on, com fallback vetorial de seguranca.
// ============================================================================

const BRUNO_CORREDOR_CARD_TAG = 'bruno-corredor-card';

const BRUNO_CORREDOR_DEFAULT_CONFIG = {
  "name": "Corredor",
  "icon_size": 94,
  "room_on_states": [
    "on",
  ...
  "semantic_on_states": [],
  "entities": {
    // Circuito unico: sem grupo, o proprio switch faz os dois papeis.
    "room_group": "light.corredor_switch_1",
    "room_toggle": "light.corredor_switch_1",
    "room_fallback_lights": [
      "light.corredor_switch_1"
    ],
    // Nao existe sensor.corredor_active; a contagem cai no fallback.
    "active_sensor": "",
    // Camada de presenca/ocupacao/semantica ja existente em
    // packages/presence_supervision.yaml (PIR do corredor).
    "motion_recent": "binary_sensor.corredor_motion_recent",
    "occupancy": "binary_sensor.corredor_occupancy",
    "semantic_sensor": "sensor.corredor_semantic_state",
    "presence": "binary_sensor.pir_motion_sensor_corredor_movimento",
    "illuminance": "",
    // Corredor nao tem sensor de clima — listas vazias ocultam o bloco.
    "temperature": [],
    "humidity": [],
    "dishwasher": ""
  },
  ...
  "status_dots": [
    {
      ...
      "label": "Presenca",
      "tone": "blue",
      // Mesma regra dos demais comodos: presenca IMEDIATA (motion_recent).
      // A ocupacao aparece so no texto semantico, nunca no dot.
      "entities": [
        "binary_sensor.corredor_motion_recent"
      ],
      "states": [
        "on"
      ]
...
const BRUNO_CORREDOR_ACTION_COOLDOWN = 1200;

class BrunoCorredorCard extends HTMLElement {
  ...
  setConfig(config) {
    const entities = {
      ...BRUNO_CORREDOR_DEFAULT_CONFIG.entities,
      ...(config?.entities || {}),
    };
    const icon = {
      ...BRUNO_CORREDOR_DEFAULT_CONFIG.icon,
      ...(config?.icon || {}),
    };

    this._config = {
      ...BRUNO_CORREDOR_DEFAULT_CONFIG,
      ...config,
      entities,
      icon,
      room_on_states: this._array(config?.room_on_states || BRUNO_CORREDOR_DEFAULT_CONFIG.room_on_states),
      semantic_on_states: this._array(config?.semantic_on_states || BRUNO_CORREDOR_DEFAULT_CONFIG.semantic_on_states || []),
      status_dots: Array.isArray(config?.status_dots) ? config.status_dots : BRUNO_CORREDOR_DEFAULT_CONFIG.status_dots,
    };
    this._render();
  }
  ...
  _sensorValue(entityIds, suffix = '') {
    ...
    if (!entity) return '';
    return `${BrunoCorredorCard._escape(entity.state)}${suffix}`;
  }
  ...
  _runAction(key, gesture) {
    ...
    // Diagnostico barato: se a entidade nao existir no hass, o toggle falha em
    // silencio. Aqui o motivo aparece no console em vez de virar mais uma
    // rodada de tentativa e erro.
    const targetId = entities.room_toggle || entities.room_group;
    if (this._hass && targetId && !this._hass.states?.[targetId]) {
      console.warn(`[bruno-corredor-card] entidade inexistente no Home Assistant: ${targetId}`);
    }

    // `homeassistant.toggle` em vez de `light.toggle`: e agnostico de dominio,
    // entao o tile continua funcionando se o circuito do Corredor for
    // reexposto como switch.* (ja aconteceu com o Q. Miguel no passado). O
    // dock de acoes rapidas usa exatamente este servico para o mesmo circuito.
    // ANTERIOR: this._callService('light.toggle', ...)
    this._callService('homeassistant.toggle', {}, { entity_id: entities.room_toggle || entities.room_group });
  }
  ...
  _isActionCoolingDown(key) {
    ...
    if (now - previous < BRUNO_CORREDOR_ACTION_COOLDOWN) return true;
  ...
  _wireAction(button) {
    ...
    // CORRECAO (2026-07-29): na base este handler apenas ENGOLE o clique
    // (preventDefault + stopPropagation), porque a acao viria do par
    // pointerdown/pointerup. Se essa sequencia falhar por qualquer motivo, o
    // toque nao produz nada — e era o sintoma relatado no Corredor.
    // Causa mais provavel: _render() reescreve o innerHTML a CADA atualizacao
    // de hass; se uma chegar entre o pointerdown e o pointerup, o botao (e o
    // activePointerId dele) e substituido, e o pointerup cai num elemento novo
    // que ignora o evento. Nos outros tiles isso passa despercebido porque a
    // zona de navegacao tambem responde; aqui ela foi desligada de proposito.
    // O clique agora e FALLBACK: dispara a acao se o caminho de pointer nao
    // disparou. Nao ha risco de acionar duas vezes — _runAction ja tem
    // cooldown de 1200ms, que absorve o par pointerup+click.
  ...
  _statusDot(dot) {
    ...
    return `
      <span class="status-dot tone-${dot.tone}${activeClass}" title="${BrunoCorredorCard._escape(dot.label)}" aria-label="${BrunoCorredorCard._escape(dot.label)}">
  ...
  _statusLines(lines) {
    ...
    return lines.map((line) => `<span>${BrunoCorredorCard._escape(line)}</span>`).join('');
  }

  _assetVisual(active) {
    const icon = this._config.icon;
    const fallback = BrunoCorredorCard._escape(icon.fallback || 'mdi:home-outline');
    const off = BrunoCorredorCard._escape(icon.off || '');
    const on = BrunoCorredorCard._escape(icon.on || '');
    return `
  ...
  _render() {
    ...
    const iconSize = Number(this._config.icon_size) || BRUNO_CORREDOR_DEFAULT_CONFIG.icon_size;
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        * {
        ...
        .room-action {
          ...
          /* ANTERIOR (2026-07-03): 112px fixo — mesmo risco de clip do Miguel.
             grid-template-columns: 112px minmax(0, 1fr) 40px; */
          /* NOVO (padrao Office): coluna do icone absorve o deficit. */
        ...
        /* Fallback de seguranca: assume o lugar do render 3D apenas se os
           caminhos off/on forem removidos da configuracao. */
        .room-icon.has-no-asset .room-asset-fallback {
          opacity: 1;
          display: grid;
        ...
        .room-icon.has-no-asset .room-asset-fallback bruno-icon {
          --mdc-icon-size: 54px;
          width: 54px;
          height: 54px;
        }
        ...
        .room-nav-zone.is-navigating .room-chevron {
          animation: brunoCorredorChevronNavigate 360ms ease both;
        }

        @keyframes brunoCorredorChevronNavigate {
      ...
      <div class="room-card${roomActiveClass}${this._tileClasses()}" style="--room-icon-size:${iconSize}px;">
        ${this._tileDivider()}
        <!-- O fallback vetorial aparece apenas se os caminhos off/on estiverem vazios. -->
        <button class="room-action${hasMetricClass}" type="button" data-action-key="room" aria-label="${BrunoCorredorCard._escape(this._config.name)}">
          <div class="room-icon${(this._config.icon?.off || this._config.icon?.on) ? '' : ' has-no-asset'}" aria-hidden="true">${this._assetVisual(model.iconActive)}</div>
          ...
          <span class="room-nav-zone" data-room-nav role="button" tabindex="0" aria-label="Abrir ${BrunoCorredorCard._escape(this._config.name)}">
            <span class="room-title-row">
              <span class="title">${BrunoCorredorCard._escape(this._config.name)}</span>
              <!-- Sem chevron: o Corredor nao tem subview. O chevron dos
                   demais tiles anuncia navegacao; aqui seria promessa falsa. -->
            </span>
    ...
    // CORRECAO (2026-07-29): a zona de navegacao NAO e ligada no Corredor.
    // Ela cobre o bloco de titulo/estado (grid-row 3/5, z-index 4) e chamava
    // stopPropagation no pointerdown; como o Corredor nao tem `section` nem
    // `double_tap_action`, o tap caia em _moreInfo() e abria a janela de
    // detalhes em vez de acender a luz. Sem o wiring, o evento borbulha ate o
    // botao .room-action e o tile inteiro passa a ser toggle — que e o unico
    // proposito dele.
    // ANTERIOR: this._wireRoomNavZone(this.shadowRoot.querySelector('[data-room-nav]'));
    // REDE DE SEGURANCA (2026-07-29): listener de clique no PROPRIO card, fora
    // da maquina de estado de pointer herdada da base. Duas rodadas tentando
    // fazer o caminho pointerdown/pointerup funcionar aqui nao resolveram, e o
    // Corredor tem exatamente UMA funcao: acender e apagar. Qualquer clique em
    // qualquer ponto do tile agora aciona o toggle.
    // Nao dispara duas vezes: _runAction tem cooldown de 1200ms, que absorve
    // o clique vindo pelo botao e o vindo por aqui.
    const cardRoot = this.shadowRoot.querySelector('.room-card');
    if (cardRoot) {
      cardRoot.addEventListener('click', () => this._runAction('room', 'tap'));
    }
...
if (!customElements.get(BRUNO_CORREDOR_CARD_TAG)) {
  customElements.define(BRUNO_CORREDOR_CARD_TAG, BrunoCorredorCard);
}

window.customCards = window.customCards || [];
window.customCards.push({
  type: BRUNO_CORREDOR_CARD_TAG,
