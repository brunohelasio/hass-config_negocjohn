...
const BRUNO_HOME_CAMERA_LIVE_TIMEOUT_MS = 10000;
...
class BrunoHomeCameraCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    this._liveSuspendedEntities = this._liveSuspendedEntities || new Set();
  ...
  connectedCallback() {
    this._liveSuspendedEntities?.clear();
    this._startRefreshTimer();
  ...
  _openMoreInfo(entityId) {
    if (!entityId) return;
    // O more-info recebe a sessao unica da camera. O card fica em snapshots
    // para esta entidade ate ser reconectado, como no tile aprovado do Office.
    this._liveSuspendedEntities.add(entityId);
    this._stopLiveFeed();
   ...
   * A ordem e deliberada: conecta primeiro ao DOM e so depois atribui entityid.
   */
  _mountLiveFeed(camera) {
    ...
    const mount = entityId
      ...
      : null;
    if (!this.isConnected || !mount || camera?.unavailable || this._liveSuspendedEntities.has(entityId)) {
      this._stopLiveFeed();
      return;
    }
    ...
    if (!this._liveEl || this._liveEntity !== entityId) {
      ...
      if (!globalThis.customElements?.get('ha-web-rtc-player')) {
        // ANTERIOR (rollback): um fallback poderia montar hui-image live.
        // Ele reabre HLS e os 12 s de buffer; sem WebRTC direto, fica a foto.
        this._liveSuspendedEntities.add(entityId);
        return;
      }
      const el = document.createElement('ha-web-rtc-player');
      ...
      this._liveStreamsHandler = (event) => {
        if (event?.detail?.hasVideo === false) this._failLiveFeed(entityId);
      };
