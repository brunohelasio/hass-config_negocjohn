...
const Na = 720, Ra = 200, ja = 24, Va = 4, Ge = 3e4, Ba = 12, ve = 10 * 1024 * 1024, Fa = 64;
function _e(r) {
  let e = Number.POSITIVE_INFINITY;
  for (const a of r) a.usado < e && (e = a.usado);
...
class Ua {
  ...
  de(e) {
    ...
    return a || (a = La(e), this.componentes.set(e, a)), a;
  }
  // ── Pontos de coleta ──────────────────────────────────────────────────────
  conectou(e) {
   ...
   */
  renderizou(e, a, i) {
    const t = this.de(e), o = t.render;
    if (o.total++, o.duracaoTotal += a, o.ultima = a, a > o.pior && (o.pior = a), !i) return;
    const n = t.motivos.has(i) || t.motivos.size < ja ? i : "outros";
    t.motivos.set(n, (t.motivos.get(n) ?? 0) + 1);
  }
  timerCriado(e) {
  ...
  requisicao(e, a, i) {
    const t = this.de(e).requisicoes;
    t.total++, i || t.falhas++, t.duracaoTotal += a, a > t.pior && (t.pior = a);
  }
  memoriaAmostrada(e) {
    this.memoria.push(e), this.memoria.length > Na && this.memoria.shift(), e.usado < this.pisoGlobal && (this.pisoGlobal = e.usado), e.usado > this.picoGlobal && (this.picoGlobal = e.usado), this.valoresDeMemoria.size < Fa && this.valoresDeMemoria.add(e.usado);
  }
  tarefaLonga(e) {
    this.tarefas.push(e), this.tarefas.length > Ra && this.tarefas.shift(), this.tarefasTotal++, this.tarefasDuracao += e.duracao, e.duracao > this.tarefasPior && (this.tarefasPior = e.duracao), e.em < Ge && this.tarefasNaCarga++;
  }
  // ── Ciclo de navegação ────────────────────────────────────────────────────
  /**
  ...
  contagem() {
    const e = (a) => {
      let i = 0;
      for (const t of this.componentes.values()) i += a(t).criados - a(t).encerrados;
      return i;
    };
    return {
  ...
  // ── Leitura ───────────────────────────────────────────────────────────────
  leituraDeMemoria() {
    const e = this.memoria, a = e[0], i = e[e.length - 1], t = e.length, o = Math.max(1, Math.floor(t / 3)), n = t ? _e(e.slice(0, o)) : 0, c = t ? _e(e.slice(t - o)) : 0, s = c - n, d = this.valoresDeMemoria.size, m = t ? _e(e.slice(o, t - o)) : 0, p = c - m;
    let h;
    return t < Ba ? h = `Só ${t} amostra(s) — o piso ainda não significa nada.` : d < 2 ? h = `${t} amostras, mas um único valor de heap: o navegador ainda não atualizou a leitura. Sem informação — precisa de sessão longa.` : s > ve && p > ve ? h = `Piso subiu ${(s / 1048576).toFixed(1)} MB e AINDA sobe (${(p / 1048576).toFixed(1)} MB no último terço) — isto é retenção.` : s > ve ? h = `Subiu ${(s / 1048576).toFixed(1)} MB desde a carga e estabilizou em ${(c / 1048576).toFixed(0)} MB — é custo de partida, não vazamento.` : h = `Piso estável em ${d} degraus — a variação do heap é coleta de lixo, não vazamento.`, {
      amostras: t,
      ...a ? { primeira: a } : {},
      ...i ? { ultima: i } : {},
      crescimento: a && i ? i.usado - a.usado : 0,
  ...
  leituraDeTarefas(e) {
    const a = this.tarefasTotal - this.tarefasNaCarga, i = Math.max(0, e - Ge) / 6e4;
    return {
      ...
      depoisDaCarga: a,
      porMinuto: i > 0 ? Number((a / i).toFixed(1)) : 0
    };
  }
  /**
  ...
  instantaneo(e) {
    const i = [...this.componentes.values()].sort(
      ...
      render: { ...c.render },
      motivos: [...c.motivos.entries()].map(([s, d]) => ({ motivo: s, total: d })).sort((s, d) => d.total - s.total).slice(0, Va),
      timers: { ...c.timers },
      listeners: { ...c.listeners },
      assinaturas: { ...c.assinaturas },
      requisicoes: { ...c.requisicoes }
    })), t = this.contagem(), o = Math.round(e - this.inicio), n = this.marca;
    return {
      formato: 2,
      build: this.buildId,
      ...
      ...n ? {
        desdeAMarca: {
          instancias: t.instancias - n.instancias,
          timers: t.timers - n.timers,
          listeners: t.listeners - n.listeners,
          assinaturas: t.assinaturas - n.assinaturas
        }
      } : {}
    };
  }
...
const y = new Ua(), Ga = 5e3, Z = { timerMemoria: void 0, observador: void 0 };
function Ha() {
  const r = performance;
  if (r.memory)
...
function Qa(r) {
  ...
  const e = () => {
    const a = Ha();
    a && y.memoriaAmostrada({ em: Math.round(performance.now()), ...a });
  };
  if (e(), Z.timerMemoria = window.setInterval(e, Ga), "PerformanceObserver" in window)
    try {
      const a = new PerformanceObserver((i) => {
        for (const t of i.getEntries())
          y.tarefaLonga({
            em: Math.round(t.startTime),
            duracao: Math.round(t.duration)
          });
      });
      a.observe({ entryTypes: ["longtask"] }), Z.observador = a;
    } catch {
    }
}
function Xa() {
...
function I(r, e, a) {
  const i = window.setTimeout(() => {
    K.delete(i), y.timerEncerrado(r), e();
  }, a);
...
function $(r, e) {
  if (e === void 0) return;
  const a = K.get(e);
  a !== void 0 && (a === "intervalo" ? window.clearInterval(e) : window.clearTimeout(e), K.delete(e), y.timerEncerrado(r));
}
function Ya() {
  return K.size;
}
function se(r, e, a, i, t) {
  e.addEventListener(a, i, t), y.listenerCriado(r);
}
function ce(r, e, a, i, t) {
  e.removeEventListener(a, i, t), y.listenerEncerrado(r);
}
function B(r, e, a) {
  y.requisicao(r, e, a);
}
...
function Ie(r, e, a) {
  ...
  try {
    ...
    y.renderizou(r, performance.now() - i, a);
  }
}
function ua() {
...
function pe() {
  return { ...y.instantaneo(performance.now()), timersVivos: Ya() };
}
function Ja() {
  if (Xa()) return;
  Qa(Za());
  const r = {
    instantaneo: pe,
    texto: () => JSON.stringify(pe(), null, 2),
    zerar: () => y.zerar(),
    marcar: ua,
    limparMarca: Wa
  };
 ...
 */
const de = globalThis, Oe = de.ShadowRoot && (de.ShadyCSS === void 0 || de.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype, Pe = Symbol(), He = /* @__PURE__ */ new WeakMap();
let ha = class {
  constructor(e, a, i) {
  ...
  get styleSheet() {
    ...
    if (Oe && e === void 0) {
      ...
      i && (e = He.get(a)), e === void 0 && ((this.o = e = new CSSStyleSheet()).replaceSync(this.cssText), i && He.set(a, e));
    }
    return e;
  }
  toString() {
...
const Ka = (r) => new ha(typeof r == "string" ? r : r + "", void 0, Pe), _ = (r, ...e) => {
  const a = r.length === 1 ? r[0] : e.reduce((i, t, o) => i + ((n) => {
    if (n._$cssResult$ === !0) return n.cssText;
    if (typeof n == "number") return n;
    throw Error("Value passed to 'css' function must be a 'css' function result: " + n + ". Use 'unsafeCSS' to pass non-literal values, but take care to ensure page security.");
  })(t) + r[o + 1], r[0]);
  return new ha(a, r, Pe);
}, ei = (r, e) => {
  if (Oe) r.adoptedStyleSheets = e.map((a) => a instanceof CSSStyleSheet ? a : a.styleSheet);
  else for (const a of e) {
    const i = document.createElement("style"), t = de.litNonce;
    t !== void 0 && i.setAttribute("nonce", t), i.textContent = a.cssText, r.appendChild(i);
  }
}, Qe = Oe ? (r) => r : (r) => r instanceof CSSStyleSheet ? ((e) => {
  let a = "";
 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const { is: ai, defineProperty: ii, getOwnPropertyDescriptor: ti, getOwnPropertyNames: oi, getOwnPropertySymbols: ri, getPrototypeOf: ni } = Object, fe = globalThis, Xe = fe.trustedTypes, si = Xe ? Xe.emptyScript : "", ci = fe.reactiveElementPolyfillSupport, J = (r, e) => r, Me = { toAttribute(r, e) {
  switch (e) {
    case Boolean:
      r = r ? si : null;
      break;
  ...
  return a;
} }, ga = (r, e) => !ai(r, e), Ye = { attribute: !0, type: String, converter: Me, reflect: !1, useDefault: !1, hasChanged: ga };
...
let F = class extends HTMLElement {
  ...
  static createProperty(e, a = Ye) {
    if (a.state && (a.attribute = !1), this._$Ei(), this.prototype.hasOwnProperty(e) && ((a = Object.create(a)).wrapped = !0), this.elementProperties.set(e, a), !a.noAccessor) {
      const i = Symbol(), t = this.getPropertyDescriptor(e, i, a);
      t !== void 0 && ii(this.prototype, e, t);
    }
  }
  static getPropertyDescriptor(e, a, i) {
    const { get: t, set: o } = ti(this.prototype, e) ?? { get() {
      return this[a];
    }, set(n) {
      this[a] = n;
    } };
  ...
  static finalize() {
    ...
    if (this.finalized = !0, this._$Ei(), this.hasOwnProperty(J("properties"))) {
      const a = this.properties, i = [...oi(a), ...ri(a)];
      for (const t of i) this.createProperty(t, a[t]);
    }
    const e = this[Symbol.metadata];
    if (e !== null) {
      const a = litPropertyMetadata.get(e);
      if (a !== void 0) for (const [i, t] of a) this.elementProperties.set(i, t);
    }
    this._$Eh = /* @__PURE__ */ new Map();
    for (const [a, i] of this.elementProperties) {
      const t = this._$Eu(a, i);
  ...
  static finalizeStyles(e) {
    ...
    if (Array.isArray(e)) {
      ...
      for (const t of i) a.unshift(Qe(t));
    } else e !== void 0 && a.push(Qe(e));
    return a;
  }
  static _$Eu(e, a) {
    const i = a.attribute;
  ...
  _$ET(e, a) {
    const i = this.constructor.elementProperties.get(e), t = this.constructor._$Eu(e, i);
    if (t !== void 0 && i.reflect === !0) {
      const o = (i.converter?.toAttribute !== void 0 ? i.converter : Me).toAttribute(a, i.type);
      this._$Em = e, o == null ? this.removeAttribute(t) : this.setAttribute(t, o), this._$Em = null;
    }
  }
  _$AK(e, a) {
    const i = this.constructor, t = i._$Eh.get(e);
    if (t !== void 0 && this._$Em !== t) {
      const o = i.getPropertyOptions(t), n = typeof o.converter == "function" ? { fromAttribute: o.converter } : o.converter?.fromAttribute !== void 0 ? o.converter : Me;
      this._$Em = t;
      const c = n.fromAttribute(a, o.type);
      this[t] = c ?? this._$Ej?.get(t) ?? c, this._$Em = null;
    }
  }
  requestUpdate(e, a, i, t = !1, o) {
    if (e !== void 0) {
      const n = this.constructor;
      if (t === !1 && (o = this[e]), i ??= n.getPropertyOptions(e), !((i.hasChanged ?? ga)(o, a) || i.useDefault && i.reflect && o === this._$Ej?.get(e) && !this.hasAttribute(n._$Eu(e, i)))) return;
      this.C(e, a, i);
    }
    this.isUpdatePending === !1 && (this._$ES = this._$EP());
  }
  C(e, a, { useDefault: i, reflect: t, wrapped: o }, n) {
    i && !(this._$Ej ??= /* @__PURE__ */ new Map()).has(e) && (this._$Ej.set(e, n ?? a ?? this[e]), o !== !0 || n !== void 0) || (this._$AL.has(e) || (this.hasUpdated || i || (a = void 0), this._$AL.set(e, a)), t === !0 && this._$Em !== e && (this._$Eq ??= /* @__PURE__ */ new Set()).add(e));
  }
  async _$EP() {
  ...
  performUpdate() {
    ...
    if (!this.hasUpdated) {
      ...
      if (i.size > 0) for (const [t, o] of i) {
        const { wrapped: n } = o, c = this[t];
        n !== !0 || this._$AL.has(t) || c === void 0 || this.C(t, void 0, o, c);
      }
    }
    let e = !1;
    const a = this._$AL;
...
F.elementStyles = [], F.shadowRootOptions = { mode: "open" }, F[J("elementProperties")] = /* @__PURE__ */ new Map(), F[J("finalized")] = /* @__PURE__ */ new Map(), ci?.({ ReactiveElement: F }), (fe.reactiveElementVersions ??= []).push("2.1.2");
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
const Le = globalThis, We = (r) => r, me = Le.trustedTypes, Ze = me ? me.createPolicy("lit-html", { createHTML: (r) => r }) : void 0, ba = "$lit$", T = `lit$${Math.random().toFixed(9).slice(2)}$`, fa = "?" + T, li = `<${fa}>`, j = document, ee = () => j.createComment(""), ae = (r) => r === null || typeof r != "object" && typeof r != "function", Ne = Array.isArray, di = (r) => Ne(r) || typeof r?.[Symbol.iterator] == "function", we = `[ 	
\f\r]`, Y = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, Je = /-->/g, Ke = />/g, O = RegExp(`>|${we}(?:([^\\s"'>=/]+)(${we}*=${we}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), ea = /'/g, aa = /"/g, xa = /^(?:script|style|textarea|title)$/i, va = (r) => (e, ...a) => ({ _$litType$: r, strings: e, values: a }), l = va(1), ye = va(2), G = Symbol.for("lit-noChange"), u = Symbol.for("lit-nothing"), ia = /* @__PURE__ */ new WeakMap(), L = j.createTreeWalker(j, 129);
function _a(r, e) {
...
const pi = (r, e) => {
  ...
  let t, o = e === 2 ? "<svg>" : e === 3 ? "<math>" : "", n = Y;
  for (let c = 0; c < a; c++) {
    const s = r[c];
    let d, m, p = -1, h = 0;
    for (; h < s.length && (n.lastIndex = h, m = n.exec(s), m !== null); ) h = n.lastIndex, n === Y ? m[1] === "!--" ? n = Je : m[1] !== void 0 ? n = Ke : m[2] !== void 0 ? (xa.test(m[2]) && (t = RegExp("</" + m[2], "g")), n = O) : m[3] !== void 0 && (n = O) : n === O ? m[0] === ">" ? (n = t ?? Y, p = -1) : m[1] === void 0 ? p = -2 : (p = n.lastIndex - m[2].length, d = m[1], n = m[3] === void 0 ? O : m[3] === '"' ? aa : ea) : n === aa || n === ea ? n = O : n === Je || n === Ke ? n = Y : (n = O, t = void 0);
    const b = n === O && r[c + 1].startsWith("/>") ? " " : "";
    o += n === Y ? s + li : p >= 0 ? (i.push(d), s.slice(0, p) + ba + s.slice(p) + T + b) : s + T + (p === -2 ? c : b);
  }
  return [_a(r, o + (r[a] || "<?>") + (e === 2 ? "</svg>" : e === 3 ? "</math>" : "")), i];
};
class ie {
  constructor({ strings: e, _$litType$: a }, i) {
    ...
    this.parts = [];
    let o = 0, n = 0;
    const c = e.length - 1, s = this.parts, [d, m] = pi(e, a);
    if (this.el = ie.createElement(d, i), L.currentNode = this.el.content, a === 2 || a === 3) {
      const p = this.el.content.firstChild;
      p.replaceWith(...p.childNodes);
    }
    for (; (t = L.nextNode()) !== null && s.length < c; ) {
      if (t.nodeType === 1) {
        if (t.hasAttributes()) for (const p of t.getAttributeNames()) if (p.endsWith(ba)) {
          const h = m[n++], b = t.getAttribute(p).split(T), f = /([.?@])?(.*)/.exec(h);
          s.push({ type: 1, index: o, name: f[2], strings: b, ctor: f[1] === "." ? ui : f[1] === "?" ? hi : f[1] === "@" ? gi : xe }), t.removeAttribute(p);
        } else p.startsWith(T) && (s.push({ type: 6, index: o }), t.removeAttribute(p));
        if (xa.test(t.tagName)) {
          const p = t.textContent.split(T), h = p.length - 1;
          if (h > 0) {
            t.textContent = me ? me.emptyScript : "";
            for (let b = 0; b < h; b++) t.append(p[b], ee()), L.nextNode(), s.push({ type: 2, index: ++o });
            t.append(p[h], ee());
          }
        }
      } else if (t.nodeType === 8) if (t.data === fa) s.push({ type: 2, index: o });
      else {
        let p = -1;
        for (; (p = t.data.indexOf(T, p + 1)) !== -1; ) s.push({ type: 7, index: o }), p += T.length - 1;
      }
      o++;
    }
  }
...
function H(r, e, a = r, i) {
  if (e === G) return e;
  let t = i !== void 0 ? a._$Co?.[i] : a._$Cl;
  const o = ae(e) ? void 0 : e._$litDirective$;
  return t?.constructor !== o && (t?._$AO?.(!1), o === void 0 ? t = void 0 : (t = new o(r), t._$AT(r, a, i)), i !== void 0 ? (a._$Co ??= [])[i] = t : a._$Cl = t), t !== void 0 && (e = H(r, t._$AS(r, e.values), t, i)), e;
}
class mi {
  constructor(e, a) {
  ...
  u(e) {
    const { el: { content: a }, parts: i } = this._$AD, t = (e?.creationScope ?? j).importNode(a, !0);
    L.currentNode = t;
    let o = L.nextNode(), n = 0, c = 0, s = i[0];
    for (; s !== void 0; ) {
      if (n === s.index) {
        let d;
        s.type === 2 ? d = new te(o, o.nextSibling, this, e) : s.type === 1 ? d = new s.ctor(o, s.name, s.strings, this, e) : s.type === 6 && (d = new bi(o, this, e)), this._$AV.push(d), s = i[++c];
      }
      n !== s?.index && (o = L.nextNode(), n++);
    }
    return L.currentNode = j, t;
  }
  p(e) {
    let a = 0;
...
class te {
  ...
  constructor(e, a, i, t) {
    this.type = 2, this._$AH = u, this._$AN = void 0, this._$AA = e, this._$AB = a, this._$AM = i, this.options = t, this._$Cv = t?.isConnected ?? !0;
  }
  get parentNode() {
  ...
  _$AI(e, a = this) {
    e = H(this, e, a), ae(e) ? e === u || e == null || e === "" ? (this._$AH !== u && this._$AR(), this._$AH = u) : e !== this._$AH && e !== G && this._(e) : e._$litType$ !== void 0 ? this.$(e) : e.nodeType !== void 0 ? this.T(e) : di(e) ? this.k(e) : this._(e);
  }
  O(e) {
  ...
  $(e) {
    const { values: a, _$litType$: i } = e, t = typeof i == "number" ? this._$AC(e) : (i.el === void 0 && (i.el = ie.createElement(_a(i.h, i.h[0]), this.options)), i);
    if (this._$AH?._$AD === t) this._$AH.p(a);
    else {
      const o = new mi(t, this), n = o.u(this.options);
      o.p(a), this.T(n), this._$AH = o;
    }
  }
  _$AC(e) {
    let a = ia.get(e.strings);
    return a === void 0 && ia.set(e.strings, a = new ie(e)), a;
  }
  k(e) {
    Ne(this._$AH) || (this._$AH = [], this._$AR());
    const a = this._$AH;
    let i, t = 0;
    for (const o of e) t === a.length ? a.push(i = new te(this.O(ee()), this.O(ee()), this, this.options)) : i = a[t], i._$AI(o), t++;
    t < a.length && (this._$AR(i && i._$AB.nextSibling, t), a.length = t);
  }
...
class xe {
  ...
  constructor(e, a, i, t, o) {
    this.type = 1, this._$AH = u, this._$AN = void 0, this.element = e, this.name = a, this._$AM = t, this.options = o, i.length > 2 || i[0] !== "" || i[1] !== "" ? (this._$AH = Array(i.length - 1).fill(new String()), this.strings = i) : this._$AH = u;
  }
  _$AI(e, a = this, i, t) {
    const o = this.strings;
    let n = !1;
    if (o === void 0) e = H(this, e, a, 0), n = !ae(e) || e !== this._$AH && e !== G, n && (this._$AH = e);
    else {
      const c = e;
      let s, d;
      for (e = o[0], s = 0; s < o.length - 1; s++) d = H(this, c[i + s], a, s), d === G && (d = this._$AH[s]), n ||= !ae(d) || d !== this._$AH[s], d === u ? e = u : e !== u && (e += (d ?? "") + o[s + 1]), this._$AH[s] = d;
    }
    n && !t && this.j(e);
  }
  j(e) {
...
class gi extends xe {
  constructor(e, a, i, t, o) {
    super(e, a, i, t, o), this.type = 5;
  }
  _$AI(e, a = this) {
    if ((e = H(this, e, a, 0) ?? u) === G) return;
    const i = this._$AH, t = e === u && i !== u || e.capture !== i.capture || e.once !== i.once || e.passive !== i.passive, o = e !== u && (i === u || t);
    t && this.element.removeEventListener(this.name, this, i), o && this.element.addEventListener(this.name, this, e), this._$AH = e;
  }
  handleEvent(e) {
...
const fi = Le.litHtmlPolyfillSupport;
fi?.(ie, te), (Le.litHtmlVersions ??= []).push("3.3.3");
const xi = (r, e, a) => {
  const i = a?.renderBefore ?? e;
  let t = i._$litPart$;
  if (t === void 0) {
    const o = a?.renderBefore ?? null;
    i._$litPart$ = t = new te(e.insertBefore(ee(), o), o, void 0, a ?? {});
  }
  return t._$AI(r), t;
};
/**
...
class C extends F {
  ...
  update(e) {
    const a = this.render();
    this.hasUpdated || (this.renderOptions.isConnected = this.isConnected), super.update(e), this._$Do = xi(a, this.renderRoot, this.renderOptions);
  }
  connectedCallback() {
...
const vi = Re.litElementPolyfillSupport;
vi?.({ LitElement: C });
...
`, _i = "media_player.spotifyplus_bruno_helasio", W = ["cool", "heat", "fan_only", "dry", "heat_cool", "auto"], le = ["playing", "paused", "on"], je = [
  {
    id: "sala",
    name: "Sala",
    section: "sala",
...
function wi() {
  const r = [];
  for (const e of je)
    for (const [a, i] of Object.entries(e.entities))
      if (typeof i == "string")
        r.push({ entityId: i, roomId: e.id, field: a });
      else if (Array.isArray(i))
        for (const t of i) r.push({ entityId: t, roomId: e.id, field: a });
  return r;
}
const yi = [
  {
...
function qi(r) {
  const e = [], a = [];
  for (const i of yi)
    r?.states?.[i.entity] ? e.push(i) : a.push(i);
...
function ki(r) {
  ...
  const a = [];
  let i = 0;
  for (const { entityId: t, roomId: o, field: n } of e) {
    const c = r.states[t];
    c ? c.state === "unavailable" ? a.push({ entityId: t, roomId: o, field: n, problem: "unavailable" }) : i++ : a.push({ entityId: t, roomId: o, field: n, problem: "missing" });
  }
  return { total: e.length, ok: i, issues: a };
}
function $i(r) {
  return qi(r).ausentes.map((e) => ({
    tipo: "scene",
    entityId: e.entity,
    nome: e.name,
...
function Si(r, e) {
  ...
  const a = String(r.frontend_stream_type ?? "");
  return a === "web_rtc" ? "web_rtc" : a === "hls" ? "hls" : "instantaneo";
}
async function Ci(r) {
  const e = ya(r), a = r?.callWS;
  ...
  const i = await Promise.all(
    e.cameras.map(async (o) => {
      ...
      try {
        ...
        return c.includes("web_rtc") ? { ...o, caminho: "web_rtc" } : c.includes("hls") ? { ...o, caminho: "hls" } : o;
      } catch {
        return o;
      }
    })
  ...
  for (const o of i) t[o.caminho]++;
  return {
    streamCarregado: e.streamCarregado,
    cameras: i,
    resumo: t,
    veredito: qa(t, i.length, e.streamCarregado)
  };
}
function ya(r) {
  const e = {
  ...
  for (const [o, n] of Object.entries(r.states)) {
    ...
    a.push({
      entityId: o,
      nome: String(c.friendly_name ?? o),
      estado: String(n.state),
      caminho: Si(c, String(n.state)),
      suportaStream: (s & Ai) !== 0
    });
  }
  a.sort((o, n) => o.entityId.localeCompare(n.entityId));
  const i = {
    web_rtc: 0,
    hls: 0,
    instantaneo: 0,
  ...
  for (const o of a) i[o.caminho]++;
  const t = a.some((o) => o.suportaStream);
  return {
    streamCarregado: t,
    cameras: a,
    resumo: i,
    veredito: qa(i, a.length, t)
  };
}
function qa(r, e, a) {
...
class Ei extends C {
   ...
   */
  _runtime() {
    const e = pe(), a = (s) => (s / 1048576).toFixed(1) + " MB", i = (s) => s.toFixed(1) + " ms", t = e.vazamentos, o = t.timers + t.listeners + t.assinaturas, n = e.desdeAMarca, c = n ? n.instancias + n.timers + n.listeners + n.assinaturas : 0;
    return l`
      <div>
        <h2>Runtime</h2>
        <dl>
  ...
  _cameras() {
    !this._sondaProfunda && !this._sondando && this._hass && (this._sondando = !0, Ci(this._hass).then((a) => {
      this._sondaProfunda = a, this._sondando = !1, this.requestUpdate();
    }));
    const e = this._sondaProfunda ?? ya(this._hass);
    return e.cameras.length ? l`
      <div>
        ...
        <ul>
          ${e.cameras.map((a) => l`<li>${a.entityId} → ${a.caminho}${a.suportaStream ? " · stream" : ""}</li>`)}
        </ul>
      </div>
    ` : u;
  }
  ...
  async _copiarBaseline() {
    const e = JSON.stringify(pe(), null, 2);
    try {
      ...
      a.remove(), this._mensagem = i ? "Baseline copiada." : "Não foi possível copiar — use brunoRuntime.texto().";
    }
    this.requestUpdate();
  }
  render() {
    const e = this._env, a = ki(this._hass), i = a.issues.filter((n) => n.problem === "missing"), t = a.issues.filter((n) => n.problem === "unavailable"), o = $i(this._hass);
    return l`
      <div class="card">
        <div>
          <h2>Diagnóstico</h2>
...
ue.customCards.some((r) => r.type === "bruno-diagnostics") || ue.customCards.push({
...
function Mi(r) {
  const e = r / 6e4, a = r / 36e5, i = r / 864e5;
  return e < 1 ? "<1m" : e < 60 ? `${Math.trunc(e)}m` : a < 24 ? `${Math.trunc(a)}h` : `${Math.trunc(i)}d`;
}
function Ti(r) {
  const { hass: e, groupEntityId: a, activeSensorId: i, fallbackLightIds: t = [] } = r, o = r.now ?? Date.now(), n = a ? e.states[a] : void 0, c = i ? e.states[i] : void 0;
  let s = null;
  ...
  const m = Di(n);
  s === null && m.length > 0 && (s = m.filter((b) => e.states[b]?.state === "on").length), s === null && t.length > 0 && (s = t.filter((b) => e.states[b]?.state === "on").length), s === null && (s = n?.state === "on" ? 1 : 0);
  let p = String(c?.attributes.lights_elapsed ?? "");
  if (!p) {
    const b = m.length > 0 ? m : t;
    let f = null;
    for (const v of b) {
      const A = e.states[v];
...
function Ii(r) {
  const { hass: e, semanticSensorId: a, motionRecentId: i, occupancyId: t } = r, o = a ? e.states[a] : void 0, n = String(o?.state ?? "").toLowerCase(), c = o?.attributes.display;
  if (c && !["none", "unknown", "unavailable", ""].includes(n))
    return String(c).trim();
  const s = i ? e.states[i] : void 0;
  return s && s.state !== "on" ? "" : (t ? e.states[t] : void 0)?.state === "on" ? "Ocupado" : "";
}
function ta(r, e, a = ["on", "home", "active", "yes"]) {
  if (!e) return !1;
  const i = r.states[e];
  return i ? a.includes(String(i.state).toLowerCase()) : !1;
}
function oa(r, e, a = "") {
  const i = typeof e == "string" ? [e] : e ?? [];
  for (const t of i) {
    const o = r.states[t], n = String(o?.state ?? "").toLowerCase();
...
function Pi(r, e) {
  ...
  return Oi.some((t) => {
    const o = P(i[t]);
    return !!(o && (o === a || o.includes(a) || o.length >= 10 && a.includes(o)));
  });
}
const ka = ["playing", "paused"];
function $a(r, e, a) {
  return !r || !ka.includes(String(r.state).toLowerCase()) ? !1 : Pi(r.attributes, e) ? !0 : Li(r.attributes, a);
}
function Li(r, e) {
  if (!e || !ka.includes(String(e.state).toLowerCase())) return !1;
  const a = e.attributes ?? {};
  if (P(
    [a.app_name, a.source, a.media_content_type, a.media_channel].join(" ")
  ).includes("spotify")) return !0;
  const t = r ?? {}, o = P(a.media_title), n = P(t.media_title);
  if (o && n && o === n) return !0;
  const c = P(a.media_artist), s = P(t.media_artist);
  return !!(o && n && o.includes(n) && c && s && c === s);
}
const Ni = "∅", Ri = (r) => r ? `${r.state}@${r.last_changed}` : Ni;
class Ve {
  constructor(e = [], a = {}) {
  ...
  observar(e) {
    ...
    for (const t of e)
      typeof t != "string" || !t || i.has(t) || (i.add(t), a.push(t));
  ...
  projetar(e, a) {
    return (this.projecoes[a] ?? Ri)(e.states[a]);
  }
  /**
  ...
  mudancas(e) {
    ...
    for (const i of this.ids) {
      const t = this.projetar(e, i);
      t !== this.ultimo.get(i) && (this.ultimo.set(i, t), (a ??= []).push(i));
    }
    return a ?? [];
  }
...
function ra(r, e = 0) {
  const a = [], i = /* @__PURE__ */ new Set(), t = (o, n) => {
    if (!(n > Vi || o == null)) {
      if (typeof o == "string") {
        ji.test(o) && !i.has(o) && (i.add(o), a.push(o));
        return;
      }
      if (Array.isArray(o)) {
        for (const c of o) t(c, n + 1);
        return;
      }
      if (typeof o == "object")
        for (const c of Object.values(o)) t(c, n + 1);
    }
  };
  return t(r, e), a;
}
const qe = "bruno-room-tile", Bi = 1200, Fi = 560, na = 10;
function sa() {
  return {
    pointerId: null,
    down: !1,
...
class Ui extends C {
  constructor() {
    super(...arguments), this._lastAction = 0, this._observador = new Ve(), this._motivo = "", this._gestures = {
      room: sa(),
      nav: sa()
  ...
  set hass(e) {
    ...
    a.length !== 0 && (this._motivo = Be(a), this.requestUpdate());
  }
  _watched() {
  ...
  connectedCallback() {
    super.connectedCallback(), Te(qe), this._tileModeCache = void 0, globalThis.addEventListener?.("bruno-theme-changed", this._onThemeChanged);
  }
  disconnectedCallback() {
    super.disconnectedCallback(), De(qe), globalThis.removeEventListener?.("bruno-theme-changed", this._onThemeChanged);
  ...
  _onDown(e, a) {
    ...
    a.stopPropagation(), i.down = !0, i.moved = !1, i.holdFired = !1, i.pointerId = a.pointerId, i.startX = a.clientX, i.startY = a.clientY, this._classe(this._alvoSeletor(e), "is-pressed", !0), i.holdTimer = this._later(() => {
  ...
  _onMove(e, a) {
    const i = this._gestures[e];
    if (!i.down || a.pointerId !== i.pointerId) return;
    const t = Math.abs(a.clientX - i.startX), o = Math.abs(a.clientY - i.startY);
    t <= na && o <= na || (i.moved = !0, i.holdTimer !== null && (window.clearTimeout(i.holdTimer), this._timers.delete(i.holdTimer), i.holdTimer = null), this._classe(this._alvoSeletor(e), "is-pressed", !1));
  }
  _onUp(e, a) {
    const i = this._gestures[e];
    if (a.pointerId !== i.pointerId) {
      a.stopPropagation();
      return;
    }
    a.preventDefault(), a.stopPropagation();
    const t = i.down, o = i.moved, n = i.holdFired;
    if (this._resetGesture(e), !(!t || o || n)) {
      if (e === "room") {
        this._runAction("tap");
        return;
      }
  ...
  _runAction(e) {
    const a = this._room, i = this._hass;
    if (!a || !i) return;
    if (ke(e), e === "hold") {
    ...
    const t = Date.now();
    if (t - this._lastAction < Bi) return;
    this._lastAction = t;
    const o = a.toggleTarget ?? a.entities.lightGroup ?? a.entities.lights?.[0];
  ...
  _posicionarPainel() {
    ...
    const i = 10, t = window.innerWidth || document.documentElement.clientWidth, o = window.innerHeight || document.documentElement.clientHeight, n = e.offsetWidth || 520, c = e.offsetHeight || 240;
    let s = a.right - n;
    s = Math.min(Math.max(s, i), Math.max(i, t - n - i));
    let d = a.bottom + i;
    d + c > o - i && (d = a.top - c - i), d = Math.min(Math.max(d, i), Math.max(i, o - c - i)), e.style.left = `${Math.round(s)}px`, e.style.top = `${Math.round(d)}px`;
  }
  _alternarLuzDoPainel(e) {
    this._hass && (ke("tap"), this._hass.callService("light", "toggle", { entity_id: e }, { entity_id: e }));
  }
  // ── Modelo ───────────────────────────────────────────────────────────────
  _dots() {
    const e = this._hass, a = this._room;
    if (!e || !a) return [];
    const i = a.activeSensor ? e.states[a.activeSensor] : void 0, t = (o) => {
      const n = (o.states ?? []).map((m) => m.toLowerCase()), c = (o.entities ?? []).some((m) => {
        const p = e.states[m];
        return !!p && n.includes(String(p?.state ?? "").toLowerCase());
      }), s = o.activeAttr ? ca(i?.attributes[o.activeAttr]) : !1, d = o.spotifyDevice ? $a(e.states[_i], o.spotifyDevice) : !1;
      return c || s || d;
    };
    return (a.statusDots ?? []).filter(t).map((o) => ({ icon: o.icon, label: o.label, tone: o.tone }));
  }
  _statusLines() {
    const e = this._hass, a = this._room;
    if (!e || !a) return [];
    ...
    }), t = a.entities.semanticState ? Ii({
      hass: e,
      semanticSensorId: a.entities.semanticState,
      motionRecentId: a.entities.motionRecent,
      occupancyId: a.entities.occupancy
    }) : "", o = [];
    i.label ? o.push(i.label) : ta(e, a.entities.lightGroup) && o.push("On");
    const n = this._applianceLine();
    return n && o.push(n), t && o.push(t), o;
  }
  /** Linha do eletrodoméstico, no formato "Lavando / 12m". */
  _applianceLine() {
    const e = this._hass, a = this._room, i = a?.applianceLine;
    if (!e || !a || !i) return "";
    const t = a.activeSensor ? e.states[a.activeSensor] : void 0, o = (i.states ?? []).map((d) => d.toLowerCase()), n = i.entity ? e.states[i.entity] : void 0;
    if (!(!!n && o.includes(String(n?.state ?? "").toLowerCase()) || (i.activeAttr ? ca(t?.attributes[i.activeAttr]) : !1))) return "";
    const s = i.elapsedAttr ? String(t?.attributes[i.elapsedAttr] ?? "") : "";
  ...
  render() {
    ...
    if (!e) return u;
    const i = a ? ta(a, e.entities.lightGroup ?? e.entities.lights?.[0]) : !1, t = a ? oa(a, e.entities.temperature, "°") : "--", o = a ? oa(a, e.entities.humidity, "%") : "--", n = !!(e.entities.temperature ?? e.entities.humidity), c = this._statusLines(), s = this._dots(), d = "20260808-maquetes-premium-1", m = e.assetOff ? `/local/bruno-ui/assets/${e.assetOff}.png?v=${d}` : "", p = e.assetOn ? `/local/bruno-ui/assets/${e.assetOn}.png?v=${d}` : "", h = [
      "room-card",
      i ? "is-room-on" : "",
      this._tileMode ? "is-tile" : "",
      this._tileMode && this._config?.divider_left ? "has-divider" : ""
    ].filter(Boolean).join(" "), b = this._config?.name ?? e.name, f = !!(e.section ?? e.popup), v = e.popup, A = globalThis.BrunoThemeManager?.current?.() === "josh" ? "josh" : "default";
    return l`
      <div class=${h}>
            ...
            >
              <section class="room-popup-panel" role="document" @click=${(g) => g.stopPropagation()}>
                ...
                <div class="room-popup-lights">
                  ${v.lights.map((g) => {
      const S = a?.states[g.entity], Q = String(S?.state ?? "").toLowerCase(), X = Q === "on", oe = !S || ["unavailable", "unknown", "none", ""].includes(Q), w = [
        "room-popup-light",
        X ? "is-on" : "",
        oe ? "is-unavailable" : ""
      ].filter(Boolean).join(" ");
      return l`<button
                    ...
                    >
                      ...
                      <span class="room-popup-light-copy">
                        ...
                        <span>${oe ? "Indisponivel" : X ? "Ligada" : "Desligada"}</span>
                      </span>
                    </button>`;
    })}
                </div>
...
he.customCards.some((r) => r.type === "bruno-room-tile") || he.customCards.push({
...
}, U = /* @__PURE__ */ new Set(), R = { timer: void 0, ouvindoVisibilidade: !1 }, Hi = 1e3;
function za() {
  for (const r of [...U])
    try {
      r();
...
function Sa() {
  R.timer !== void 0 || U.size === 0 || Aa() || (R.timer = globalThis.setInterval(za, Hi));
}
function Ca() {
  R.timer !== void 0 && (globalThis.clearInterval(R.timer), R.timer = void 0);
}
function Qi() {
  if (Aa()) {
    Ca();
...
function Xi() {
  R.ouvindoVisibilidade || typeof document > "u" || (document.addEventListener("visibilitychange", Qi), R.ouvindoVisibilidade = !0);
}
function Yi(r) {
  U.add(r), Xi(), Sa();
  let e = !1;
  return () => {
    e || (e = !0, U.delete(r), U.size === 0 && Ca());
  };
}
const D = "ha-web-rtc-player", Wi = 6e3, $e = 48, ze = 27;
let Ae;
function Zi(r) {
  return r.split(".")[1] ?? r;
}
function N(r, e, a = 0, i = !0) {
  B(`marco: ${Zi(r)} · player webrtc · ${e}`, a, i);
}
function Ji() {
  return typeof customElements > "u" ? Promise.resolve(!1) : customElements.get(D) ? Promise.resolve(!0) : new Promise((r) => {
    let e = !1;
    const a = (t) => {
      e || (e = !0, globalThis.clearTimeout(i), r(t));
    }, i = globalThis.setTimeout(() => a(!!customElements.get(D)), Wi);
    customElements.whenDefined(D).then(() => a(!0));
  });
}
async function Ki(r, e) {
  if (typeof customElements > "u") return !1;
  if (customElements.get(D)) return !0;
  const a = globalThis.loadCardHelpers;
  if (typeof a != "function") return !1;
  try {
    const i = await a();
    if (customElements.get(D)) return !0;
    const t = i.createCardElement?.({
      type: "picture-entity",
      entity: r,
      camera_view: "live",
      show_name: !1,
    ...
    return t && e && (t.hass = e), await Ji();
  } catch {
    return !1;
  }
}
async function Ea(r, e) {
  if (typeof customElements < "u" && customElements.get(D)) return !0;
  const a = typeof performance < "u" ? performance.now() : Date.now();
  N(r, "ausente; carregando modulo", 0, !1), Ae ??= Ki(r, e).finally(() => {
    Ae = void 0;
  });
  const i = await Ae, t = typeof performance < "u" ? performance.now() : Date.now();
  return N(r, i ? "definido sob demanda" : "definicao indisponivel", t - a, i), i;
}
function Ma() {
  if (typeof customElements > "u" || !customElements.get(D)) return;
...
function et(r) {
  ...
  for (let s = 0; s + 3 < r.length; s += 4) {
    ...
    e.set(h, (e.get(h) ?? 0) + 1), a++;
  }
  if (a < 32) return !1;
  let i = 0, t = 0;
  for (const [s, d] of e)
    d <= i || (i = d, t = s);
  const o = (t & 15) * 16 + 8, n = (t >> 4 & 15) * 16 + 8, c = (t >> 8 & 15) * 16 + 8;
  return i / a >= 0.45 && n >= 80 && n - o >= 48 && n - c >= 48;
}
function Fe(r) {
  if (typeof document > "u") return !1;
  try {
    const e = document.createElement("canvas");
    e.width = $e, e.height = ze;
    const a = e.getContext("2d", { willReadFrequently: !0 });
    return a ? (a.drawImage(r, 0, 0, $e, ze), et(
      a.getImageData(0, 0, $e, ze).data
    )) : !1;
  } catch {
    return !1;
  }
...
const at = {
  ...
  principal: 1500,
  secundaria: 3e3
}, tt = 25e3, ot = 300, rt = 6e4, nt = 12e3, la = { comImagem: 2, semImagem: 4 };
function st(r, e) {
...
const ct = (r, e) => {
  ...
  return a.onload = () => t(Fe(a) ? "quadro-verde" : !0), a.onerror = () => t(!1), a.src = r, () => {
    i = !1, a.onload = null, a.onerror = null, a.src = "";
  };
}, lt = {
...
class Da {
  constructor(e = {}) {
    this.cameras = /* @__PURE__ */ new Map(), this.ligado = !1, this.carregador = e.carregador ?? ct, this.agenda = e.agenda ?? lt, this.aoCarregar = e.aoCarregar ?? (() => {
  ...
  definirAlvos(e) {
    ...
    for (const i of e) {
      ...
      a.add(i.entityId);
      const t = this.cameras.get(i.entityId);
      if (t) {
        t.alvo = i;
        continue;
      }
      this.cameras.set(i.entityId, {
        ...
        ultimaDuracao: 0,
        pior: 0
      }), this.ligado && this.agendarPrimeiro(i.entityId, a.size - 1);
    }
    for (const [i, t] of [...this.cameras])
      a.has(i) || (this.desmontar(t), this.cameras.delete(i));
  }
  ...
  buscar(e) {
    ...
    const a = st(e.alvo.base, Math.round(e.inicio) || 1);
    e.prazo = this.agenda.agendar(() => this.encerrar(e, "prazo", a), tt), e.abortar = this.carregador(
      a,
      (i) => this.encerrar(
        e,
  ...
  encerrar(e, a, i) {
    ...
    const t = this.agenda.agora() - e.inicio;
    e.ultimaDuracao = t, t > e.pior && (e.pior = t), e.ultimoDesfecho = a;
    const o = a === "ok" && e.quadros === 0;
    a === "ok" ? (e.quadros++, e.falhasSeguidas = 0, o && (e.primeiroQuadro = t), this.aoCarregar({ entityId: e.alvo.entityId, url: i, duracao: t, primeiro: o })) : (e.falhas++, e.falhasSeguidas++), this.aoMedir(e.alvo.entityId, t, a, o), this.ligado && this.agendarProximo(e, t);
  }
  agendarProximo(e, a) {
    const i = e.alvo.prioridade;
    let t = Math.max(it[i], Ta[i] - a);
    const o = e.quadros === 0, n = o ? la.semImagem : la.comImagem, c = o ? nt : rt;
    if (e.falhasSeguidas >= n) {
      const s = 2 ** Math.min(e.falhasSeguidas - n + 1, 5);
      t = Math.min(c, t * s);
    }
    e.timer = this.agenda.agendar(() => this.buscar(e), t);
  }
}
const dt = [
...
`, wt = {
  ...
  @media (max-width: 800px) {
    ...
    :host([data-room]) .room-subview {
      ...
      /* Altura da barra "Concluir" + o respiro entre ela e a folha. */
      --fone-fechar-h: 54px;
    ...
    :host([data-room]) .room-subview .cameras-card {
      ...
      height: auto;
      min-height: 0;
      grid-template-rows: clamp(34.32px, 2.42cqi, 57.2px)
    ...
    :host([data-room]) .room-subview .curtain-dock {
      ...
      grid-template-columns: 1fr;
      gap: clamp(7.8px, 0.55cqi, 13px);
      padding: clamp(10.92px, 0.77cqi, 18.2px) !important;
      border-radius: var(--fone-raio) !important;
      background: var(
        --bruno-liquid-surface-off-background,
        rgba(255, 255, 255, 0.062)
      ) !important;
      border: var(--bruno-liquid-surface-off-border, 1px solid rgba(255, 255, 255, 0.105)) !important;
      ...
      -webkit-backdrop-filter: var(--bruno-liquid-surface-off-filter, none) !important;
    }
    ...
    :host([data-room]) .room-subview .curtain-status { justify-self: start; }
    ...
    :host([data-room]) .room-subview .resumo-linha {
      ...
      min-height: 64px;
      padding: 0 clamp(10.92px, 0.77cqi, 18.2px);
      border-radius: var(--fone-raio);
      background: var(--bruno-liquid-surface-off-background, rgba(255, 255, 255, 0.062));
      border: var(--bruno-liquid-surface-off-border, 1px solid rgba(255, 255, 255, 0.105));
    ...
    :host([data-folha]) .room-subview .glass-card.appliances-card {
      ...
      height: auto;
      margin: 0;
      /* Piso: sem ele o hub de mídia com as duas fontes recolhidas abria com
         125px e lia como uma tira, não como folha.
         Teto: garante que a folha nunca alcance a câmera. */
      ...
      /* O respiro de baixo é o rodapé "Concluir", que flutua sobre a folha. */
      padding: 22px clamp(10.92px, 0.77cqi, 18.2px)
    ...
    :host([data-folha='luzes']) .room-subview .lights-card .module-head {
      min-height: 0;
      flex-wrap: wrap;
      gap: clamp(7.8px, 0.55cqi, 13px);
    }
    :host([data-folha='luzes']) .room-subview .head-actions {
      width: 100%;
      display: grid;
      grid-template-columns: repeat(2, minmax(0, 1fr));
    }
    :host([data-folha='luzes']) .room-subview .head-actions .chip-button,
    :host([data-folha='luzes']) .room-subview .zone-header {
      min-height: clamp(34.32px, 2.42cqi, 57.2px);
    }
    :host([data-folha='ac']) .room-subview .ac-card.ac-card-lean {
      grid-template-rows: clamp(34.32px, 2.42cqi, 57.2px)
        minmax(clamp(171.6px, 12.09cqi, 286px), auto) auto;
    }
    :host([data-folha='ac']) .room-subview .ac-lean-foot { align-items: stretch; }
    ...
    /* Rodapé DA FOLHA, não pílula solta: encosta na borda inferior, tem a
       mesma largura dela e um filete separando do conteúdo. A folha reserva
       espaço com padding-bottom, então nada fica escondido atrás. */
    :host([data-folha]) .room-subview .folha-fechar {
      display: flex;
      align-items: center;
      justify-content: center;
      position: fixed;
      ...
      border-radius: 0;
      background: var(--bruno-liquid-surface-off-background, rgba(20, 24, 33, 0.92));
    ...
    :host([data-folha]) .room-subview .folha-fechar:focus-visible {
      outline: 2px solid rgba(120, 178, 245, 0.85);
      outline-offset: 2px;
    }
    :host([data-room]) .room-subview .folha-fechar[hidden] { display: none; }
...
const kt = ["on", "playing", "paused", "idle"], $t = ["playing", "paused", "on", "idle"], zt = ["streaming", "recording", "idle", "on"], At = ["cool", "heat", "fan_only", "dry", "heat_cool", "auto"], St = ["cooling", "heating", "drying", "fan", "preheating"], Ct = ["off", "idle"], Et = 4e3, Mt = 3e4, Tt = 700, Dt = "/local/bruno-ui/assets/tcl-qled-mini-led-75.png?v=20260802-assets-resize-1", It = "/local/images/echo_pop.png?v=20260702-all-images-1", Ot = "/local/images/office_pc.png?v=20260702-all-images-1";
function V(r, e = 0) {
  const a = Number(r);
  return Number.isFinite(a) ? a.toFixed(e).replace(/\.0+$/, "") : "--";
}
function da(r) {
  const e = Math.max(0, Math.floor(Number(r) || 0)), a = Math.floor(e / 3600), i = Math.floor(e % 3600 / 60), t = e % 60;
  return a > 0 ? `${a}:${String(i).padStart(2, "0")}:${String(t).padStart(2, "0")}` : `${i}:${String(t).padStart(2, "0")}`;
}
function Ce(r) {
...
class Pt extends C {
  constructor() {
    super(...arguments), this._lightsOpen = !1, this._folha = null, this._fonteMidia = "", this._midiaAtivasAntes = [], this._menuMidiaAberto = !1, this._spotifyFerramentas = !1, this._painelClima = "", this._controlesCameraAbertos = !1, this._cameraAtiva = "", this._urlsCarregadas = {}, this._ultimaImagem = {}, this._motorCameras = new Da({
      ...
      const e = this._liveEl, a = this._liveEntity;
      if (!e || !a || !this.isConnected) return;
      const i = e.shadowRoot?.querySelector("video");
      if (!i || i.readyState < 2 || this._livePronto === a) return;
      if (Fe(i)) {
        this._quadroVerdeRegistrado !== a && (this._quadroVerdeRegistrado = a, N(
          a,
          "quadro verde rejeitado",
          performance.now() - this._liveIniciadoEm,
          !1
        )), $(x, this._timerQuadroVerde), this._timerQuadroVerde = I(x, () => {
          this._timerQuadroVerde = void 0, this._aoCarregarAoVivo();
        }, 700);
        return;
      }
      this._livePronto = a, this._estadoAoVivo = "ao-vivo", $(x, this._timerQuadroVerde), this._timerQuadroVerde = void 0, e.classList.add("is-ready"), $(x, this._timerAoVivo), this._timerAoVivo = void 0;
      const t = a.split(".")[1] ?? a;
      B(
        `marco: ${t} · player ${this._modoPlayer} · primeiro quadro`,
        performance.now() - this._liveIniciadoEm,
        !0
      ), this._sincronizarCameras();
    }, this._aoInformarStreams = (e) => {
      e.detail?.hasVideo === !1 && this._falharAoVivo("sem video");
    }, this._aoFecharDialogo = (e) => {
      e.detail?.dialog === "ha-more-info-dialog" && this._estadoAoVivo === "entregue-more-info" && (this._estadoAoVivo = "retomando", N(this._cameraAtiva, "more-info fechado; retomando"), $(x, this._timerRetomadaAoVivo), this._timerRetomadaAoVivo = I(x, () => {
    ...
    }, this._montadoEm = 0, this._quadrosNaTela = /* @__PURE__ */ new Set(), this._socorros = /* @__PURE__ */ new Set(), this._materialInjetado = !1, this._luzesAssentadas = !1, this._appsTvAbertos = !1;
  }
  static {
    this.properties = {};
  }
  setConfig(e) {
    ...
    const a = je.find((i) => i.id === e.room);
    if (!a) throw new Error(`bruno-room-subview: cômodo desconhecido "${e.room}"`);
    this._config = e, this._room = a, this._sub = Gi[e.room], this._config, this._hass, this._observador.observar([
      ...ra(a),
      ...ra(this._sub)
  ...
  update(e) {
    const a = this._motivo;
    this._motivo = "", Ie(x, () => super.update(e), a || this._motivoPadrao());
  }
  _motivoPadrao() {
  ...
  connectedCallback() {
    super.connectedCallback(), Te(x), this._estadoAoVivo = "ocioso", this._fallbackAoVivo = "", this._montadoEm = performance.now(), this._quadrosNaTela.clear(), this._socorros.clear(), this._aplicarAtributos();
    const e = globalThis;
    e.BrunoLiquidGlass?.apply?.(), e.BrunoSurfaceMaterial?.connect?.(this), this._injetarMaterial(), this._iniciarTimerCameras(), this._armarVigiaDeCameras(), this._iniciarTimerRelogio(), !this._ouvindoVisibilidade && typeof document < "u" && (se(x, document, "visibilitychange", this._aoMudarVisibilidade), this._ouvindoVisibilidade = !0), !this._ouvindoFechamentoDialogo && typeof window < "u" && (se(x, window, "dialog-closed", this._aoFecharDialogo), this._ouvindoFechamentoDialogo = !0);
  }
  /**
  ...
  _iniciarTimerRelogio() {
    this._cancelarRelogio || (this._cancelarRelogio = Yi(() => {
      const e = this._hora();
  ...
  _sincronizarCameras() {
    ...
    const a = e.find((o) => o.entity === this._cameraAtiva) ?? e[0], i = e.find((o) => o.online), t = a?.online || !i ? a : i;
    this._motorCameras.definirAlvos(
      e.filter((o) => !(this._liveEl?.isConnected && this._livePronto === o.entity)).map((o) => ({
        entityId: o.entity,
        base: o.base,
        prioridade: o.entity === t?.entity ? "principal" : "secundaria"
      }))
    ), this._cuidarDoAoVivo(t?.entity);
  }
  /**
  ...
  _cuidarDoAoVivo(e) {
    ...
    if (this._estadoAoVivo === "fallback" && this._fallbackAoVivo !== a && (this._estadoAoVivo = "ocioso", this._fallbackAoVivo = ""), this._estadoAoVivo === "entregue-more-info" || this._estadoAoVivo === "retomando" || this._estadoAoVivo === "fallback" || this._estadoAoVivo === "carregando-player") {
      this._pararAoVivo();
      return;
    }
    const i = this.shadowRoot?.querySelector(
      `.camera-live-slot[data-camera-live="${a}"]`
    );
    if (!i) return;
    if (!this._liveEl || this._liveEntity !== a) {
      this._pararAoVivo();
      const o = this._criarPlayer();
      if (!o) {
        this._estadoAoVivo = "carregando-player";
        const n = ++this._tokenDefinicaoPlayer;
        Ea(a, this._hass).then((c) => {
          !this.isConnected || n !== this._tokenDefinicaoPlayer || (this._estadoAoVivo = c ? "ocioso" : "fallback", this._fallbackAoVivo = c ? "" : a, this._sincronizarCameras());
        });
        return;
      }
      this._estadoAoVivo = "negociando", this._liveEl = o, this._liveEntity = a, se(x, o, "load", this._aoCarregarAoVivo), se(x, o, "streams", this._aoInformarStreams), i.appendChild(o), this._iniciarPlayerAposContexto(o, a);
      return;
    }
    const t = this._liveEl;
    t.parentElement !== i && i.appendChild(t), t.entityid !== a && (t.entityid = a);
  }
  /**
  ...
  _criarPlayer() {
    const e = Ma();
    if (e) {
      e.classList.add("camera-live-el"), e.setAttribute("muted", ""), e.setAttribute("playsinline", ""), e.setAttribute("autoplay", "");
      try {
        e.fitMode = "cover";
      } catch {
      }
      return this._modoPlayer = "webrtc", e;
    }
   ...
   */
  _iniciarPlayerAposContexto(e, a) {
    Promise.resolve(e.updateComplete).then(() => {
      this._liveEl !== e || this._liveEntity !== a || !e.isConnected || (this._liveIniciadoEm = performance.now(), e.entityid = a, N(a, "entityid atribuido"), this._armarPrazoAoVivo(e, a));
    }).catch(() => {
      this._liveEl === e && this._liveEntity === a && this._falharAoVivo("contexto");
    });
  }
  _armarPrazoAoVivo(e, a) {
    $(x, this._timerAoVivo), this._timerAoVivo = I(x, () => {
      this._timerAoVivo = void 0, !(this._liveEl !== e || this._liveEntity !== a || this._livePronto === a) && this._falharAoVivo("prazo");
    }, Mt);
  }
  _falharAoVivo(e) {
    const a = this._liveEntity;
    if (!a) return;
    const i = a.split(".")[1] ?? a;
    B(
      `marco: ${i} · player ${this._modoPlayer} · ${e}`,
      performance.now() - this._liveIniciadoEm,
      !1
    ), this._estadoAoVivo = "fallback", this._fallbackAoVivo = a, this._pararAoVivo(), this._sincronizarCameras();
  }
  _pararAoVivo() {
    $(x, this._timerAoVivo), this._timerAoVivo = void 0, $(x, this._timerQuadroVerde), this._timerQuadroVerde = void 0;
    const e = this._liveEl;
    e && (ce(x, e, "load", this._aoCarregarAoVivo), ce(x, e, "streams", this._aoInformarStreams), e.remove()), this._liveEl = void 0, this._liveEntity = "", this._livePronto = "", this._quadroVerdeRegistrado = "", this._modoPlayer = "nenhum";
  }
  /**
  ...
  _marcarQuadroNaTela(e) {
    ...
    const a = e.split(".")[1] ?? e;
    B(`câmera ${a} · até aparecer`, performance.now() - this._montadoEm, !0);
  }
  /**
  ...
  _quadroPronto(e, a) {
    this._urlsCarregadas[e] = a;
    const i = this.shadowRoot?.querySelector(
      `img[data-camera-entity="${e}"]`
    );
    i && (i.src = a, i.classList.add("is-loaded"), i.closest(".camera-main")?.classList.add("has-loaded-image"));
  }
  /**
  ...
  _injetarMaterial(e = 0) {
    ...
    if (!a || this._materialInjetado || !this.isConnected) return;
    const t = globalThis.BrunoSurfaceMaterial?.subviewStyles?.();
    if (!t) {
      e < 20 && I(x, () => this._injetarMaterial(e + 1), 60);
      return;
    }
    try {
      const o = new CSSStyleSheet();
      o.replaceSync(t), a.adoptedStyleSheets = [...a.adoptedStyleSheets, o], this._materialInjetado = !0;
    } catch {
      const o = document.createElement("style");
      o.textContent = t, a.appendChild(o), this._materialInjetado = !0;
    }
  }
  /**
  ...
  _aplicarAtributos() {
    ...
    const a = this._sub?.entities, i = (t, o) => {
      o ? this.setAttribute(t, "") : this.removeAttribute(t);
    };
    i("data-appliances", !!(a?.appliances ?? a?.dishwasher)), i("data-tvhub", !!a?.tv), this._folha ? this.setAttribute("data-folha", this._folha) : this.removeAttribute("data-folha"), i("data-ps5", !!a?.ps5);
  }
   ...
   */
  _renderTopBand() {
    const e = this._room?.entities, a = this._hass, i = (c) => c && a ? a.states[c] : void 0, t = this._contarLuzes(), o = i(e?.motionRecent)?.state === "on", n = [
      {
  ...
  _contarLuzes() {
    const e = this._hass, a = this._room?.entities;
    return !e || !a?.lights ? 0 : a.lights.filter((i) => e.states[i]?.state === "on").length;
  }
  /**
  ...
  _linhaLuzes() {
    ...
    for (const i of e) {
      const t = i.zone || "sala", o = this._hass?.states[i.entity]?.state === "on" ? 1 : 0;
      a.set(t, (a.get(t) ?? 0) + o);
    }
    return [...a.entries()].map(([i, t]) => `${i.charAt(0).toUpperCase()}${i.slice(1)} ${t}`).join(" · ");
  }
  _linhaPresenca() {
    const e = this._hass, a = this._room?.entities;
    if (!e || !a?.semanticState) return "Sensor indisponível";
    const t = e.states[a.semanticState]?.attributes.display;
    return t ? String(t) : e.states[a.motionRecent ?? ""]?.state === "on" ? "Presença" : "Sem presença";
  }
  /**
   ...
   */
  _valorSensor(e, a, i = 0) {
    const t = e && this._hass ? this._hass.states[e] : void 0, o = String(t?.state ?? "").toLowerCase();
    return !t || ["unknown", "unavailable", "none", ""].includes(o) ? "--" : `${V(t.state, i)}${a}`;
  }
  /** Roteador e hub Zigbee: "Online" quando conectado, senão o próprio estado. */
  _linhaRede(e) {
    if (!e) return "Online";
    const a = String(this._hass?.states[e]?.state ?? "Online");
    return ["on", "home", "connected", "online"].includes(a.toLowerCase()) ? "Online" : a;
  }
  _hora() {
   ...
   */
  _data() {
    const e = ["DOMINGO", "SEGUNDA-FEIRA", "TERÇA-FEIRA", "QUARTA-FEIRA", "QUINTA-FEIRA", "SEXTA-FEIRA", "SÁBADO"], a = ["JAN", "FEV", "MAR", "ABR", "MAI", "JUN", "JUL", "AGO", "SET", "OUT", "NOV", "DEZ"], i = /* @__PURE__ */ new Date();
    return `${e[i.getDay()]}, ${i.getDate()} ${a[i.getMonth()]}`;
  }
  /**
   ...
   */
  _alternarDock() {
    this._lightsOpen = !this._lightsOpen, this._luzesAssentadas = !1, $(x, this._timerLuzes), this._timerLuzes = I(x, () => {
  ...
  _abrirFolha(e) {
    this._folha = this._folha === e ? null : e, this._folha === "luzes" && (this._lightsOpen = !0, this._luzesAssentadas = !0), this._aplicarAtributos(), this._avisarFolha(), this.requestUpdate();
  }
  _fecharFolha() {
    this._folha && (this._folha = null, this._aplicarAtributos(), this._avisarFolha(), this.requestUpdate());
  }
  /**
  ...
  _linhasResumo() {
    ...
    return this._temEletrodomesticos ? e.push({
      chave: "eletro",
  ...
  _resumoClimate() {
    const e = this._modeloClimate();
    if (e.indisponivel) return "Indisponível";
    if (!e.ativo) return "Desligado";
    const a = this._rotuloModo(String(e.modo));
    return e.alvo == null ? a : `${a} · ${V(e.alvo)}°`;
  }
  /**
  ...
  _resumoMidia() {
    ...
    const e = this._modeloTv();
    if (e.ativo) return `Ligada · ${e.fonte}`;
    const a = this._modeloSpotify();
    return a.ativo ? a.titulo : "Nada tocando";
  }
  /**
  ...
  _resumoEletrodomesticos() {
    ...
    if (!Array.isArray(e) || !e.length) return "Sem aparelhos";
    const a = e.filter((t) => !t.placeholder && t.entity);
    return a.length ? `${a.filter((t) => {
      const o = this._hass?.states[String(t.stateEntity ?? t.entity)]?.state, n = Array.isArray(t.activeStates) ? t.activeStates : ["on"];
      return o != null && n.includes(String(o));
    }).length} de ${a.length} ligados` : `${e.length} sem tomada`;
  }
  /**
  ...
  _renderResumoTelefone() {
    ...
    return l`
      ...
      (a) => l`
            <button
              type="button"
              class="resumo-linha ${this._folha === a.chave ? "is-active" : ""}"
              aria-expanded=${this._folha === a.chave ? "true" : "false"}
              @click=${() => this._abrirFolha(a.chave)}
            >
              <span class="micro-icon ${a.tom}"><bruno-icon icon=${a.icone}></bruno-icon></span>
              <span class="resumo-texto">
                <span class="resumo-titulo">${a.titulo}</span>
                <span class="resumo-estado">${a.resumo}</span>
              </span>
  ...
  _renderSecoesDeLuz() {
    ...
    const a = this._sub?.lightZoneLabels ?? {}, i = this._sub?.lightZoneIcons ?? {}, t = { sala: "Sala", varanda: "Varanda" }, o = { sala: "mdi:sofa-outline", varanda: "bruno:balcony" }, n = [];
    ...
    const c = n.map((d) => {
      ...
      return {
        ...
        nome: a[d] ?? t[d] ?? d.charAt(0).toUpperCase() + d.slice(1),
  ...
  _luzesDaConfiguracao() {
    ...
    return Array.isArray(e) ? e.filter((a) => !!a && typeof a == "object").filter((a) => typeof a.entity == "string" && !a.placeholder).map((a) => ({
      entity: String(a.entity),
      name: String(a.name ?? "Luz"),
      zone: String(a.zone ?? "sala"),
      icon: typeof a.iconType == "string" ? a.iconType : void 0
    })) : [];
  }
  _renderCelulaDeLuz(e, a, i) {
    const t = this._hass?.states[e.entity]?.state === "on", o = i && a === 0, n = i ? a - 1 : a, c = o ? 0 : Math.floor(n / 2) + (i ? 1 : 0), s = [
      "light-cell",
      t ? "is-on" : "",
      o ? "is-wide" : "",
      !o && c > 0 ? "has-rule-top" : "",
    ...
    return l`
      ...
      >
        <span class="lc-icon">${this._iconeDaLuz(e.icon, t)}</span>
   ...
   */
  _iconeDaLuz(e, a) {
    const t = String(e ?? "light_flush").replace(/^mdi:/, "").replace(/[^a-z0-9_-]/gi, "") || "light_flush";
    return l`<span class="tpl-light-icon icon-${t} ${a ? "is-on" : ""}">
  ...
  _apagarZona(e) {
    ...
    const a = e.map((i) => i.entity);
    this._hass.callService("light", "turn_off", { entity_id: a }, { entity_id: a });
  }
  _todasAsLuzes(e) {
    const a = this._room?.entities.lightGroup;
    !a || !this._hass || this._hass.callService("light", e, { entity_id: a }, { entity_id: a });
  }
  render() {
    return this._room ? l`
  ...
  _estadoCortina() {
    ...
    return a ? a.state === "open" ? "Aberta" : a.state === "closed" ? "Fechada" : "Indisponível" : "Indisponível";
  }
  _percentualCortina() {
  ...
  _posicionarCortina(e) {
    ...
    !a || !this._hass || this._hass.callService("cover", "set_cover_position", { entity_id: a, position: e }, { entity_id: a });
  }
  _estado(e) {
  ...
  _servico(e, a, i) {
    this._hass && this._hass.callService(e, a, i, i);
  }
  /**
  ...
  _camerasConfiguradas() {
    ...
    return Array.isArray(e) ? e.filter((a) => !!a && typeof a.entity == "string") : [];
  }
  /**
   ...
   */
  _cameraViva(e) {
    const a = this._estado(e.entity), i = this._indisponivel(a), t = !i && zt.includes(String(a?.state ?? "")), o = String(a?.attributes.entity_picture ?? "");
  ...
  _controleCamera(e, a) {
    const i = (e?.controls ?? []).find((n) => String(n.key ?? "").toLowerCase() === a);
    if (!i?.entity) return;
    const t = this._estado(i.entity), o = this._indisponivel(t);
    return {
      ...i,
      entity: i.entity,
      ativo: !o && String(t?.state ?? "").toLowerCase() === "on",
      indisponivel: o
    };
  }
  /**
   ...
   */
  _renderFeed(e, a) {
    const i = e?.shortName || e?.name || "Câmera", o = !!this._controleCamera(e, "privacy")?.ativo, n = !e || e.indisponivel, c = [
      "camera-main",
      "camera-feed",
      a ? "camera-pip-feed" : "camera-primary-feed",
      o ? "is-private" : "",
      n ? "is-unavailable" : ""
        ...
        </div>` : o ? l`<div class="camera-state-surface">
            <bruno-icon icon="mdi:eye-off-outline"></bruno-icon><span>Modo privacidade ativo</span>
          </div>` : u, d = !!(e && !a && Se(e.entity)), m = l`
      <div class="camera-row-image">
        ${d ? l`<div class="camera-live-slot" data-camera-live=${e.entity}></div>` : u}
        ${e ? l`<img
              src=${e.url}
  ...
  _renderCameras() {
    const e = this._camerasConfiguradas().map((c) => this._cameraViva(c));
    if (!e.length)
      return l`
      ...
      `;
    const a = e.find((c) => c.entity === this._cameraAtiva) ?? e[0], i = e.find((c) => c.online), t = a?.online || !i ? a : i, o = e.find((c) => c.entity !== t?.entity), n = this._controlesCameraAbertos;
    return l`
      <div class="glass-card cameras-card cameras-card-controls">
        <div class="mh-head cameras-head">
          <div class="mh-head-title">
      ...
      this._controlesCameraAbertos = !this._controlesCameraAbertos, this.requestUpdate();
        ...
        <div class="camera-stage camera-pip-stage ${n ? "is-controls-open" : ""}">
          ${this._renderFeed(t, !1)}
          ${o ? this._renderFeed(o, !0) : u}
          ${n ? this._renderControlesCamera(t) : u}
        </div>
      </div>
    `;
  }
  _renderControlesCamera(e) {
    const a = ["sound", "motion", "privacy"].map((t) => this._controleCamera(e, t)).filter((t) => !!t);
    ...
    return l`
      <div class="camera-control-strip" aria-label=${`Controles da câmera ${i}`}>
        <div class="camera-controls">
          ${a.map((t) => {
      const o = t.description || t.label || "Controle";
      return l`
              <button
                type="button"
                class="camera-control ${t.ativo ? "is-on" : ""} ${t.indisponivel ? "is-unavailable" : ""}"
                ?disabled=${t.indisponivel}
                aria-pressed=${t.ativo ? "true" : "false"}
                title=${`${o} — câmera ${i}`}
                @click=${() => this._servico("homeassistant", "toggle", { entity_id: t.entity })}
              >
                <bruno-icon icon=${t.icon ?? "mdi:toggle-switch-outline"}></bruno-icon>
                <span class="camera-control-label">${t.label || o}</span>
  ...
  _resolverId(e) {
    ...
    const a = e.filter((t) => typeof t == "string" && !!t);
    return a.find((t) => !this._indisponivel(this._hass?.states[t])) ?? a[0];
  }
  _idDe(e) {
    return this._resolverId(this._sub?.entities?.[e]);
  }
  _modeloTv() {
    const e = this._estado(this._idDe("tv")), a = e?.attributes ?? {}, i = e?.state ?? "off", t = kt.includes(i), o = String(a.source ?? a.app_name ?? "") || "HDMI 1", n = String(a.media_title ?? a.media_series_title ?? a.app_name ?? "");
    return {
      st: e,
      estado: i,
      ativo: t,
  ...
  _modeloSpotify() {
    const e = this._estado(this._idDe("spotify")), a = e?.attributes ?? {}, i = e?.state ?? "off", o = $t.includes(i) && $a(
      e,
      this._sub?.spotifyDeviceName,
      this._estado(this._idDe("speaker"))
    ), n = String(a.media_title ?? "") || "SpotifyPlus", c = Number(a.media_duration) || 0, s = Number(a.media_position) || 0, d = Date.parse(String(a.media_position_updated_at ?? "")), m = o && i === "playing", p = m && Number.isFinite(d) ? s + (Date.now() - d) / 1e3 : s, h = c > 0 ? Math.max(0, Math.min(c, p)) : Math.max(0, p);
    return {
      st: e,
      ativo: o,
      tocando: m,
      titulo: o ? /^SpotifyPlus\s+Bruno/i.test(n) ? "SpotifyPlus" : n : "SpotifyPlus",
      artista: o ? String(a.media_artist ?? a.media_album_name ?? "") : "",
      capa: o ? String(a.entity_picture ?? a.media_image_url ?? "") : "",
      volume: a.volume_level != null ? Math.round(Number(a.volume_level) * 100) : null,
      dispositivo: this._sub?.spotifyDeviceName || String(a.source ?? "") || "SpotifyPlus",
  ...
  _modeloPc() {
    const e = this._estado(this._idDe("pcActive"))?.state === "on", a = this._estado(this._idDe("pcSession"))?.state ?? "", i = this._estado(this._idDe("pcWindow"))?.state ?? "";
    return { ativo: e, sessao: a, janela: i };
  }
  /**
   ...
   */
  _fonteAberta(e, a) {
    const i = e.filter((o) => a[o]), t = this._midiaAtivasAntes;
    return this._midiaAtivasAntes = i, this._temPc ? (a.spotify && !t.includes("spotify") && (this._fonteMidia = "spotify"), !a.pc && this._fonteMidia === "pc" && a.spotify && (this._fonteMidia = "spotify"), e.includes(this._fonteMidia) ? this._fonteMidia : a.spotify ? "spotify" : "pc") : (i.some((o) => !t.includes(o)) && (this._fonteMidia = ""), e.includes(this._fonteMidia) ? this._fonteMidia : i[0] ?? e[0] ?? "");
  }
  ...
  _linhaVolume(e, a) {
    return l`
      <div class=${e ? "mh-vol" : "mh-vol is-disabled"}>
        ...
        <span class="mh-vol-label">Volume ${a}%</span>
        <input
          type="range"
          min="0"
          max="100"
      ...
      e && this._servico("media_player", "volume_set", {
        entity_id: e,
        volume_level: Number(t.value) / 100
      });
    }}
        />
      </div>
  ...
  _botaoMidia(e, a, i, t = {}) {
    ...
    return l`
      <button
        ...
        title=${e}
        aria-label=${e}
        ?disabled=${t.desabilitado}
        @click=${i}
      >
        <bruno-icon icon=${a}></bruno-icon>${o ? u : l`<span>${e}</span>`}
      </button>
    `;
  }
  /**
  ...
  _arteMidia(e, a, i, t) {
    return l`
      <div class="mh-art mh-art-${a} ${t ? "is-cover" : "is-standby"}">
        ${e ? l`<img src=${e} alt="" loading="lazy" />` : l`<bruno-icon icon=${i}></bruno-icon>`}
      </div>
    `;
  }
  _corpoTv() {
    const e = this._modeloTv(), a = this._idDe("tv"), i = this._sub?.tvStandbyImage ?? Dt, o = (!(!e.titulo || /^TV (ligada|desligada)$/i.test(e.titulo) || e.titulo === e.fonte) && e.estado === "playing" ? e.titulo : "") || e.fonte;
    if (!e.ativo)
      return l`
        <div class="mh-left">
        ...
        "Ligar TV",
        "mdi:power",
        () => this._servico("homeassistant", "toggle", { entity_id: a }),
        { principal: !0, desabilitado: !a }
      )}
          </div>
        </div>
    ...
    const n = Array.isArray(this._sub?.tvApps) ? this._sub.tvApps : [], c = this._appsTvAbertos && n.length ? l`<div class="mh-btn-row mh-btn-row-5">
      ...
      this._appsTvAbertos = !1, this.requestUpdate();
          ...
          ${this._botaoMidia("Pausar", "mdi:pause", () => this._servico("media_player", "media_play_pause", { entity_id: a }), { soIcone: !0 })}
    ...
    return l`
      <div class="mh-left">
        <div class="mh-info">
          <small>Ligada</small>${o ? l`<em>${o}</em>` : u}
        </div>
        <div class="mh-controls">${this._linhaVolume(a, e.volume ?? 60)} ${c}</div>
      </div>
              ...
              content: {
                ...
                custom_actions: [
                  ...
                  i("menu", "mdi:menu", "button.tv_sala_menu"),
                  {
                    ...
                    tap_action: a("button.tv_sala_ok"),
                    up: { icon: "mdi:chevron-up", tap_action: a("button.tv_sala_navigate_up"), hold_action: { action: "repeat" } },
                    down: { icon: "mdi:chevron-down", tap_action: a("button.tv_sala_navigate_down"), hold_action: { action: "repeat" } },
                    left: { icon: "mdi:chevron-left", tap_action: a("button.tv_sala_navigate_left"), hold_action: { action: "repeat" } },
                    right: { icon: "mdi:chevron-right", tap_action: a("button.tv_sala_navigate_right"), hold_action: { action: "repeat" } }
                  },
  ...
  _corpoPc() {
    const e = this._modeloPc(), a = this._sub?.pcImage ?? Ot, i = e.ativo ? [e.sessao, e.janela].filter((o) => o && o !== "--")[0] || "Sessão ativa" : "Pronto para ligar", t = (o) => () => {
    ...
    return l`
      <div class="mh-left">
        ...
        <div class="mh-controls">
          ${e.ativo ? l`<div class="mh-btn-row mh-btn-row-5 office-pc-actions">
                ${this._botaoMidia("Sleep", "mdi:weather-night", t("pcSleep"), {
      soIcone: !0,
      desabilitado: !this._idDe("pcSleep")
    })}
                ${this._botaoMidia("Reiniciar", "mdi:restart", t("pcRestart"), {
      soIcone: !0,
      desabilitado: !this._idDe("pcRestart")
    })}
                ${this._botaoMidia("Desligar", "mdi:power-standby", t("pcShutdown"), {
      soIcone: !0,
      desabilitado: !this._idDe("pcShutdown")
    })}
                ${this._botaoMidia("Bloquear", "mdi:lock-outline", t("pcLock"), {
      soIcone: !0,
      ...
      { soIcone: !0 }
                ...
                ${this._botaoMidia("Ligar PC", "mdi:power", t("pcPower"), {
      principal: !0,
      ...
      ${this._arteMidia(a, "wide", "mdi:desktop-tower", !1)}
    `;
  }
  _corpoSpotify() {
    const e = this._modeloSpotify(), a = this._idDe("spotify"), i = this._sub?.spotifyStandbyImage ?? It;
    if (!e.ativo)
      return l`
        <div class="mh-left">
      ...
      `;
    const t = this._spotifyFerramentas ? l`<div class="mh-btn-row mh-btn-row-4">
      ...
      this._spotifyFerramentas = !1, this.requestUpdate();
          ...
          ${this._botaoMidia("Anterior", "mdi:skip-previous", () => this._servico("media_player", "media_previous_track", { entity_id: a }), { soIcone: !0 })}
          ${this._botaoMidia(e.tocando ? "Pausar" : "Tocar", e.tocando ? "mdi:pause" : "mdi:play", () => {
      e.tocando ? this._servico("media_player", "media_pause", { entity_id: a }) : this._tocarSpotify();
    }, { soIcone: !0 })}
          ${this._botaoMidia("Próxima", "mdi:skip-next", () => this._servico("media_player", "media_next_track", { entity_id: a }), { soIcone: !0 })}
    ...
    return l`
      <div class="mh-left">
        <div class="mh-info">
          <small>${e.titulo}</small>${e.artista ? l`<em>${e.artista}</em>` : u}
        ...
        <div class="mh-controls">${this._linhaVolume(a, e.volume ?? 66)} ${t}</div>
      </div>
  ...
  _tocarSpotify() {
    ...
    if (!e) return;
    const a = this._sub?.spotifyDeviceName || String(this._estado(e)?.attributes.source ?? "");
    if (a) {
  ...
  _maisInfo(e) {
    e && (Se(e) && (this._tokenDefinicaoPlayer++, this._estadoAoVivo = "entregue-more-info", N(e, "entregue ao more-info"), this._pararAoVivo(), this._sincronizarCameras()), this.dispatchEvent(
   ...
   */
  _renderMediaHub() {
    const e = this._temPc, a = e ? void 0 : this._modeloTv(), i = e ? this._modeloPc() : void 0, t = this._modeloSpotify(), n = [
      e ? {
        chave: "pc",
        rotulo: "PC",
        ...
        ativo: !!a?.ativo,
        resumo: a?.ativo ? `Ligada · ${a.fonte}` : "Desligada",
        corpo: () => this._corpoTv()
      },
      {
        chave: "spotify",
        ...
        icone: "mdi:spotify",
        ativo: t.ativo,
        resumo: t.ativo ? t.titulo : "Nenhuma faixa",
  ...
  _renderMenuMidia() {
    const e = this._idDe("ps5"), i = this._estado(e)?.state === "on", t = e ? [{ icone: "mdi:sony-playstation", titulo: "PS5", sub: i ? "Online" : "Offline", entidade: e, ativo: i }] : [];
    return t.length ? l`
  ...
  _renderEletrodomesticos() {
    const e = this._sub?.entities?.appliances;
    return Array.isArray(e) ? e.filter((a) => !!a && typeof a == "object").map((a) => {
      const i = String(a.key ?? "item").replace(/[^a-z0-9_-]/gi, "-").toLowerCase(), t = String(a.name ?? "Eletrodoméstico"), o = typeof a.image == "string" ? a.image : "", n = typeof a.entity == "string" ? a.entity : "", c = typeof a.stateEntity == "string" ? a.stateEntity : n, s = c && this._hass ? this._hass.states[c] : void 0, d = Array.isArray(a.activeStates) ? a.activeStates.map((A) => String(A).toLowerCase()) : ["on"], m = typeof a.activeAttr == "string" ? a.activeAttr : "", p = this._room?.activeSensor ? this._hass?.states[this._room.activeSensor] : void 0, h = d.includes(String(s?.state ?? "").toLowerCase()) || (m ? qt(p?.attributes[m]) : !1), b = !!a.placeholder || !n, f = typeof a.moreInfoEntity == "string" ? a.moreInfoEntity : n, v = ["appliance-tile", `is-${i}`, h ? "is-on" : "", b ? "is-muted" : ""].filter(Boolean).join(" ");
      return l`
          <article class=${v}>
            <button
              type="button"
            ...
            >
              <div class="appliance-visual" data-image-wrapper>
                ${o ? l`<img src=${o} alt="" loading="lazy" decoding="async" />` : u}
              </div>
              <div class="appliance-copy">
                <strong>${t}</strong>
                <small>${this._rotuloDoAparelho(a, s, h, b)}</small>
              </div>
            </button>
            <button
              type="button"
  ...
  _rotuloDoAparelho(e, a, i, t) {
    const o = (c, s) => typeof e[c] == "string" ? e[c] : s;
    if (t) return o("placeholderLabel", "Sem tomada");
    if (!a) return "Indisponível";
    if (i) return o("activeLabel", "Ligada");
    const n = String(a.state).toLowerCase();
    return n === "off" || n === "unavailable" ? o("offLabel", "Desligada") : o("idleLabel", "Ligada");
  }
  _alternarAparelho(e) {
    if (!this._hass) return;
    const a = e.split(".")[0] ?? "switch";
    this._hass.callService(a, "toggle", { entity_id: e }, { entity_id: e });
  }
  _entidadeClimate() {
   ...
   */
  _modeloClimate() {
    const e = this._estadoClimate(), a = e?.attributes ?? {}, i = String(a.hvac_action ?? "").toLowerCase(), t = this._indisponivel(e), o = t || e?.state === "off" ? !1 : St.includes(i) ? !0 : Ct.includes(i) ? !1 : At.includes(String(e?.state ?? "")), n = (c, s) => Number.isFinite(Number(c)) ? Number(c) : s;
    return {
      st: e,
      indisponivel: t,
      ativo: o,
      alvo: n(a.temperature, null),
      atual: n(a.current_temperature, null),
      minima: n(a.min_temp, 16),
      maxima: n(a.max_temp, 30),
      modo: e?.state ?? "off",
      ventilacao: String(a.fan_mode ?? "auto"),
      swing: String(a.swing_mode ?? ""),
      modos: Array.isArray(a.hvac_modes) ? a.hvac_modes : [],
      ventilacoes: Array.isArray(a.fan_modes) ? a.fan_modes : [],
      swings: Array.isArray(a.swing_modes) ? a.swing_modes : []
    };
  }
  _rotuloModo(e) {
    return {
  ...
  _rotuloVentilacao(e) {
    ...
    return a === "auto" ? "Auto" : a.includes("low") || a.includes("baixo") ? "Baixa" : a.includes("med") ? "Média" : a.includes("high") || a.includes("alto") ? "Alta" : a.includes("fort") ? "Forte" : Ce(e);
  }
  _iconeVentilacao(e) {
    const a = String(e).toLowerCase();
    return a.includes("auto") ? "mdi:fan-auto" : a.includes("low") || a.includes("baixo") ? "mdi:fan-speed-1" : a.includes("med") ? "mdi:fan-speed-2" : a.includes("high") || a.includes("alto") || a.includes("fort") ? "mdi:fan-speed-3" : "mdi:fan";
  }
  _rotuloSwing(e) {
    const a = String(e).toLowerCase();
    return a ? ["off", "desativado", "desativada", "disabled"].includes(a) ? "Desligado" : ["on", "ativo", "ativada", "enabled"].includes(a) ? "Ativo" : Ce(e) : "Indisponível";
  }
  /**
  ...
  _renderAnelClimate(e) {
    const s = Number.isFinite(e.minima) ? e.minima : 12, d = Number.isFinite(e.maxima) ? e.maxima : 30, m = Number.isFinite(Number(e.alvo)) ? Math.max(s, Math.min(d, Number(e.alvo))) : s + (d - s) / 2, h = -180 + 180 * Math.max(0, Math.min(1, (m - s) / Math.max(1, d - s))), b = (w, q) => {
      const k = q * Math.PI / 180;
      return { x: 360 + w * Math.cos(k), y: 410 + w * Math.sin(k) };
    }, f = (w, q, k) => {
      const E = b(w, q), M = b(w, k), re = Math.abs(k - q) <= 180 ? "0" : "1";
      return `M ${E.x.toFixed(3)} ${E.y.toFixed(3)} A ${w} ${w} 0 ${re} 1 ${M.x.toFixed(3)} ${M.y.toFixed(3)}`;
    }, v = Array.from({ length: 91 }, (w, q) => {
      const k = -180 + 180 * (q / 90), E = q % 15 === 0, M = q % 5 === 0, re = b(319, k), Ue = b(E ? 293 : M ? 299 : 306, k), Pa = E ? "icg-tick major" : M ? "icg-tick medium" : "icg-tick minor";
      return ye`<line x1=${re.x.toFixed(3)} y1=${re.y.toFixed(3)} x2=${Ue.x.toFixed(3)} y2=${Ue.y.toFixed(3)} class=${Pa}></line>`;
    }), A = Array.from({ length: 73 }, (w, q) => {
      const k = -180 + 180 * (q / 72), E = b(267, k), M = b(251, k);
      return ye`<line x1=${E.x.toFixed(3)} y1=${E.y.toFixed(3)} x2=${M.x.toFixed(3)} y2=${M.y.toFixed(3)} class="icg-inner-tick"></line>`;
    }), g = [
      ...
      const q = b(w.r, w.ang);
      return ye`<text x=${q.x.toFixed(3)} y=${q.y.toFixed(3)} text-anchor="middle" dominant-baseline="middle" class=${`icg-label ${w.cls}`}>${w.texto}</text>`;
    }), S = b(285, h), Q = e.alvo == null ? "--" : V(e.alvo, 0), X = e.atual == null ? "--" : V(e.atual, 1), oe = (e.modo === "cool" ? "Resfriamento" : e.modo === "heat" ? "Aquecimento" : e.modo === "fan_only" ? "Ventilacao" : "Temperatura").toUpperCase();
    return l`
      <div class="icg-root">
        <div class="icg-shell">
          <svg
            ...
            viewBox="0 0 720 460"
            role="img"
            aria-label=${`Temperatura alvo ${Q}°. Ambiente ${X}°.`}
          >
            <defs>
  ...
  _renderAC() {
    const e = this._entidadeClimate(), a = this._modeloClimate(), i = a.swing.toLowerCase(), t = ["on", "ativo", "ativada", "enabled"].includes(i) || i.includes("ativ") && !i.includes("desativ"), o = (m) => [...new Set(m.filter(Boolean))], n = this._painelClima, c = {
      mode: o(a.modos).map((m) => ({
        modo: m,
        rotulo: this._rotuloModo(m),
        icone: this._iconeModo(m),
        ativo: m.toLowerCase() === String(a.modo).toLowerCase(),
        servico: "set_hvac_mode",
        campo: "hvac_mode"
      })),
      fan: o(a.ventilacoes).map((m) => ({
        modo: m,
        rotulo: this._rotuloVentilacao(m),
        icone: this._iconeVentilacao(m),
        ativo: m.toLowerCase() === a.ventilacao.toLowerCase(),
        servico: "set_fan_mode",
        campo: "fan_mode"
      })),
      swing: o(a.swings).map((m) => ({
        modo: m,
    ...
    return l`
      ...
      this._painelClima = "", this._maisInfo(e);
            ...
            <button
              type="button"
              class="ac-power-floating ${a.ativo ? "is-active" : ""}"
              aria-label=${a.ativo ? "Desligar ar condicionado" : "Ligar ar condicionado"}
              ?disabled=${a.indisponivel || !e}
              @click=${() => {
      e && (this._painelClima = "", this._servico("climate", a.ativo ? "turn_off" : "turn_on", { entity_id: e }));
    }}
            >
              <bruno-icon icon="mdi:power"></bruno-icon>
            </button>
        ...
        <div class="ac-lean-mid">
          <div class="ac-ring">${this._renderAnelClimate(a)}</div>
        </div>
      ...
      "mdi:thermostat-auto",
      "Modo",
      !a.ativo || a.modo === "off" ? "Desligado" : this._rotuloModo(a.modo)
    )}
          ${d("fan", "mdi:fan", "Ventilação", this._rotuloVentilacao(a.ventilacao))}
          ${d(
      "swing",
      "mdi:air-conditioner",
      "Swing",
      a.swing ? this._rotuloSwing(a.swing) : t ? "Ativo" : "Desligado"
    )}
        </div>
      </div>
    `;
...
customElements.get("bruno-room-subview") || customElements.define("bruno-room-subview", Pt);
const ge = window;
ge.customCards = ge.customCards ?? [];
ge.customCards.some((r) => r.type === "bruno-room-subview") || ge.customCards.push({
...
function Rt(r) {
  ...
  for (const [i, t] of r.entries()) {
    const o = t.id || `posição ${i}`;
    t.id ? a.has(t.id) ? e.push(`id repetido — "${t.id}"`) : a.add(t.id) : e.push(`dispositivo em ${o}: falta "id"`), t.type ? z.conhece(t.type) || e.push(`dispositivo "${o}": tipo não registrado — "${t.type}"`) : e.push(`dispositivo "${o}": falta "type"`), t.name || e.push(`dispositivo "${o}": falta "name"`);
    const n = z.obter(t.type)?.validate?.(t);
...
function Ia(r, e) {
  ...
  const a = r.filter((t) => typeof t == "string" && !!t);
  return a.find((t) => {
    const o = e?.states[t];
...
function jt(r) {
  const e = /* @__PURE__ */ new Map();
  for (const a of r) {
    const i = a.group ?? "Casa", t = e.get(i);
    t ? t.push(a) : e.set(i, [a]);
  }
  return [...e.entries()].map(([a, i]) => ({ grupo: a, itens: i }));
}
const ma = [
  {
    id: "sala-tv",
...
], Vt = ["cool", "heat", "fan_only", "dry", "heat_cool", "auto"], Bt = ["on", "playing", "paused", "idle", "buffering"];
class be extends C {
  static {
  ...
  get _entityId() {
    return Ia(this._instancia?.entity, this._hass);
  }
  _estado(e) {
    return e && this._hass ? this._hass.states[e] : void 0;
  }
  _servico(e, a, i) {
    this._hass?.callService(e, a, i, i);
  }
  static {
...
class Ft extends be {
  static {
    this.styles = [be.estilosComuns];
  }
  render() {
    const e = this._entityId, a = this._estado(e);
    if (!a) return l`<p class="indisponivel">Entidade indisponível: ${e ?? "—"}</p>`;
    const i = Bt.includes(String(a.state)), t = String(a.attributes.source ?? a.attributes.app_name ?? "") || "HDMI 1", o = a.attributes.volume_level != null ? Math.round(Number(a.attributes.volume_level) * 100) : null;
    return l`
      <div class="titulo">
        <strong>${this._instancia?.name ?? "TV"}</strong>
        <small>${i ? `Ligada · ${t}` : "Desligada"}</small>
      </div>
...
class Ut extends be {
  ...
  render() {
    const e = this._entityId, a = this._estado(e);
    if (!a) return l`<p class="indisponivel">Entidade indisponível: ${e ?? "—"}</p>`;
    const i = Vt.includes(String(a.state)), t = Number(a.attributes.temperature), o = Number(a.attributes.current_temperature), n = Number.isFinite(Number(a.attributes.min_temp)) ? Number(a.attributes.min_temp) : 16, c = Number.isFinite(Number(a.attributes.max_temp)) ? Number(a.attributes.max_temp) : 30, s = Number(a.attributes.target_temp_step) || 1, d = Array.isArray(a.attributes.hvac_modes) ? a.attributes.hvac_modes : [], m = (p) => this._servico("climate", "set_temperature", {
      entity_id: e,
    ...
    return l`
      <div class="titulo">
        ...
        <small>${i ? `${this._rotulo(a.state)} · ambiente ${Number.isFinite(o) ? o : "—"}°` : "Desligado"}</small>
      </div>
      ...
      <div class="linha">
        <button ?disabled=${!i} @click=${() => m((Number.isFinite(t) ? t : 22) - s)}>
          <bruno-icon icon="mdi:minus"></bruno-icon>
        </button>
        <span class="valor">${Number.isFinite(t) ? `${t}°` : "—"}</span>
        <button ?disabled=${!i} @click=${() => m((Number.isFinite(t) ? t : 22) + s)}>
...
customElements.get("bruno-control-tv") || customElements.define("bruno-control-tv", Ft);
customElements.get("bruno-control-climate") || customElements.define("bruno-control-climate", Ut);
function Oa(r) {
  return (e) => {
...
z.conhece("climate") || z.registrar({
  ...
  icon: "mdi:air-conditioner",
  create: Oa("bruno-control-climate"),
  entities: (r) => [typeof r.entity == "string" ? r.entity : r.entity?.[0] ?? ""].filter(Boolean),
  validate: (r) => ({
    ok: !!r.entity,
...
const Ee = "bruno-devices-panel", Gt = ["on", "playing", "paused", "idle", "buffering", "cool", "heat", "fan_only", "dry", "heat_cool", "auto"];
class Ht extends C {
  constructor() {
    super(...arguments), this._selecionado = "", this._controles = /* @__PURE__ */ new Map(), this._observador = new Ve(
      ma.flatMap((e) => pa(e))
    ), this._motivo = "";
  }
  static {
  ...
  _estaAtivo(e) {
    const a = Ia(e.entity, this._hass), i = a && this._hass ? this._hass.states[a] : void 0;
    return !!i && Gt.includes(String(i?.state ?? "").toLowerCase());
  }
  /**
  ...
  _controleDe(e) {
    ...
    if (!i) {
      const t = Nt(a);
      if (!t) return u;
      i = t, this._controles.set(e, i);
    }
    return this._hass && (i.hass = this._hass), i;
  }
  connectedCallback() {
    super.connectedCallback(), Te(Ee);
  }
  disconnectedCallback() {
    super.disconnectedCallback(), De(Ee);
  }
  /** Mede o custo de cada atualizacao (Fase 6.0.1). */
  update(e) {
    const a = this._motivo;
    this._motivo = "", Ie(Ee, () => super.update(e), a || "outro");
  }
  render() {
    const e = Rt(this._dispositivos), a = this._idAberto;
    return l`
      <div class="painel" role="dialog" aria-modal="true" aria-label="Dispositivos">
        ...
        ${e.ok ? u : l`<p class="aviso">
              Configuração de dispositivos com problema:
              ${e.erros.map((i) => l`<span>${i}</span>`)}
            </p>`}

        <div class="corpo">
          <nav class="lista" aria-label="Lista de dispositivos">
            ${jt(this._dispositivos).map(
      (i) => l`
                <div class="grupo">
                  <h3>${i.grupo}</h3>
                  ${i.itens.map((t) => {
        const o = this._estaAtivo(t), n = t.id === a;
        return l`<button
                      type="button"
                      class="item ${n ? "is-selected" : ""} ${o ? "is-active" : ""}"
                      aria-pressed=${n ? "true" : "false"}
                      @click=${() => {
          this._selecionado = t.id, this.requestUpdate();
        }}
                    >
                      <span class="item-icone">
                        <bruno-icon icon=${t.icon ?? z.obter(t.type)?.icon ?? "mdi:remote"}></bruno-icon>
                      </span>
                      <span class="item-nome">${t.name}</span>
  ...
  entidadesObservadas() {
    return [...new Set(this._dispositivos.flatMap((e) => pa(e)))];
  }
  static {
    this.styles = _`
...
customElements.get("bruno-devices-panel") || customElements.define("bruno-devices-panel", Ht);
Ja();
console.info("[bruno-dashboard] build 20260811");
globalThis.BrunoCameraEngine = Da;
globalThis.BrunoCameraLive = at;
//# sourceMappingURL=bruno-dashboard.JpxYam5N.js.map
