...
const BRUNO_SALA_HYBRID_TIMINGS = Object.freeze({
  ...
  tv: Object.freeze({ on: 920, off: 840, fade: 320 }),
...
function HybridTvIcon({ active, transition = '', elapsed = 0, size = 42, glowPhase = 0, key = 'tv' }) {
  ...
  const base = `/local/bruno-ui/assets/hybrid-icons/tv`;
  const version = BRUNO_SALA_HYBRID_ASSET_VERSION;

  return `
    <span class="${rootClass}" data-hybrid-key="${key}" style="${style}">
      <span class="tvHybrid__canvas">
        <img class="tvHybrid__layer tvHybrid__screenGlow" src="${base}/tv-glow.png?v=${version}" alt="">
        <img class="tvHybrid__layer tvHybrid__screenOn" src="${base}/tv-screen-on.png?v=${version}" alt="">
        <span class="tvHybrid__screenClip">
          <span class="tvHybrid__screenWash"></span>
          <span class="tvHybrid__oledLine"></span>
        </span>
        <img class="tvHybrid__layer tvHybrid__frameOff" src="${base}/tv-frame-off.png?v=${version}" alt="">
        <img class="tvHybrid__layer tvHybrid__frameOn" src="${base}/tv-frame-on.png?v=${version}" alt="">
        <!-- DASHBOARD EDITION V3: estrutura fisica permanente. Bezel, haste e
             base nao participam da troca emissiva OFF/ON, portanto a TV nao
             perde sua silhueta de produto quando a tela acende. -->
        <span class="tvHybrid__bezel"></span>
        <span class="tvHybrid__neck"></span>
        <span class="tvHybrid__foot"></span>
      </span>
    </span>
  `;
}
...
class BrunoSalaCard extends HTMLElement {
  ...
  _hybridIcon(key, iconName, view, now) {
    const props = {
      active: view.active,
      transition: view.transition,
      elapsed: view.elapsed,
      // ANTERIOR (rollback): size: 42,
      // Dashboard Edition: ganho optico sem alterar a caixa ou a linha do card.
      size: 46,
      glowPhase: view.glowPhase,
      airflowPhase: view.airflowPhase,
      key,
    };

    if (iconName === 'tv') return HybridTvIcon(props);
    if (iconName === 'climate') return HybridAcIcon(props);
    return HybridLedStripIcon(props);
  }
  ...
  _actionButton(key, iconName, name, label, active, tone, options = {}) {
    ...
    return `
      <button class="command-row icon-${iconName} tone-${tone}${activeClass}" type="button" data-action-key="${key}" aria-label="${BrunoSalaCard._escape(ariaLabel)}" aria-pressed="${active ? 'true' : 'false'}">
        <span class="command-icon is-hybrid" aria-hidden="true">${hybridIcon}</span>
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        * {
        ...
        .command-row.icon-ledstrip.is-active .command-state,
        ...
        .command-row.icon-ledstrip { --tone: 255, 204, 125; }
        ...
        /* NOVO (2026-07-09) — Fase 2 mobile: SALA COMPACTA no phone.
      ...
      <div class="sala-card${roomActiveClass}${statusStackClass}">
        ...
        -->
        <div class="action-strip">
          ${this._actionButton('corridor', 'ledstrip', 'Corredor', model.corridorStateLabel, model.corridorOn, 'blue', {
