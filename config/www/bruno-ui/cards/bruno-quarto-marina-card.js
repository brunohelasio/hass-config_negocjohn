...
const BRUNO_QUARTO_MARINA_DEFAULT_CONFIG = {
  ...
  "status_dots": [
    ...
    },
    {
      ...
      "spotify_device": "Echo Pop Marina",
      "states": [
        "playing",
        "paused",
        "on"
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="room-card${roomActiveClass}${this._tileClasses()}" style="--room-icon-size:${iconSize}px;">
        ...
        <button class="room-action${hasMetricClass}" type="button" data-action-key="room" aria-label="${BrunoQuartoMarinaCard._escape(this._config.name)}">
          ...
          <span class="room-nav-zone" data-room-nav role="button" tabindex="0" aria-label="Abrir ${BrunoQuartoMarinaCard._escape(this._config.name)}">
            <span class="room-title-row">
              <span class="title">${BrunoQuartoMarinaCard._escape(this._config.name)}</span>
