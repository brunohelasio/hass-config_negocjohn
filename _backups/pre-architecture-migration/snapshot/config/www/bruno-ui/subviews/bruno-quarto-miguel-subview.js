...
const BRUNO_QUARTO_MIGUEL_SUBVIEW_DEFAULT_CONFIG = {
  ...
  entities: {
    ...
    lights: [
      ...
      { entity: 'light.quarto_miguel_2_switch_2', name: 'Arandela poltrona', icon_type: 'sconce', zone: 'sala' },
      { entity: 'light.quarto_miguel_2_switch_3', name: 'Arandela berco', icon_type: 'sconce', zone: 'sala' },
      ...
      { entity: 'light.suite_miguel_switch_1', name: 'Luz suite', icon_type: 'light_flush', zone: 'varanda' },
...
if (action === 'toggle-lights-panel') {
    ...
    return `
      <section class="light-section">
        <div class="section-head">
          ...
          ${showZoneAction
            ? `<button type="button" class="zone-off" data-action="zone-off" data-zone="${zoneKey}">Apagar ${BrunoQuartoMiguelSubview._escape(zone.name.toLowerCase())}</button>`
            : ''}
        </div>
  ...
  _renderLightCell(light, index, isOdd) {
    ...
    return `
      <button type="button" class="${cls}"
        ...
        <span class="lc-icon">${BrunoQuartoMiguelSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <span class="lc-name">${BrunoQuartoMiguelSubview._escape(light.name)}</span>
    ...
    const controlMarkup = controls.map((control) => {
      ...
      return `
        <button
          ...
          title="${BrunoQuartoMiguelSubview._escapeAttr(ariaLabel)}"
        >
          <bruno-icon icon="${BrunoQuartoMiguelSubview._escapeAttr(control.icon || 'mdi:toggle-switch-outline')}"></bruno-icon>
  ...
  _renderMediaHub(model) {
    ...
    const ps5 = model.ps5;
    // NOVO (2026-07-22) — consolidacao mobile: marca o modulo sem fonte
    // configurada; estados unavailable de fontes configuradas continuam visiveis.
    const mediaConfigured = Boolean(
      this._config.entities.tv
      || this._config.entities.spotify
      || this._config.entities.speaker
      || this._config.entities.ps5
    );
    ...
    return `
      <!-- ANTERIOR (rollback): media-hub-card sem o marcador is-unconfigured. -->
      <section class="glass-card media-hub-card mh-accordion${sourceMeta[selected]?.active ? ' is-playing' : ''}${this._mediaMenuOpen ? ' is-menu-open' : ''}${mediaConfigured ? '' : ' is-unconfigured'}">
        <div class="mh-head">
          <div class="mh-head-title">
  ...
  _renderMediaHubLegacy(model) {
    ...
    const mediaActionButton = ({
      ...
      <button
        ...
        ${disabled ? 'disabled' : ''}
      >
        <bruno-icon icon="${BrunoQuartoMiguelSubview._escapeAttr(icon)}"></bruno-icon>
      </button>
    `;
    ...
    const standbyImage = (src, className, fallbackIcon) => (
      src
        ...
        : `<bruno-icon icon="${BrunoQuartoMiguelSubview._escapeAttr(fallbackIcon)}"></bruno-icon>`
    );
      ...
      return `
        ...
        >
          ${image
            ...
            : `<bruno-icon icon="${BrunoQuartoMiguelSubview._escapeAttr(app.icon || 'mdi:apps')}"></bruno-icon>`}
        </button>
      `;
    }).join('');
        ...
        <div class="ps5-body ps5-minimal">
          ...
          <div class="ps5-footer">
            ...
            <div class="ps5-actions">
              <button type="button" class="primary-button" data-action="toggle-ps5" ${ps5.configured ? '' : 'disabled'}>${ps5.active ? 'Desligar' : 'Ligar'}</button>
              <button type="button" class="control-button" data-action="more-info" data-entity="${BrunoQuartoMiguelSubview._escapeAttr(ps5.entityId || '')}" ${ps5.configured ? '' : 'disabled'}><bruno-icon icon="mdi:dots-horizontal"></bruno-icon></button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="glass-card spotify-card${spotify.active ? ' is-active' : ''}">
        ...
        <div class="spotify-body">
          <div class="spotify-art${artwork ? ' has-art' : ''}">
            ${artwork ? `<img src="${BrunoQuartoMiguelSubview._escapeAttr(artwork)}" alt="">` : '<bruno-icon icon="mdi:music-note"></bruno-icon>'}
          </div>
