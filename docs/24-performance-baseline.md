...
## Baseline 12 — expansão ONVIF: publicação correta, prazo incorreto (2026-08-10)

### Resposta objetiva sobre pasta local e VM

A expansão para as oito câmeras ONVIF foi copiada corretamente para a pasta
local e para a VM. Antes da nova correção, os arquivos ativos do build
`bruno-dashboard.ZlG6UJRv.js`, `configuration.yaml`, card dinâmico da Home,
subview de câmeras, helper e automação tinham o mesmo SHA-256 nos dois lados.
Portanto, o defeito observado não era uma cópia parcial ou divergência entre o
repositório e `/config`: era comportamento do código efetivamente carregado.

O diagnóstico capturado em `2026-08-10T11:19:25.344Z` também identifica
explicitamente o build `bruno-dashboard.ZlG6UJRv.js`. Ele é uma medição válida
da revisão com prazo de 10 s, mas não valida a correção de 30 s publicada depois.

### Evidência que fecha a causa

| tentativa direta | total | falhas | pior duração |
|---|---:|---:|---:|
| `as_camera_profile_1 · player webrtc · prazo` | 1 | 1 | 10.000,9 ms |
| `of_camera_profile_1 · player webrtc · prazo` | 3 | 3 | 10.008,7 ms |
| `sl_camera_profile_1 · player webrtc · prazo` | 1 | 1 | 10.001,2 ms |

As cinco tentativas medidas foram encerradas pelo nosso próprio prazo exatamente
em torno de 10 s. Ao expirar, o componente destruía o `ha-web-rtc-player`,
marcava a entidade como temporariamente indisponível para o modo ao vivo e
voltava ao motor de instantâneos. É daí que vem a cadência percebida de cerca de
10 s nas tiles: não era vídeo em tempo real lento; era o fallback de snapshots.

O `more-info` funciona porque pertence ao Home Assistant e não é encerrado por
esse prazo customizado. A travada inicial relatada é a negociação/inicialização
do fluxo; depois dela, o player do HA permanece em tempo real.

Os instantâneos estavam respondendo em outra escala: primeiro quadro entre
1,29 s e 3,07 s nas amostras válidas, e `até aparecer` entre 1,45 s e 6,31 s.
Isso confirma que a imagem que continuava mudando era o fallback, não o player.
A Sala teve 6 falhas em 11 pedidos de snapshot durante a disputa com o WebRTC,
o que merece nova observação, mas não muda a causa fechada dos cinco abortos.

Também não há indício de gargalo geral no componente: `bruno-room-subview`
teve pior render de 5,9 ms, 10 instâncias criadas e 10 encerradas, com 117 timers
e 28 listeners igualmente criados e encerrados. O diagnóstico registrou zero
vazamentos de timers, listeners ou assinaturas.

### Correção aplicada nas três superfícies

O prazo de negociação passou de **10 s para 30 s** de forma uniforme em:

1. subviews dos cômodos, no `bruno-room-subview` compilado;
2. subview geral de câmeras;
3. card dinâmico de câmeras da Home.

Durante a negociação, o último snapshot permanece visível. Quando o player
produz vídeo, o motor deixa de atualizar a câmera principal; se ainda assim não
houver vídeo em 30 s, o componente libera o player e preserva o fallback. O
limite continua existindo para evitar sessões WebRTC órfãs e consumo indefinido.
O fluxo continua limitado a uma câmera ao vivo por superfície.

Não foi reintroduzido fallback HLS customizado. Ao abrir `more-info`, o player
embutido é liberado primeiro para que o modal do HA seja o único dono do fluxo.

### Artefato publicado e cache

Novo build publicado localmente e na VM:

```text
/local/dashboard/bruno-dashboard.Cvth7Jgi.js
```

Também foram publicados o source map, `manifest.json`, loader, card da Home,
subview de câmeras e `configuration.yaml`. Os SHA-256 conferidos após a cópia
foram idênticos nos dois lados; entre eles:

| arquivo | SHA-256 |
|---|---|
| `bruno-dashboard.Cvth7Jgi.js` | `E8588D118E4F6909198F67218EB57D359BF897811509898AE00A9A9EE4E94AA8` |
| `bruno-home-camera-card.js` | `B8CC2AD7EFFEF6C0D7A174E3B773892C253F1A13A9E5BD3C770CD42F05CA1F21` |
| `bruno-cameras-security-subview.js` | `D17AE9CCD23092724595052E2649E3C2BD7899D5B017C3DF75B5A6FA39560F09` |
| `configuration.yaml` | `34AC01F7766D3BCC58EAA2D33A04F3C0DC444C68E1A226BC15C5F681C086626B` |

As URLs dos dois arquivos clássicos receberam cache-bust
`20260810-webrtc-timeout-30s-1`. A URL anterior e os valores anteriores foram
mantidos comentados in-place para rollback. O bundle `ZlG6UJRv` também continua
na VM, e a cópia de segurança local desta etapa está em
`tmp/rollback-20260810-camera-webrtc-timeout/`.

### Validação executada

- TypeScript: `tsc --noEmit` aprovado;
- ESLint: aprovado;
- Vitest: 11 arquivos e 204 testes aprovados;
- sintaxe: 54 arquivos JavaScript aprovados;
- YAML: 247 arquivos sem erro sintático;
- comentários em template literals: 103 arquivos, nenhuma crase perigosa;
- bundle final: `node --check` aprovado;
- hashes local/VM: idênticos em todos os artefatos publicados.

### Estado e critério de encerramento

A correção está **implementada e publicada**, mas a eficácia em runtime só pode
ser declarada depois que o frontend carregar `bruno-dashboard.Cvth7Jgi.js`.
Como `frontend.extra_module_url` fica em `configuration.yaml`, é necessário
reiniciar o Home Assistant e fazer recarga forçada/limpeza de cache no cliente.

O próximo diagnóstico precisa mostrar `build: bruno-dashboard.Cvth7Jgi.js` e
ser coletado após abrir uma subview por mais de 30 s. Aceitação:

- ausência de `player webrtc · prazo` em tentativas que consigam iniciar;
- vídeo realmente contínuo na tile/subview, e não trocas discretas de snapshot;
- nenhum crescimento persistente de falhas de snapshot nas câmeras secundárias;
- timers/listeners continuam balanceados ao entrar e sair das subviews.

Se o build `Cvth7Jgi` ainda terminar todas as tentativas aos 30 s, aumentar de
novo o prazo não será tratado como solução: a etapa seguinte será instrumentar
os eventos internos do `ha-web-rtc-player` e isolar a negociação por entidade.

---

## Baseline 13 — prazo maior confirma player inerte por contexto Lit (2026-08-10)

### O que o build `Cvth7Jgi` provou

Captura em `2026-08-10T11:40:55.614Z`, depois de 315 s de execução:

| medição | total | falhas | pior |
|---|---:|---:|---:|
| `of_camera_profile_1 · player webrtc · prazo` | 1 | 1 | 30.000,6 ms |
| snapshots `of_camera_profile_1` | 9 | 0 | 2.995,1 ms |
| primeiro snapshot do Office | 1 | 0 | 1.436,0 ms |
| Office `até aparecer` | 2 | 0 | 39.861,6 ms |

O aumento de 10 s para 30 s mudou apenas o instante do mesmo desfecho. Não
houve `player webrtc · primeiro quadro`, nem `streams` declarando ausência de
vídeo. O player ficou inerte até o relógio externo removê-lo. Isso elimina a
hipótese de negociação apenas lenta e impede novo aumento de prazo como solução.

O restante continuou saudável: três instâncias de `bruno-room-subview` criadas
e três encerradas, pior render de 4,3 ms, zero vazamento de timers, listeners ou
assinaturas e memória estabilizada em aproximadamente 110 MB.

### Causa no contrato do componente oficial

O código oficial de `ha-web-rtc-player` consome `apiContext` e
`connectionContext` por contexto Lit. Quando `entityid` muda, `willUpdate()`
chama `_startWebRtc()`, mas essa função retorna imediatamente enquanto qualquer
um dos contextos ainda não estiver disponível. A chegada posterior dos
contextos não dispara nova tentativa porque o gatilho observa somente a mudança
de `entityid`.

Nos três componentes Bruno a sequência era:

1. criar o player;
2. anexá-lo ao DOM;
3. atribuir `entityid` imediatamente na linha seguinte;
4. iniciar o prazo.

Conectar antes de atribuir era necessário, mas não suficiente. O primeiro ciclo
Lit ainda não havia terminado e os contextos podiam continuar vazios. O estado
medido — sem `load`, sem `streams`, sem erro e apenas timeout externo — é a
assinatura exata desse retorno antecipado.

Referência do contrato oficial:
`https://github.com/home-assistant/frontend/blob/dev/src/components/ha-web-rtc-player.ts`.

### Correção

A sequência passou a ser uniforme nas subviews de cômodos, subview geral de
câmeras e card dinâmico da Home:

1. criar e anexar `ha-web-rtc-player`;
2. aguardar `updateComplete` do primeiro ciclo Lit;
3. confirmar que elemento, entidade e conexão ainda são os mesmos;
4. atribuir `entityid` somente então;
5. iniciar o relógio de proteção de 30 s a partir da negociação real.

O evento oficial `load`, emitido a partir de `loadeddata`, e a confirmação
`video.readyState >= 2` continuam sendo o critério para promover o vídeo. Não
foi trocado um critério correto por um atalho visual.
...
```text
/local/dashboard/bruno-dashboard.tdYjE8QO.js
```

Cache-busts dos dois componentes clássicos:

```text
20260810-webrtc-lit-context-1
```

Bundle, source map, manifesto, loader, card da Home, subview de câmeras e
`configuration.yaml` foram copiados para a VM e conferidos por SHA-256. Todos
os sete pares local/VM são idênticos. O build `Cvth7Jgi` permanece na VM para
rollback e as URLs anteriores ficaram comentadas em `configuration.yaml`.

### Validação

- TypeScript aprovado;
- ESLint aprovado;
- 11 arquivos e 204 testes Vitest aprovados;
- 54/54 arquivos JavaScript com sintaxe válida;
- 247 arquivos YAML sem erro;
- 103 arquivos sem crase perigosa em template literal;
- bundle `tdYjE8QO` aprovado por `node --check`;
- sete hashes local/VM idênticos após publicação.

### Quadro verde do Office

Na captura visual, o tile do Office preserva a faixa superior e o relógio da
câmera, mas o restante do quadro fica verde. Ao mesmo tempo, a instrumentação
conta nove de nove snapshots como carregados. Portanto, não é placeholder CSS
nem falha HTTP detectável pelo `Image.onerror`: é uma imagem que o navegador
aceitou, porém com conteúdo/decodificação corrompido. Esse defeito é separado do
player WebRTC inerte. Se persistir após o fluxo ao vivo assumir, deve receber
instrumentação de conteúdo própria; não será mascarado aceitando qualquer cor
como sucesso.

### Critério da próxima captura

Depois de reinício do HA e recarga forçada, o diagnóstico precisa identificar
`bruno-dashboard.tdYjE8QO.js`. O resultado esperado é um marcador
`player webrtc · primeiro quadro` antes do limite. Se ainda houver apenas
`prazo`, a próxima inspeção será do erro renderizado dentro do player e dos
estados de ICE, sem ampliar novamente o tempo.
