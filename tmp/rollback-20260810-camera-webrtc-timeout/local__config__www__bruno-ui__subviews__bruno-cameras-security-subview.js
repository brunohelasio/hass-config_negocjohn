...
class BrunoCamerasSecuritySubview extends HTMLElement {
  ...
  constructor() {
    ...
    this._liveSuspendedEntities = new Set();
  }
  ...
  _handleClick(event) {
    ...
    if (action === 'more-info') {
      ...
      this._liveSuspendedEntities.add(activeId);
      this._stopLiveFeed();
  ...
  _mountLiveFeed(activeId) {
    ...
    if (!this.isConnected || !mount || !activeId || camera?.unavailable || this._liveSuspendedEntities.has(activeId)) {
      this._stopLiveFeed();
      return;
    }
    ...
    if (!this._liveEl || this._liveEntity !== activeId) {
      ...
      if (!globalThis.customElements?.get('ha-web-rtc-player')) {
        // Sem o player final, nao reabrir hui-image/HLS: a foto permanece.
        this._liveSuspendedEntities.add(activeId);
        return;
      }
      ...
      this._liveStreamsHandler = (event) => {
        if (event?.detail?.hasVideo === false) this._failLiveFeed(activeId);
      };
      ...
      // Mesma ordem aprovada no Office: conecta primeiro, entityid depois.
      mount.appendChild(el);
      el.entityid = activeId;
      this._liveTimer = globalThis.setTimeout(() => {
        if (this._liveEl === el && this._liveEntity === activeId && this._liveReady !== activeId) {
          this._failLiveFeed(activeId);
        }
      }, 10000);
      return;
    }
