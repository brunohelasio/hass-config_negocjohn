...
const BRUNO_COZINHA_CURTAIN_CALIBRATION = [
  { visual: 0, position: 0 },
...
const BRUNO_COZINHA_CURTAIN_PRESETS = [
...
const BRUNO_COZINHA_CURTAIN_TRAVEL_MS = 30000;
const BRUNO_COZINHA_CURTAIN_MIN_MOTION_MS = 1200;
const BRUNO_COZINHA_CURTAIN_MOTION_TICK_MS = 350;
const BRUNO_COZINHA_CURTAIN_TARGET_TOLERANCE = 2;
...
const BRUNO_COZINHA_SUBVIEW_DEFAULT_CONFIG = {
  ...
  fallback_background: '/local/images/cozinha.jpg?v=20260702-all-images-1',
  ...
  entities: {
    ...
    // ANTERIOR (rollback): semantic_sensor: 'sensor.cozinha_active',
    // ANTERIOR (rollback): occupancy: 'binary_sensor.sensor_4_in_1_cozinha_presence',
    ...
    dishwasher_power: 'switch.cz_tomada_maq_lav_louca_socket_1',
    lights: [
      { entity: 'light.cozinha_switch_2', name: 'Luz principal 1', icon_type: 'ledstrip', zone: 'cozinha' },
      { entity: 'light.cozinha_switch_3', name: 'Luz principal 2', icon_type: 'ledstrip', zone: 'cozinha' },
      { entity: 'light.cozinha_switch_1', name: 'Lavanderia', icon_type: 'light_flush', zone: 'cozinha' },
    ],
    appliances: [
      {
        key: 'dishwasher',
        ...
        state_entity: 'sensor.lava_loucas_operation_state',
        more_info_entity: 'switch.cz_tomada_maq_lav_louca_socket_1',
        image: '/local/images/lava_louca.png?v=20260702-all-images-1',
        active_states: ['on', 'run'],
        active_attr: 'dishwasher_running',
        active_label: 'Lavando',
        on_label: 'Ligada',
        idle_label: 'Ligada',
        off_label: 'Desligada',
      },
      { key: 'airfryer', name: 'Air fryer', entity: '', image: '/local/images/air_fry.png?v=20260702-all-images-1', placeholder: true, placeholder_label: 'Sem tomada' },
      { key: 'fridge', name: 'Geladeira', entity: '', image: '/local/images/geladeira.png?v=20260702-all-images-1', placeholder: true, placeholder_label: 'Sem tomada' },
      { key: 'microwave', name: 'Micro-ondas', entity: '', image: '/local/images/microondas.png?v=20260702-all-images-1', placeholder: true, placeholder_label: 'Sem tomada' },
      { key: 'washer', name: 'Lavadora', entity: '', image: '/local/images/lava_roupa.png?v=20260702-all-images-1', placeholder: true, placeholder_label: 'Wi-Fi pendente' },
    ],
    cameras: [
      {
        ...
        short_name: 'Cozinha',
        controls: [
          { key: 'sound', label: 'Som', description: 'Detecção de som', icon: 'mdi:microphone-outline', entity: 'switch.cz_camera_deteccao_de_som' },
          { key: 'motion', label: 'Mov.', description: 'Alarme de movimento', icon: 'mdi:run-fast', entity: 'switch.cz_camera_alarme_de_movimento' },
          { key: 'privacy', label: 'Priv.', description: 'Modo de privacidade', icon: 'mdi:eye-off-outline', entity: 'switch.cz_camera_modo_de_privacidade' },
        ],
      },
      {
...
const BRUNO_COZINHA_SUBVIEW_CLIMATE_ON_STATES = ['cool', 'heat', 'fan_only', 'dry', 'heat_cool', 'auto'];
const BRUNO_COZINHA_SUBVIEW_CLIMATE_ACTIVE_ACTIONS = ['cooling', 'heating', 'drying', 'fan', 'preheating'];
const BRUNO_COZINHA_SUBVIEW_CLIMATE_INACTIVE_ACTIONS = ['off', 'idle'];
const BRUNO_COZINHA_SUBVIEW_TV_ON_STATES = ['on', 'playing', 'paused', 'idle'];
const BRUNO_COZINHA_SUBVIEW_MEDIA_ON_STATES = ['playing', 'paused', 'on', 'idle'];
const BRUNO_COZINHA_SUBVIEW_CAMERA_ONLINE_STATES = ['streaming', 'recording', 'idle', 'on'];
const BRUNO_COZINHA_SUBVIEW_UNAVAILABLE_STATES = ['unavailable', 'unknown', '', 'none', 'null'];
const BRUNO_COZINHA_SUBVIEW_TV_ICON_ANIMATION_MS = 1000;
...
class BrunoCozinhaSubview extends HTMLElement {
  ...
  _normalizeConfig(config) {
    ...
    return {
      ...
      ...config,
      refresh_interval: Math.max(4000, Number(config.refresh_interval) || BRUNO_COZINHA_SUBVIEW_DEFAULT_CONFIG.refresh_interval),
      room_nav: Array.isArray(config.room_nav) ? config.room_nav : BRUNO_COZINHA_SUBVIEW_DEFAULT_CONFIG.room_nav,
      tv_apps: Array.isArray(config.tv_apps) ? config.tv_apps : BRUNO_COZINHA_SUBVIEW_DEFAULT_CONFIG.tv_apps,
      entities,
    };
  }
  ...
  _renderError(error) {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="error">Erro na subview Cozinha: ${BrunoCozinhaSubview._escape(error?.message || error)}</div>
    `;
  }
  ...
  _refreshCameraImages() {
    ...
    this.shadowRoot.querySelectorAll('img[data-camera-src-base]').forEach((image) => {
      ...
      const nextSrc = BrunoCozinhaSubview._withCacheBust(baseSrc, stamp);
  ...
  _isUnavailable(entity) {
    return !entity || BRUNO_COZINHA_SUBVIEW_UNAVAILABLE_STATES.includes(String(entity.state || '').toLowerCase());
  }
  ...
  _interpolateCurtainPercent(value, fromKey, toKey) {
    ...
    const points = BRUNO_COZINHA_CURTAIN_CALIBRATION;
  ...
  _curtainMotionDuration(startClosed, targetClosed) {
    ...
    return Math.max(BRUNO_COZINHA_CURTAIN_MIN_MOTION_MS, BRUNO_COZINHA_CURTAIN_TRAVEL_MS * (distance / 100));
  }
  ...
  _curtainMotionClosedPosition(motion = this._curtainMotion, now = Date.now()) {
    ...
    const duration = Math.max(1, Number(motion.duration) || BRUNO_COZINHA_CURTAIN_MIN_MOTION_MS);
  ...
  _curtainMotionDisplayPosition(entityId, state, reportedClosed) {
    ...
    const reportedAtTarget = reported != null && Math.abs(reported - motion.targetClosed) <= BRUNO_COZINHA_CURTAIN_TARGET_TOLERANCE;
  ...
  _startCurtainMotionTimer() {
    ...
    this._curtainMotionTimer = globalThis.setInterval(() => {
      ...
      this._safeRender();
    }, BRUNO_COZINHA_CURTAIN_MOTION_TICK_MS);
  }
  ...
  _truthy(value) {
    return value === true || value === 'true' || value === 'True' || value === 'on' || value === 'yes' || value === '1' || value === 1;
  }

  _applianceModel(item = {}) {
    const entityId = item.entity || '';
    const stateEntityId = item.state_entity || entityId;
    const moreInfoEntity = item.more_info_entity || entityId || stateEntityId;
    const entity = this._state(entityId);
    const stateEntity = this._state(stateEntityId);
    const configured = Boolean(entityId) && !item.placeholder;
    const rawState = String((stateEntity || entity)?.state || '').toLowerCase();
    const controlState = String(entity?.state || '').toLowerCase();
    const activeStates = (Array.isArray(item.active_states) ? item.active_states : (item.active_states ? [item.active_states] : ['on'])).map((state) => String(state).toLowerCase());
    const activeSensor = this._state(this._config.entities.active_sensor);
    const attrActive = item.active_attr ? this._truthy(activeSensor?.attributes?.[item.active_attr]) : false;
    const unavailable = configured && this._isUnavailable(entity) && (!stateEntity || this._isUnavailable(stateEntity));
    const active = configured && !unavailable && (
      activeStates.includes(rawState) ||
      activeStates.includes(controlState) ||
      attrActive
    );
    const switchedOn = configured && !unavailable && controlState === 'on';

    let label = item.placeholder_label || '';
    if (configured) {
      if (active && item.active_label) label = item.active_label;
      else if (switchedOn) label = item.on_label || item.idle_label || 'Ligada';
      else label = item.off_label || 'Desligada';
    }
    ...
    return {
      ...
      active,
      label,
      icon: item.icon || 'mdi:power-plug-outline',
    };
  }
  ...
  _curtainModel(options = {}) {
    ...
    let status = 'Fechada';
    if (!configured || unavailable) status = 'Indisponível';
  ...
  _cameraState(camera) {
    ...
    const online = !unavailable && BRUNO_COZINHA_SUBVIEW_CAMERA_ONLINE_STATES.includes(state);
    ...
    return {
      ...
      online,
      image,
      imageUrl: this._loadedCameraUrls[camera.entity] || BrunoCozinhaSubview._withCacheBust(image, this._refreshSeed),
      status: online ? 'Ao vivo' : (unavailable ? 'Indisponível' : 'Online'),
    };
  }
  ...
  _spotifyModel() {
    ...
    const globalActive = BRUNO_COZINHA_SUBVIEW_MEDIA_ON_STATES.includes(state);
...
_mediaSourceFromAction(action) {
  ...
  _climateIsActive(entity) {
    ...
    if (BRUNO_COZINHA_SUBVIEW_CLIMATE_ACTIVE_ACTIONS.includes(hvacAction)) return true;
    if (BRUNO_COZINHA_SUBVIEW_CLIMATE_INACTIVE_ACTIONS.includes(hvacAction)) return false;
    return BRUNO_COZINHA_SUBVIEW_CLIMATE_ON_STATES.includes(entity.state || '');
  }
  ...
  _sceneContextLabel(model) {
    ...
    if (model.lights > 0) return 'Cozinha iluminado';
    if (semantic) return semantic;
    return active?.state === 'yes' ? 'Cozinha ativo' : 'Disponivel';
  }
...
if (action === 'toggle-lights-panel') {
    ...
    if (action === 'toggle-appliance') {
      if (entityId && !this._cooldown(`appliance-${entityId}`, 700)) {
  ...
  // NOVO (2ª passada): rótulo "ligado há" — pega a luz ACESA mais antiga da lista
  ...
  _renderHero() {
    return `
      <div class="hero-stage hero-atmosphere">
        <div class="hero-content" aria-hidden="true"></div>
      </div>
    `;
  }

  // Controles de cortina (mantidos fora da renderizacao da Cozinha).
  ...
  _renderCurtain(model) {
    ...
    const curtainMarks = BRUNO_COZINHA_CURTAIN_PRESETS.map((preset) => `
      <button
        type="button"
      ...
      <div class="curtain-dock curtain-overlay${curtain.available ? '' : ' is-disabled'}" style="--curtain-position: ${curtainVisualPosition}%;">
          <div class="curtain-control-row">
            <div class="curtain-identity">
              <span class="curtain-icon-shell">${BrunoCozinhaSubview._curtainSvg('main')}</span>
              <span class="curtain-title">Cortina</span>
            </div>
            <div class="curtain-status" aria-live="polite">
              <span class="curtain-status-text">${BrunoCozinhaSubview._escape(curtainStatusLabel)}</span>
  ...
  _renderTopBand(model) {
    const badges = [
      ...
      { icon: 'mdi:lightbulb', title: 'Luzes', sub: `Cozinha ${model.lights || 0}`, tone: '247,198,0', active: (model.lights || 0) > 0 },
    ...
    return `
      <header class="subview-topband">
        <div class="topband-badges">
          ${badges.map((badge) => `
            <div class="tb-badge${badge.active ? ' is-active' : ''}"${badge.phoneHide ? ' data-phone-hide' : ''} style="--tone: ${badge.tone};">
              <span class="tb-badge-icon"><bruno-icon icon="${badge.icon}"></bruno-icon></span>
              <span class="tb-badge-text">
                <span class="tb-badge-title">${BrunoCozinhaSubview._escape(badge.title)}</span>
                <span class="tb-badge-sub">${BrunoCozinhaSubview._escape(badge.sub)}</span>
              </span>
            </div>
          `).join('')}
        </div>
        <div class="topband-clock" aria-label="Data e hora">
          ...
          <small>${BrunoCozinhaSubview._escape(this._dateLine())}</small>
        </div>
      </header>
    `;
  }
  ...
  _renderFrameTop() {
    return `
      <header class="subview-topbar">
        <span class="subview-room">${BrunoCozinhaSubview._escape(this._config.title)}</span>
    ...
    return `
      <footer class="subview-footer">
        <span class="subview-presence">
          ...
          ${BrunoCozinhaSubview._escape(this._presenceLine())}
        </span>
      </footer>
    `;
  }
  ...
  _renderRoomSidebar() {
    ...
    const homeButton = `
      <button
        ...
        data-path="${BrunoCozinhaSubview._escapeAttr(this._config.navigation_path)}"
        title="Home"
        aria-label="Home"
      >
        ${BrunoCozinhaSubview._roomNavIcon('home')}
      ...
      <nav class="room-sidebar" aria-label="Navegacao de comodos">
        ...
        ${items.map((item) => `
          ...
          >
            ...
            <span class="room-nav-label">${BrunoCozinhaSubview._escape(item.name)}</span>
          </button>
        `).join('')}
      </nav>
    `;
  ...
  _renderLightZoneRail(lights, selectedZone) {
    ...
    return `
      <aside
        ...
        data-zone="${BrunoCozinhaSubview._escapeAttr(selectedZone)}"
  ...
  _renderLightSection(zone, showZoneAction) {
    const zoneKey = BrunoCozinhaSubview._escapeAttr(zone.key);
    ...
    return `
      <section class="light-section">
        <div class="section-head">
          ...
          <span class="zone-id">
            <strong>${BrunoCozinhaSubview._escape(zone.name)}</strong>
            <small>${zone.onCount}/${zone.total} acesas${onLabel ? ` · ${onLabel}` : ''}</small>
          </span>
          ${showZoneAction
            ? `<button type="button" class="zone-off" data-action="zone-off" data-zone="${zoneKey}">Apagar ${BrunoCozinhaSubview._escape(zone.name.toLowerCase())}</button>`
            : ''}
        </div>
  ...
  _renderLightCell(light, index, isOdd) {
    ...
    return `
      <button type="button" class="${cls}"
        ...
        aria-label="${BrunoCozinhaSubview._escapeAttr(light.name)}">
        <span class="lc-icon">${BrunoCozinhaSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <span class="lc-name">${BrunoCozinhaSubview._escape(light.name)}</span>
  ...
  _renderLights(model) {
    ...
    const isOddLights = visibleLights.length % 2 === 1;
    // NOVO (2ª passada): contagem + tempo ligado para o mini-cabeçalho.
    const onCount = visibleLights.filter((l) => this._state(l.entity)?.state === 'on').length;
    const onLabel = this._zoneOnLabel(visibleLights);

    // ANTERIOR (rollback 2026-07-29, rev.9): card em fluxo com cabecalho
    // proprio e grade de tiles. NOVO: dock colapsavel. Sem secoes aqui — a
    // faixa de titulo do dock ja concentra "Apagar todas".
    // ANTERIOR (rollback rev.9): uma linha de status solta em vez de secao.
    // REV.10: mesma composicao das demais subviews — UMA secao isolada, com
    // icone, titulo e contagem. Sem a acao "Apagar", que aqui seria redundante
    // com "Apagar todas" da faixa de titulo.
    return this._renderLightsDock(this._renderLightSection({
      key: 'principal',
      name: 'Cozinha',
      icon: 'mdi:lightbulb-group',
      lights: visibleLights,
      onCount,
      total: visibleLights.length,
    }, false));
  }
    ...
    return `
      <section class="light-zone${expanded ? ' is-expanded' : ''}">
        <div class="zone-header" ${onlyOne ? '' : `role="button" data-action="toggle-zone" data-zone="${zoneKey}"`}>
          ...
          ${expanded ? `<span class="zone-off" role="button" data-action="zone-off" data-zone="${zoneKey}">Apagar ${BrunoCozinhaSubview._escape(zone.name.toLowerCase())}</span>` : ''}
  ...
  _renderLightRow(light) {
    const active = this._state(light.entity)?.state === 'on';
    return `
      <button type="button" class="light-row${active ? ' is-on' : ''}" data-action="toggle-light" data-entity="${BrunoCozinhaSubview._escapeAttr(light.entity)}">
        <span class="light-row-icon">${BrunoCozinhaSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <span class="light-row-name">${BrunoCozinhaSubview._escape(light.name)}</span>
  ...
  _renderZoneTile(light, wide) {
    ...
    return `
      <button type="button"
        ...
        role="switch" aria-checked="${active ? 'true' : 'false'}"
        aria-label="${BrunoCozinhaSubview._escapeAttr(light.name)}">
        <span class="zl-icon">${BrunoCozinhaSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <span class="zl-switch" aria-hidden="true"><span class="zl-knob"></span></span>
        <span class="zl-name">${BrunoCozinhaSubview._escape(light.name)}</span>
      </button>
    `;
  ...
  _renderCameraFeed(camera, options = {}) {
    ...
    const content = `
      ...
      <div class="camera-row-copy">
        <strong>${BrunoCozinhaSubview._escape(cameraName)}</strong>
      </div>
    `;
    ...
    if (pip && camera?.entity) {
      return `
        <button
          ...
          data-entity="${BrunoCozinhaSubview._escapeAttr(camera.entity)}"
          aria-label="Mostrar câmera ${BrunoCozinhaSubview._escapeAttr(cameraName)}"
        >
          ${content}
        </button>
      `;
    ...
    return `<div class="${classes}" aria-label="Câmera ${BrunoCozinhaSubview._escapeAttr(cameraName)}">${content}</div>`;
  }
  ...
  _renderCameraControls(camera) {
    ...
    const controlMarkup = controls.map((control) => {
      ...
      return `
        <button
          ...
          ${control.unavailable ? 'disabled' : `data-action="toggle-camera-control" data-entity="${BrunoCozinhaSubview._escapeAttr(control.entity)}"`}
          aria-pressed="${control.active ? 'true' : 'false'}"
          aria-label="${BrunoCozinhaSubview._escapeAttr(ariaLabel)}"
          title="${BrunoCozinhaSubview._escapeAttr(ariaLabel)}"
        >
          <bruno-icon icon="${BrunoCozinhaSubview._escapeAttr(control.icon || 'mdi:toggle-switch-outline')}"></bruno-icon>
          <span class="camera-control-label">${BrunoCozinhaSubview._escape(control.label || description)}</span>
    ...
    return `
      <div class="camera-control-strip" aria-label="Controles da câmera ${BrunoCozinhaSubview._escapeAttr(cameraName)}">
  ...
  _cameraFrame(camera) {
    ...
    return `
      <div class="camera-row-image">
        ${image ? `<img src="${BrunoCozinhaSubview._escapeAttr(image)}" data-camera-src-base="${BrunoCozinhaSubview._escapeAttr(base)}" data-camera-entity="${BrunoCozinhaSubview._escapeAttr(camera.entity)}" alt="">` : ''}
        <div class="camera-placeholder" aria-hidden="true"></div>
      </div>
    `;
  }
  _renderApplianceTile(item) {
    const image = item.image ? BrunoCozinhaSubview._escapeAttr(item.image) : '';
    const moreInfoEntity = item.moreInfoEntity || '';
    const toggleEntity = item.controlEntityId || item.entityId || '';
    const canToggle = Boolean(toggleEntity) && !item.unavailable;
    const keyClass = String(item.key || 'item').replace(/[^a-z0-9_-]/gi, '-').toLowerCase();
    return `
      <article class="appliance-tile is-${BrunoCozinhaSubview._escapeAttr(keyClass)}${item.active ? ' is-on' : ''}${item.configured ? '' : ' is-muted'}">
        <button
          type="button"
          class="appliance-main"
          data-action="toggle-appliance"
          data-entity="${BrunoCozinhaSubview._escapeAttr(toggleEntity)}"
          aria-label="${BrunoCozinhaSubview._escapeAttr(item.name || 'Eletrodoméstico')}: ${BrunoCozinhaSubview._escapeAttr(item.label || '')}"
          ${canToggle ? '' : 'disabled'}
        >
          <div class="appliance-visual" data-image-wrapper>
            ${image ? `<img src="${image}" alt="" data-fallback-class="is-image-missing" loading="eager">` : ''}
            <bruno-icon icon="${BrunoCozinhaSubview._escapeAttr(item.icon)}"></bruno-icon>
          </div>
          <div class="appliance-copy">
            <strong>${BrunoCozinhaSubview._escape(item.name || 'Eletrodoméstico')}</strong>
        ...
        <button
          ...
          data-entity="${BrunoCozinhaSubview._escapeAttr(moreInfoEntity)}"
          title="Mais detalhes"
          aria-label="Mais detalhes de ${BrunoCozinhaSubview._escapeAttr(item.name || 'eletrodoméstico')}"
          ${moreInfoEntity ? '' : 'disabled'}
        >
          <bruno-icon icon="mdi:dots-vertical"></bruno-icon>
        </button>
      </article>
  ...
  _renderAppliances(model) {
    const appliances = model.appliances || [];
    return `
      <section class="glass-card appliances-card kitchen-appliances-card">
        <div class="mh-head appliances-head">
          <div class="mh-head-title">
            <span class="micro-icon tone-amber"><bruno-icon icon="mdi:home-lightning-bolt-outline"></bruno-icon></span>
            <div class="module-title">Eletrodomésticos</div>
          </div>
        </div>
        <div class="appliances-grid" aria-label="Eletrodomésticos da cozinha">
          ${appliances.map((item) => this._renderApplianceTile(item)).join('')}
        </div>
      </section>
    `;
  }
  ...
  _renderMediaHub(model) {
    ...
    const esc = (value) => BrunoCozinhaSubview._escape(value);
    const escAttr = (value) => BrunoCozinhaSubview._escapeAttr(value);
    const pcImage = this._config.pc_image || '/local/images/office_pc.png?v=20260702-all-images-1';
    const spotifyArtwork = spotify.artwork ? BrunoCozinhaSubview._resolvePicture(spotify.artwork) : '';
    ...
    const spotifyDeviceLabel = this._config.spotify_device_name || spotify.source || 'Spotify Cozinha';
    ...
    const btn = (action, label, opts = {}) => {
      ...
      return `
        ...
        >
          ${opts.icon ? `<bruno-icon icon="${escAttr(opts.icon)}"></bruno-icon>` : ''}
    ...
    const art = (src, shape, fallbackIcon, cover = false) => `
      <div class="mh-art mh-art-${shape}${cover ? ' is-cover' : ' is-standby'}">
        ${src
          ...
          : `<bruno-icon icon="${escAttr(fallbackIcon)}"></bruno-icon>`}
      </div>
    `;
    ...
    const renderSource = (key) => {
      ...
      const iconHtml = key === 'spotify'
        ...
        : `<bruno-icon class="mh-src-icon" icon="${escAttr(source.icon)}"></bruno-icon>`;
      return `
    ...
    return `
      <section class="glass-card media-hub-card workspace-hub-card mh-accordion${sourceMeta[selected]?.active ? ' is-playing' : ''}">
        <div class="mh-head">
          <div class="mh-head-title">
            <span class="micro-icon"><bruno-icon icon="mdi:desk"></bruno-icon></span>
  ...
  _renderSpotify(model) {
    const spotify = model.spotify;
    const artwork = spotify.artwork ? BrunoCozinhaSubview._resolvePicture(spotify.artwork) : '';
    ...
    return `
      <section class="glass-card spotify-card${spotify.active ? ' is-active' : ''}">
        <div class="module-head">
          ...
          <span class="state-chip"><span></span>${spotify.playing ? 'Playing' : BrunoCozinhaSubview._escape(spotify.state)}</span>
        </div>

        <div class="spotify-body">
          <div class="spotify-art${artwork ? ' has-art' : ''}">
            ${artwork ? `<img src="${BrunoCozinhaSubview._escapeAttr(artwork)}" alt="">` : '<bruno-icon icon="mdi:music-note"></bruno-icon>'}
          </div>
  ...
  _renderClimateRing(climate, target, current, minTarget, maxTarget, dialMode) {
    ...
    const labelMarkup = labels.map((label) => {
      ...
      return `<text x="${p.x.toFixed(3)}" y="${p.y.toFixed(3)}" text-anchor="middle" dominant-baseline="middle" class="icg-label ${label.cls}">${BrunoCozinhaSubview._escape(label.text)}</text>`;
    }).join('');

    return `
      <div class="icg-root">
        <div class="icg-shell">
          <svg class="icg-svg" viewBox="0 0 720 460" role="img" aria-label="${BrunoCozinhaSubview._escapeAttr(`Temperatura alvo ${targetLabel}°. Ambiente ${ambientLabel}°.`)}">
            <defs>
            ...
            <text x="${cx}" y="260" text-anchor="middle" dominant-baseline="middle" class="icg-center-mode">${BrunoCozinhaSubview._escape(modeLabel)}</text>
            <text x="${cx}" y="328" text-anchor="middle" dominant-baseline="middle" class="icg-center-temp">${BrunoCozinhaSubview._escape(targetLabel)}°</text>
            ...
            <text x="${cx}" y="432" text-anchor="middle" dominant-baseline="middle" class="icg-ambient">Ambient ${BrunoCozinhaSubview._escape(ambientLabel)}°</text>
          </svg>
        </div>
      </div>
    `;
  ...
  _renderAC(model) {
    ...
    const climateEntity = BrunoCozinhaSubview._escapeAttr(this._config.entities.climate || '');
    ...
    const renderOption = ({ action, mode, label, icon, active }) => `
      <button
        ...
        data-action="${action}"
        data-mode="${BrunoCozinhaSubview._escapeAttr(mode)}"
        role="menuitem"
      >
        <bruno-icon icon="${BrunoCozinhaSubview._escapeAttr(icon)}"></bruno-icon>
        <span>${BrunoCozinhaSubview._escape(label)}</span>
      </button>
    `;
    ...
    const controlButton = ({ panel, icon, title, value, options }) => `
      <div class="ac-control-wrap">
        ...
        >
          ...
          <span class="ac-action-text"><small>${title}</small><strong>${BrunoCozinhaSubview._escape(value)}</strong></span>
        </button>
  ...
  _styles() {
    return `
      ...
      */
      ...
      /* ===== Cozinha: matriz Office com faixa inferior unica para eletrodomésticos ===== */
      .cozinha-subview {
        grid-template-rows: 48px minmax(0, 1fr) var(--ac-h, 320px) 54px;
        grid-template-areas:
          "topband topband"
          "content right"
      ...
      .right-column {
        ...
        --lights-dock-bottom: calc(7px - var(--office-gap, 10px));
      }
      ...
      /* NOVO: espessura optica — mesmo peso visual dos icones da rail (ver
         comentario em .zl-icon abaixo). Exibido a 44px: 1.227 * 24/44 ≈ 0.67. */
      ...
      /* NOVO (grade 2-col): mesma grade da Sala. 3 luzes (ímpar) => a 1ª (Luz
         principal 1) ocupa a linha inteira; as 2 restantes formam um par. Altura
         controlada pelo _clampExpandedLights (faixas inteiras + scroll). */
      /* NOVO (2ª passada): mini-cabeçalho de status (N/total acesas · Xh) —
         substitui o "acesas" que os quartos têm no cabeçalho da seção. */
      .lights-substatus {
      ...
      .office-light-list::-webkit-scrollbar { width: 0; }
      ...
      */
      ...
      /* ===== Cozinha final: shell das subviews + câmera e eletrodomésticos ===== */
      .cozinha-subview {
        --office-gap: 10px;
        display: grid;
        height: 100%;
        ...
        /* ANTERIOR (rollback 2026-07-29): régua com rodapé de 54px.
           ...
           grid-template-areas:
             ...
             "bottomband bottomband bottomband";
           A linha "bottomband" só carregava o status de presença. Ela saiu para
           que a faixa de tiles (linha dos eletrodomésticos) encoste na base do
           painel; a presença subiu para a barra superior como badge. Para reverter:
           restaurar as duas declarações acima, devolver a chamada de
           _renderFrameBottom() em _render() e voltar o default de
           --bruno-subview-band-height em bruno-surface-material.js. */
      ...
      .cozinha-subview .hero-atmosphere,
      ...
      /* ===== Cozinha microajustes: tiles clicaveis e Air fryer mais discreto ===== */
      ...
      .cozinha-subview .appliance-main:focus-visible {
        ...
        outline-offset: -4px;
        border-radius: calc(var(--office-radius-small) - 3px);
      }
      ...
      /* NOVO (2026-07-22) — consolidacao mobile
         Override final, posterior a grade desktop de tres colunas. Mantem o
         codigo anterior acima para rollback e usa apenas o DOM ativo. */
      @media (max-width: 800px) {
        ...
        /* ANTERIOR (rollback 2026-07-29): seleção POSICIONAL
             .cozinha-subview .topband-badges .tb-badge:nth-child(n + 4),
           ...
           o rodapé já ficava. */
        .cozinha-subview .topband-badges .tb-badge[data-phone-hide],
        .cozinha-subview .topband-clock {
          display: none;
        }
        ...
        /* A Cozinha nao configura cortina nem outro conteudo para o hero. */
        ...
        .cozinha-subview .head-actions .chip-button,
        .cozinha-subview .zone-header {
          min-height: 44px;
        }

        /* Neutraliza os estilos inline de _clampExpandedLights no phone. */
        .cozinha-subview .lights-zones,
        .cozinha-subview .zone-lights,
        .cozinha-subview .office-light-list {
          flex: 0 0 auto;
        ...
        .cozinha-subview .camera-pip-stage,
        .cozinha-subview .camera-feed {
          min-height: 0;
          height: 100%;
        }

        /* ANTERIOR (rollback): o PiP herdava width: min(36%, 150px),
           height: 86px, right: 20px e bottom: 22px do contrato amplo. */
        /* NOVO (2026-07-22) — mantem a camera auxiliar pequena e ancorada no
           canto inferior direito, sem alterar o feed principal. */
        ...
        /* ANTERIOR (rollback): controles abertos elevavam o PiP para 76px. */
...
window.customCards.push({
  ...
  preview: false,
  description: 'Cozinha subview rebuilt from Sala matrix as an isolated Bruno Liquid Glass Web Component.',
});
