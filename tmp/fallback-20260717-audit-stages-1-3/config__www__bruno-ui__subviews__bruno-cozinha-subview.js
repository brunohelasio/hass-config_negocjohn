...
_mediaSourceFromAction(action) {
  ...
  static _curtainSvg(type = 'main') {
    ...
    return `
      <svg class="curtain-svg is-${BrunoCozinhaSubview._escapeAttr(type)}" viewBox="0 0 48 48" width="${size}" height="${size}" aria-hidden="true">
  ...
  _renderLights(model) {
    ...
    return `
      <div class="glass-card lights-card cozinha-lights-card">
        ...
        <div class="lights-substatus">${onCount}/${visibleLights.length} acesas${onLabel ? ` · ${onLabel}` : ''}</div>

        <div class="office-light-list">
          ${visibleLights.map((light, i) => this._renderZoneTile(light, isOddLights && i === 0)).join('')}
        </div>
      </div>
    `;
  }
    ...
    const controlMarkup = controls.map((control) => {
      ...
      return `
        <button
          ...
          title="${BrunoCozinhaSubview._escapeAttr(ariaLabel)}"
        >
          <ha-icon icon="${BrunoCozinhaSubview._escapeAttr(control.icon || 'mdi:toggle-switch-outline')}"></ha-icon>
  ...
  _cameraFrame(camera) {
    ...
    return `
      <div class="camera-row-image">
        ...
        ${camera?.entity ? `<div class="camera-live" data-camera-live-mount="${BrunoCozinhaSubview._escapeAttr(camera.entity)}"></div>` : ''}
        <div class="camera-placeholder" aria-hidden="true"></div>
      </div>
    `;
  }
    ...
    return `
      <section class="glass-card appliances-card kitchen-appliances-card">
        <div class="mh-head appliances-head">
          <div class="mh-head-title">
            <span class="micro-icon tone-amber"><ha-icon icon="mdi:home-lightning-bolt-outline"></ha-icon></span>
      ...
      <section class="glass-card spotify-card${spotify.active ? ' is-active' : ''}">
        ...
        <div class="spotify-body">
          <div class="spotify-art${artwork ? ' has-art' : ''}">
            ${artwork ? `<img src="${BrunoCozinhaSubview._escapeAttr(artwork)}" alt="">` : '<ha-icon icon="mdi:music-note"></ha-icon>'}
          </div>
  ...
  _renderAC(model) {
    ...
    const renderOption = ({ action, mode, label, icon, active }) => `
      <button
        ...
        role="menuitem"
      >
        <ha-icon icon="${BrunoCozinhaSubview._escapeAttr(icon)}"></ha-icon>
  ...
  _styles() {
    return `
      ...
      */
      ...
      .appliance-visual ha-icon {
        --mdc-icon-size: 44px;
        color: rgba(255,255,255,0.24);
      }

      .appliance-visual img + ha-icon {
        display: none;
      }
      ...
      .appliance-visual.is-image-missing ha-icon {
        display: block;
      }
