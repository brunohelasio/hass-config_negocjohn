const BRUNO_VISIONOS_VERSION = '20260702-visionos-yaml-2';
const BRUNO_VISIONOS_STYLE_ID = 'bruno-liquid-glass-tokens';

const BRUNO_VISIONOS_TOKENS = {
...
Object.assign(BRUNO_VISIONOS_TOKENS, {
...
const BRUNO_VISIONOS_GLOBAL_CSS = `
...
function brunoVisionOSSerialize(tokens) {
  return Object.entries(tokens)
    .map(([name, value]) => `  --${name}: ${String(value).trim().replace(/\s+/g, ' ')};`)
    .join('\n');
}

function brunoVisionOSApply(root = globalThis.document) {
  if (!root?.head) return null;

  let style = root.getElementById(BRUNO_VISIONOS_STYLE_ID);
  if (!style) {
    style = root.createElement('style');
    style.id = BRUNO_VISIONOS_STYLE_ID;
    root.head.appendChild(style);
  }

  style.textContent = `:root {\n${brunoVisionOSSerialize(BRUNO_VISIONOS_TOKENS)}\n}\n${BRUNO_VISIONOS_GLOBAL_CSS}`;
  return style;
}

function brunoVisionOSFeedback(kind = 'tap') {
...
function brunoVisionOSRouteTransition(duration = 280) {
...
const BRUNO_VISIONOS_API = {
  version: BRUNO_VISIONOS_VERSION,
  tokens: BRUNO_VISIONOS_TOKENS,
  surfaces: BRUNO_VISIONOS_SURFACES,
  states: BRUNO_VISIONOS_STATES,
  apply: brunoVisionOSApply,
  feedback: brunoVisionOSFeedback,
  routeTransition: brunoVisionOSRouteTransition,
};

globalThis.BrunoVisionOS = BRUNO_VISIONOS_API;
if (!globalThis.BrunoLiquidGlass) globalThis.BrunoLiquidGlass = BRUNO_VISIONOS_API;
