...
export const SUBVIEW_TELEFONE_CSS = css`
  @media (max-width: 800px) {
    ...
    :host([data-folha='midia']) .room-subview .mh-source-head {
      min-height: var(--fone-hit-min);
      height: var(--fone-hit-min);
      align-items: center;
      padding-block: 0;
      border: 0;
      border-radius: 0;
    ...
    :host([data-folha='midia']) .room-subview .mh-source-body {
      /* No telefone a arte compartilha apenas a linha de metadados. Volume,
         progresso e transportes ganham a largura completa na linha seguinte;
         por isso reduzir a largura do aparelho nao espreme os controles. */
      min-height: 0;
      grid-template-columns: minmax(0, 1fr) clamp(88px, 26cqi, 112px);
      grid-template-rows: auto auto;
      grid-template-areas:
        'info art'
        'controls controls';
      ...
      padding: 4px clamp(12px, 3.4cqi, 16px) clamp(12px, 3.4cqi, 16px);
      border-top: 0;
    }
