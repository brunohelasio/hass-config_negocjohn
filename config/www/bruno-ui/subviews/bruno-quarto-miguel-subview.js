...
const BRUNO_QUARTO_MIGUEL_SUBVIEW_DEFAULT_CONFIG = {
  ...
  entities: {
    ...
    temperature: ['sensor.temperatura_quarto_miguel', 'sensor.qmi_temperatura'],
    humidity: ['sensor.umidade_quarto_miguel', 'sensor.qmi_umidade'],
...
class BrunoQuartoMiguelSubview extends HTMLElement {
  ...
  _spotifyModel() {
    ...
    const attrs = entity?.attributes || {};
    const state = entity?.state || 'off';
    const active = BRUNO_QUARTO_MIGUEL_SUBVIEW_MEDIA_ON_STATES.includes(state);
  ...
  _sceneContextLabel(model) {
    ...
    if (model.lights > 0) return 'Q. Miguel iluminado';
    return active?.state === 'yes' ? 'Q. Miguel ativo' : 'Q. Miguel em repouso';
  }
  ...
  _renderHero(model) {
    const title = BrunoQuartoMiguelSubview._escape(this._config.title);
    ...
    return `
      <div class="hero-stage">
        ...
        <div class="hero-content">
          <div class="hero-top">
            <button class="back-button" type="button" data-action="navigate" data-path="${BrunoQuartoMiguelSubview._escapeAttr(this._config.navigation_path)}" aria-label="Voltar">
          ...
          <div class="hero-headline">
            <p class="hero-date-line">${BrunoQuartoMiguelSubview._escape(this._dateLine())}</p>
            <div class="hero-clock" data-clock>${this._lastMinute}</div>
            <button type="button" class="scene-pill" data-action="more-info" data-entity="${BrunoQuartoMiguelSubview._escapeAttr(this._config.entities.active_sensor)}">
              <ha-icon icon="mdi:movie-open-star"></ha-icon>
              <span>${BrunoQuartoMiguelSubview._escape(this._sceneContextLabel(model))}</span>
            </button>
          </div>
    ...
    return `
      <section class="glass-card cameras-card">
        ...
        <div class="camera-stage camera-single">
          <button type="button" class="camera-main" data-action="more-info" data-entity="${BrunoQuartoMiguelSubview._escapeAttr(active?.entity || '')}">
            ${this._cameraFrame(active)}
            <div class="camera-row-copy">
              <strong>${BrunoQuartoMiguelSubview._escape(active?.name || 'Camera')}</strong>
              <span><span class="live-dot"></span>${BrunoQuartoMiguelSubview._escape(active?.status || 'Online')}</span>
            </div>
  ...
  _renderAC(model) {
    ...
    const deviceName = BrunoQuartoMiguelSubview._escape(this._config.climate_device_name || 'Ar condicionado');
    const climateImage = BrunoQuartoMiguelSubview._escapeAttr(this._config.climate_image || '/local/images/ar-condicionado-gree-tight.png');
    const climateActiveImage = BrunoQuartoMiguelSubview._escapeAttr(this._config.climate_active_image || '/local/images/ar-condicionado-gree-on-tight.png?v=20260606-on-1');
      ...
      <section class="glass-card ac-card">
        ...
        <div class="ac-body">
          ...
          <label class="temperature-slider" aria-label="Temperatura do ar condicionado">
            <input type="range" min="${BrunoQuartoMiguelSubview._escapeAttr(minTarget)}" max="${BrunoQuartoMiguelSubview._escapeAttr(maxTarget)}" step="${BrunoQuartoMiguelSubview._escapeAttr(targetStep)}" value="${targetNumber}" data-action="climate-target" ${climateConfigured ? '' : 'disabled'}>
          </label>
          <div class="climate-mode-row" aria-label="Modo do ar condicionado">
            ${modeButtons.map((button) => `
              <button
                ...
                data-mode="${BrunoQuartoMiguelSubview._escapeAttr(button.key)}"
                title="${BrunoQuartoMiguelSubview._escapeAttr(button.label)}"
          ...
          <div class="fan-mode-row">
            ${fanButtons.map((button) => `
              <button
                ...
                data-action="${BrunoQuartoMiguelSubview._escapeAttr(button.action)}"
                data-mode="${BrunoQuartoMiguelSubview._escapeAttr(button.mode)}"
                ${button.mode && climateConfigured ? '' : 'disabled'}
              >${BrunoQuartoMiguelSubview._escape(button.label)}</button>
            `).join('')}
          </div>
        </div>
      </section>
