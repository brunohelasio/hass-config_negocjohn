 ...
 *
 * `motivo` (Fase 6.1) é o que acordou o componente — normalmente os ids de
 * entidade que mudaram, resumidos. A baseline anterior contou 3.328 renders de
 * ladrilho e não soube dizer de onde vinham; contagem sem causa não conserta
 * nada.
 */
export function medirRender<T>(dono: string, fn: () => T, motivo?: string): T {
  const t0 = performance.now();
  try {
    return fn();
  } finally {
    coletor.renderizou(dono, performance.now() - t0, motivo);
  }
}
...
/**
 * Congela a contagem de recursos como referência.
 *
 * O aceite da Fase 6.1: marcar, navegar 50 vezes por todas as seções, voltar e
 * conferir que nada sobrou. Fica exposto no `window.brunoRuntime` para poder ser
 * acionado do console do tablet, sem precisar de build novo.
 */
export function marcar(): ReturnType<typeof coletor.marcar> {
  return coletor.marcar(performance.now());
}
