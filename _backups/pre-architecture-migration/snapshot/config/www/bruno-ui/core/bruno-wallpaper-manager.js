const BRUNO_WALLPAPER_MANAGER_VERSION = '20260718-wallpaper-upload-1';
...
const BrunoWallpaperManager = {
  ...
  async upload({ hass, key, file } = {}) {
    if (!file) throw new Error('Selecione uma imagem.');
    if (!/^image\/(?:jpeg|png|gif)$/i.test(String(file.type || ''))) {
    ...
    if (!uploaded?.id) throw new Error('O Home Assistant nao retornou o identificador da imagem.');
    const url = `/api/image/serve/${encodeURIComponent(uploaded.id)}/original`;
    await this.save({ hass, key, url });
    return url;
  },
