// ============================================================================
// bruno-sala-room-card — Sala em ESTRUTURA PADRAO de comodo (Home V2).
//
// Clone estrutural fiel de bruno-quarto-casal-card.js (base aprovada pelo
// usuario em 2026-07-24), com entidades/assets/status da Sala. Criado para a
// faixa horizontal de comodos da Home V2, onde a Sala abandona a hierarquia
// especial (hero + strip TV/A-C/Corredor) e passa a ter o MESMO molde dos
// demais comodos.
//
// FALLBACK RAPIDO:
// - O card especial da Sala (bruno-sala-card.js) permanece INTACTO e continua
//   sendo usado pela Home V1 (section_home.yaml) e pelo layout phone.
// - Para remover este card: comentar o resource em configuration.yaml e o
//   bloco correspondente em v2/bento_comodos_strip.yaml.
// ============================================================================

const BRUNO_SALA_ROOM_CARD_TAG = 'bruno-sala-room-card';

const BRUNO_SALA_ROOM_DEFAULT_CONFIG = {
  "name": "Sala",
  "icon_size": 94,
  "room_on_states": [
    "on",
  ...
  "entities": {
    ...
    // Mesma camada supervisionada usada por bruno-sala-card.js.
...
const BRUNO_SALA_ROOM_ACTION_COOLDOWN = 1200;

class BrunoSalaRoomCard extends HTMLElement {
  ...
  setConfig(config) {
    const entities = {
      ...BRUNO_SALA_ROOM_DEFAULT_CONFIG.entities,
      ...(config?.entities || {}),
    };
    const icon = {
      ...BRUNO_SALA_ROOM_DEFAULT_CONFIG.icon,
      ...(config?.icon || {}),
    };

    this._config = {
      ...BRUNO_SALA_ROOM_DEFAULT_CONFIG,
      ...config,
      entities,
      icon,
      room_on_states: this._array(config?.room_on_states || BRUNO_SALA_ROOM_DEFAULT_CONFIG.room_on_states),
      semantic_on_states: this._array(config?.semantic_on_states || BRUNO_SALA_ROOM_DEFAULT_CONFIG.semantic_on_states || []),
      status_dots: Array.isArray(config?.status_dots) ? config.status_dots : BRUNO_SALA_ROOM_DEFAULT_CONFIG.status_dots,
    };
    this._render();
  }
  ...
  _sensorValue(entityIds, suffix = '') {
    ...
    if (!entity) return '';
    return `${BrunoSalaRoomCard._escape(entity.state)}${suffix}`;
  }
  ...
  // Linha semantica "Ocupada" (mesmo padrao dos demais cards de comodo).
  _semanticLine() {
    ...
    // Fallback exige presenca ativa junto com a ocupacao (regra 2026-07-04).
  ...
  _isActionCoolingDown(key) {
    ...
    if (now - previous < BRUNO_SALA_ROOM_ACTION_COOLDOWN) return true;
  ...
  _wireAction(button) {
    ...
    // Mesma tolerancia de arraste da base (consolidacao mobile 2026-07-22).
  ...
  _statusDot(dot) {
    ...
    return `
      <span class="status-dot tone-${dot.tone}${activeClass}" title="${BrunoSalaRoomCard._escape(dot.label)}" aria-label="${BrunoSalaRoomCard._escape(dot.label)}">
  ...
  _statusLines(lines) {
    ...
    return lines.map((line) => `<span>${BrunoSalaRoomCard._escape(line)}</span>`).join('');
  }

  _assetVisual(active) {
    const icon = this._config.icon;
    const fallback = BrunoSalaRoomCard._escape(icon.fallback || 'mdi:home-outline');
    const off = BrunoSalaRoomCard._escape(icon.off || '');
    const on = BrunoSalaRoomCard._escape(icon.on || '');
    return `
  ...
  _render() {
    ...
    const iconSize = Number(this._config.icon_size) || BRUNO_SALA_ROOM_DEFAULT_CONFIG.icon_size;
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        * {
        ...
        .room-nav-zone.is-navigating .room-chevron {
          animation: brunoSalaRoomChevronNavigate 360ms ease both;
        }

        @keyframes brunoSalaRoomChevronNavigate {
        ...
        /* Card de comodo no phone (mesmo ajuste da base casal). */
      ...
      <div class="room-card${roomActiveClass}${this._tileClasses()}" style="--room-icon-size:${iconSize}px;">
        ${this._tileDivider()}
        <button class="room-action${hasMetricClass}" type="button" data-action-key="room" aria-label="${BrunoSalaRoomCard._escape(this._config.name)}">
          <div class="room-icon" aria-hidden="true">${this._assetVisual(model.iconActive)}</div>

          <span class="room-nav-zone" data-room-nav role="button" tabindex="0" aria-label="Abrir ${BrunoSalaRoomCard._escape(this._config.name)}">
            <span class="room-title-row">
              <span class="title">${BrunoSalaRoomCard._escape(this._config.name)}</span>
...
if (!customElements.get(BRUNO_SALA_ROOM_CARD_TAG)) {
  customElements.define(BRUNO_SALA_ROOM_CARD_TAG, BrunoSalaRoomCard);
}

window.customCards = window.customCards || [];
window.customCards.push({
  type: BRUNO_SALA_ROOM_CARD_TAG,
  name: 'Bruno Sala Room Card',
  preview: false,
  description: 'Sala in standard room-card structure for the Home V2 rooms strip (clone of the Q. Casal base).',
});
