...
export class BrunoDevicesPanel extends LitElement {
  /**
   * `_selecionado` continua reativo — é estado de interface, e a troca DEVE
   * repintar. `_hass` saiu (Fase 6.1): propriedade reativa pede render a cada
   * atribuição, por cima da guarda do setter. Ver a nota em `bruno-room-tile.ts`.
   */
  ...
  /**
   * Estado seletivo (Fase 6.1).
   *
   * A lista observada é a união das entidades de TODAS as instâncias — é o que
   * o painel lê para pintar a lista da esquerda (nome e se está ativo), mesmo
   * dos aparelhos não selecionados. Vem do registry, que é a promessa do
   * componente: acrescentar um aparelho não muda este arquivo, e passa a ser
   * observado sozinho.
   */
  private readonly _observador = new ObservadorDeEntidades(
    DEVICES.flatMap((d) => entidadesDaInstancia(d)),
  );
  private _motivo = '';

  set hass(v: Hass) {
    this._hass = v;
    // Os controles filhos recebem o hass sempre: cada um tem a própria guarda,
    // e o de dentro sabe o que lê melhor do que o painel.
    for (const el of this._controles.values()) el.hass = v;
    const mudou = this._observador.mudancas(v);
  ...
  override update(mudancas: Map<string, unknown>): void {
    ...
    this._motivo = '';
    medirRender(SONDA, () => super.update(mudancas), motivo || 'outro');
  }
