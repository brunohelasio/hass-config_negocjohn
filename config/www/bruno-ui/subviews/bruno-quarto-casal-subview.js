const BRUNO_QUARTO_CASAL_SUBVIEW_TAG = 'bruno-quarto-casal-subview';

const BRUNO_QUARTO_CASAL_SUBVIEW_DEFAULT_CONFIG = {
  title: 'Q. Casal',
  subtitle: 'Visao geral',
  greeting_name: 'Bruno',
  navigation_path: 'bento-lab',
  background: '/local/images/quarto_casal.jpg',
  fallback_background: '/local/images/quarto_casal.jpg',
  ...
  room_nav: [
    ...
    { key: 'casal', name: 'Q. Casal', icon: 'mdi:bed-king', path: 'subview-quarto-casal', active: true },
  ...
  entities: {
    ...
    lights: [
      ...
      { entity: 'light.quarto_casal_2_switch_3', name: 'Luz cortineiro', icon_type: 'ledstrip', zone: 'varanda' },
    ...
    cameras: [
      { entity: 'camera.camera_quarto_casal_2', name: 'Quarto Casal', short_name: 'Casal' },
    ],
  },
};

const BRUNO_QUARTO_CASAL_SUBVIEW_CLIMATE_ON_STATES = ['cool', 'heat', 'fan_only', 'dry', 'heat_cool', 'auto'];
const BRUNO_QUARTO_CASAL_SUBVIEW_CLIMATE_ACTIVE_ACTIONS = ['cooling', 'heating', 'drying', 'fan', 'preheating'];
const BRUNO_QUARTO_CASAL_SUBVIEW_CLIMATE_INACTIVE_ACTIONS = ['off', 'idle'];
const BRUNO_QUARTO_CASAL_SUBVIEW_TV_ON_STATES = ['on', 'playing', 'paused', 'idle'];
const BRUNO_QUARTO_CASAL_SUBVIEW_MEDIA_ON_STATES = ['playing', 'paused', 'on', 'idle'];
const BRUNO_QUARTO_CASAL_SUBVIEW_CAMERA_ONLINE_STATES = ['streaming', 'recording', 'idle', 'on'];
const BRUNO_QUARTO_CASAL_SUBVIEW_UNAVAILABLE_STATES = ['unavailable', 'unknown', '', 'none', 'null'];
const BRUNO_QUARTO_CASAL_SUBVIEW_TV_ICON_ANIMATION_MS = 1000;

class BrunoQuartoCasalSubview extends HTMLElement {
  ...
  _normalizeConfig(config) {
    const entities = {
      ...BRUNO_QUARTO_CASAL_SUBVIEW_DEFAULT_CONFIG.entities,
      ...(config.entities || {}),
    };
    ...
    return {
      ...BRUNO_QUARTO_CASAL_SUBVIEW_DEFAULT_CONFIG,
      ...config,
      refresh_interval: Math.max(4000, Number(config.refresh_interval) || BRUNO_QUARTO_CASAL_SUBVIEW_DEFAULT_CONFIG.refresh_interval),
      room_nav: Array.isArray(config.room_nav) ? config.room_nav : BRUNO_QUARTO_CASAL_SUBVIEW_DEFAULT_CONFIG.room_nav,
      tv_apps: Array.isArray(config.tv_apps) ? config.tv_apps : BRUNO_QUARTO_CASAL_SUBVIEW_DEFAULT_CONFIG.tv_apps,
      entities,
    };
  }
  ...
  _renderError(error) {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="error">Erro na subview Q. Casal: ${BrunoQuartoCasalSubview._escape(error?.message || error)}</div>
    `;
  }
  ...
  _refreshCameraImages() {
    ...
    this.shadowRoot.querySelectorAll('img[data-camera-src-base]').forEach((image) => {
      ...
      const nextSrc = BrunoQuartoCasalSubview._withCacheBust(baseSrc, stamp);
  ...
  _isUnavailable(entity) {
    return !entity || BRUNO_QUARTO_CASAL_SUBVIEW_UNAVAILABLE_STATES.includes(String(entity.state || '').toLowerCase());
  }
  ...
  _cameraState(camera) {
    ...
    const online = !unavailable && BRUNO_QUARTO_CASAL_SUBVIEW_CAMERA_ONLINE_STATES.includes(state);
    ...
    return {
      ...
      online,
      image,
      imageUrl: this._loadedCameraUrls[camera.entity] || BrunoQuartoCasalSubview._withCacheBust(image, this._refreshSeed),
  ...
  _tvModel() {
    ...
    const active = BRUNO_QUARTO_CASAL_SUBVIEW_TV_ON_STATES.includes(state);
    return {
      entity,
      state,
      active,
  ...
  _spotifyModel() {
    ...
    const attrs = entity?.attributes || {};
    const state = entity?.state || 'off';
    const globalActive = BRUNO_QUARTO_CASAL_SUBVIEW_MEDIA_ON_STATES.includes(state);
    ...
    const roomSource = this._config.spotify_device_name || attrs.source || 'Echo Pop Quarto Casal';
    return {
      entity,
  ...
  _climateIsActive(entity) {
    ...
    if (BRUNO_QUARTO_CASAL_SUBVIEW_CLIMATE_ACTIVE_ACTIONS.includes(hvacAction)) return true;
    if (BRUNO_QUARTO_CASAL_SUBVIEW_CLIMATE_INACTIVE_ACTIONS.includes(hvacAction)) return false;
    return BRUNO_QUARTO_CASAL_SUBVIEW_CLIMATE_ON_STATES.includes(entity.state || '');
  }
  ...
  _sceneContextLabel(model) {
    ...
    if (model.lights > 0) return 'Q. Casal iluminado';
    return active?.state === 'yes' ? 'Q. Casal ativo' : 'Q. Casal em repouso';
  }
  ...
  _renderHero(model) {
    const title = BrunoQuartoCasalSubview._escape(this._config.title);
    const subtitle = BrunoQuartoCasalSubview._escape(this._config.subtitle);
    const background = BrunoQuartoCasalSubview._escapeAttr(this._config.background);
    const fallbackBackground = BrunoQuartoCasalSubview._escapeAttr(this._config.fallback_background || this._config.background);
    ...
    return `
      <div class="hero-stage">
        ...
        <div class="hero-content">
          <div class="hero-top">
            <button class="back-button" type="button" data-action="navigate" data-path="${BrunoQuartoCasalSubview._escapeAttr(this._config.navigation_path)}" aria-label="Voltar">
          ...
          <div class="hero-headline">
            <p class="hero-date-line">${BrunoQuartoCasalSubview._escape(this._dateLine())}</p>
            <div class="hero-clock" data-clock>${this._lastMinute}</div>
            <button type="button" class="scene-pill" data-action="more-info" data-entity="${BrunoQuartoCasalSubview._escapeAttr(this._config.entities.active_sensor)}">
              <ha-icon icon="mdi:movie-open-star"></ha-icon>
              <span>${BrunoQuartoCasalSubview._escape(this._sceneContextLabel(model))}</span>
            </button>
          </div>
          ...
          <div class="curtain-dock${curtainConfigured ? '' : ' is-disabled'}" style="--curtain-position: ${curtainVisualPosition}%;">
            <div class="curtain-control-row">
              <div class="curtain-identity">
                <span class="curtain-icon-shell">${BrunoQuartoCasalSubview._curtainSvg('main')}</span>
                <span class="curtain-title">Cortina</span>
              </div>
              <div class="curtain-status" aria-live="polite">
                <span class="curtain-status-text">${BrunoQuartoCasalSubview._escape(curtainStatus)}</span>
                <span class="curtain-status-percent">${BrunoQuartoCasalSubview._escape(curtainPercent)}</span>
              </div>
              <div class="curtain-main-actions">
                <button type="button" class="curtain-action-button" data-action="cover-open" ${curtainDisabled}>
                  ${BrunoQuartoCasalSubview._curtainSvg('open')}<span>Abrir</span>
                </button>
                <button type="button" class="curtain-action-button is-muted" data-action="cover-stop" ${curtainDisabled}>
                  ${BrunoQuartoCasalSubview._curtainSvg('stop')}<span>Parar</span>
                </button>
                <button type="button" class="curtain-action-button" data-action="cover-close" ${curtainDisabled}>
                  ${BrunoQuartoCasalSubview._curtainSvg('close')}<span>Fechar</span>
                </button>
              </div>
            </div>
      ...
      <nav class="room-sidebar" aria-label="Navegacao de comodos">
        ${items.map((item) => `
          <button
            ...
            data-path="${BrunoQuartoCasalSubview._escapeAttr(item.path || this._config.navigation_path)}"
            title="${BrunoQuartoCasalSubview._escapeAttr(item.name)}"
            aria-label="${BrunoQuartoCasalSubview._escapeAttr(item.name)}"
          >
            ${BrunoQuartoCasalSubview._roomNavIcon(item.key || item.icon)}
          </button>
        `).join('')}
      </nav>
    `;
  ...
  _renderLightZoneRail(lights, selectedZone) {
    ...
    return `
      <aside
        ...
        data-zone="${BrunoQuartoCasalSubview._escapeAttr(selectedZone)}"
        data-dimmer-entity="${BrunoQuartoCasalSubview._escapeAttr(dimmerTarget?.entity || '')}"
        data-dimmer-level="${BrunoQuartoCasalSubview._escapeAttr(String(fillPercent))}"
      ...
      >
        <span class="rail-zone">${BrunoQuartoCasalSubview._escape(label)}</span>
  ...
  _renderLightTile(light) {
    ...
    return `
      <button type="button" class="light-tile${active ? ' is-on' : ''}${disabled ? ' is-placeholder' : ''}" ${disabled ? 'disabled' : `data-action="toggle-light" data-entity="${BrunoQuartoCasalSubview._escapeAttr(light.entity)}"`}>
        <span class="light-icon">${BrunoQuartoCasalSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <strong>${BrunoQuartoCasalSubview._escape(light.name)}</strong>
        ...
        <div class="module-head">
          <div class="lights-title-row">
            ...
            <div class="zone-toggle" role="tablist" aria-label="Zona das luzes">
              <button type="button" class="${selectedZone === 'sala' ? 'is-active' : ''}" data-action="select-light-zone" data-zone="sala">${BrunoQuartoCasalSubview._escape(salaLabel)}</button>
              <button type="button" class="${selectedZone === 'varanda' ? 'is-active' : ''}" data-action="select-light-zone" data-zone="varanda">${BrunoQuartoCasalSubview._escape(varandaLabel)}</button>
            </div>
          </div>
    ...
    return `
      <section class="glass-card cameras-card">
        ...
        <div class="camera-stage camera-single">
          <button type="button" class="camera-main" data-action="more-info" data-entity="${BrunoQuartoCasalSubview._escapeAttr(active?.entity || '')}">
            ${this._cameraFrame(active)}
            <div class="camera-row-copy">
              <strong>${BrunoQuartoCasalSubview._escape(active?.name || 'Camera')}</strong>
              <span><span class="live-dot"></span>${BrunoQuartoCasalSubview._escape(active?.status || 'Online')}</span>
            </div>
  ...
  _renderMediaHub(model) {
    ...
    if (tvStateChanged) {
      this._mediaTvAnimationUntil = now + BRUNO_QUARTO_CASAL_SUBVIEW_TV_ICON_ANIMATION_MS;
    ...
    const renderMeta = (value) => (
      value
        ? `<small>${BrunoQuartoCasalSubview._escape(value)}</small>`
    ...
    const spotifyMeta = metaLine(
      spotify.subtitle,
      spotify.source || this._config.spotify_device_name || 'Echo Pop Quarto Casal',
    );
    const mediaActionButton = ({
      ...
      <button
        ...
        data-action="${BrunoQuartoCasalSubview._escapeAttr(action)}"
        title="${BrunoQuartoCasalSubview._escapeAttr(label)}"
        aria-label="${BrunoQuartoCasalSubview._escapeAttr(label)}"
        ${attrs}
        ${disabled ? 'disabled' : ''}
      >
    ...
    const mediaIdentityCell = (type, active = false, options = {}) => `
      <span class="media-identity-cell is-${BrunoQuartoCasalSubview._escapeAttr(type)}${active ? ' is-active' : ''}" aria-hidden="true">
        ${BrunoQuartoCasalSubview._tplMediaIcon(type, { active, animate: Boolean(options.animate) })}
      </span>
    `;
    ...
    const tvAppButtons = (this._config.tv_apps || []).slice(0, 4).map((app) => {
      ...
      const image = app.image ? BrunoQuartoCasalSubview._escapeAttr(app.image) : '';
      const keyClass = app.key ? ` app-${BrunoQuartoCasalSubview._escapeAttr(app.key)}` : '';
      return `
        <button
          type="button"
          class="media-action-button media-image-button${keyClass}"
          data-action="tv-app"
          data-script="${BrunoQuartoCasalSubview._escapeAttr(app.script || '')}"
          title="${BrunoQuartoCasalSubview._escapeAttr(label)}"
          aria-label="${BrunoQuartoCasalSubview._escapeAttr(label)}"
    ...
    const sources = {
      tv: {
        ...
        visual: tvPoster
          ? `<img src="${BrunoQuartoCasalSubview._escapeAttr(tvPoster)}" alt="">`
      ...
      spotify: {
        ...
        visual: spotifyArtwork
          ? `<img src="${BrunoQuartoCasalSubview._escapeAttr(spotifyArtwork)}" alt="">`
          ...
          <div class="media-tabs" role="tablist" aria-label="Fonte de midia">
            ${tabs.map((source) => `
              ...
              >
                ...
                <span>${BrunoQuartoCasalSubview._escape(source.label)}</span>
                <small>${BrunoQuartoCasalSubview._escape(source.state)}</small>
              </button>
            `).join('')}
          </div>
        </div>
        ...
        <div class="media-hub-body">
          ...
          <div class="media-hub-content">
            <div class="media-details">
              <strong>${BrunoQuartoCasalSubview._escape(current.title)}</strong>
              ${current.meta}
            </div>
  ...
  _renderTV(model) {
    const tv = model.tv;
    const poster = tv.poster ? BrunoQuartoCasalSubview._resolvePicture(tv.poster) : '';
      ...
      <section class="glass-card tv-card${tv.active ? ' is-active' : ''}">
        ...
        <div class="tv-body">
          ...
          <div class="poster-card${poster ? ' has-poster' : ''}">
            ${poster ? `<img src="${BrunoQuartoCasalSubview._escapeAttr(poster)}" alt="">` : '<span>Poster</span>'}
          </div>
        </div>
      </section>
    `;
  ...
  _renderSpotify(model) {
    const spotify = model.spotify;
    const artwork = spotify.artwork ? BrunoQuartoCasalSubview._resolvePicture(spotify.artwork) : '';
    ...
    return `
      <section class="glass-card spotify-card${spotify.active ? ' is-active' : ''}">
        <div class="module-head">
          ...
          <span class="state-chip"><span></span>${spotify.playing ? 'Playing' : BrunoQuartoCasalSubview._escape(spotify.state)}</span>
        </div>
  ...
  _renderClimateRing(climate, target, current, minTarget, maxTarget, dialMode) {
    ...
    const labelMarkup = labels.map((label) => {
      ...
      return `<text x="${p.x.toFixed(3)}" y="${p.y.toFixed(3)}" text-anchor="middle" dominant-baseline="middle" class="icg-label ${label.cls}">${BrunoQuartoCasalSubview._escape(label.text)}</text>`;
    }).join('');

    return `
      <div class="icg-root">
        <div class="icg-shell">
          <svg class="icg-svg" viewBox="0 0 720 460" role="img" aria-label="${BrunoQuartoCasalSubview._escapeAttr(`Temperatura alvo ${targetLabel}°. Ambiente ${ambientLabel}°.`)}">
            <defs>
            ...
            <text x="${cx}" y="260" text-anchor="middle" dominant-baseline="middle" class="icg-center-mode">${BrunoQuartoCasalSubview._escape(modeLabel)}</text>
            <text x="${cx}" y="328" text-anchor="middle" dominant-baseline="middle" class="icg-center-temp">${BrunoQuartoCasalSubview._escape(targetLabel)}°</text>
            ...
            <text x="${cx}" y="432" text-anchor="middle" dominant-baseline="middle" class="icg-ambient">Ambient ${BrunoQuartoCasalSubview._escape(ambientLabel)}°</text>
          </svg>
        </div>
      </div>
    `;
  ...
  _renderAC(model) {
    ...
    const deviceName = BrunoQuartoCasalSubview._escape(this._config.climate_device_name || 'Ar condicionado');
    const climateImage = BrunoQuartoCasalSubview._escapeAttr(this._config.climate_image || '/local/images/ar-condicionado-gree-tight.png');
    const climateActiveImage = BrunoQuartoCasalSubview._escapeAttr(this._config.climate_active_image || '/local/images/ar-condicionado-gree-on-tight.png?v=20260606-on-1');
      ...
      <section class="glass-card ac-card">
        ...
        <div class="ac-body">
          ...
          <label class="temperature-slider" aria-label="Temperatura do ar condicionado">
            <input type="range" min="${BrunoQuartoCasalSubview._escapeAttr(minTarget)}" max="${BrunoQuartoCasalSubview._escapeAttr(maxTarget)}" step="${BrunoQuartoCasalSubview._escapeAttr(targetStep)}" value="${targetNumber}" data-action="climate-target" ${climateConfigured ? '' : 'disabled'}>
          </label>
          <div class="climate-mode-row" aria-label="Modo do ar condicionado">
            ${modeButtons.map((button) => `
              <button
                ...
                data-mode="${BrunoQuartoCasalSubview._escapeAttr(button.key)}"
                title="${BrunoQuartoCasalSubview._escapeAttr(button.label)}"
          ...
          <div class="fan-mode-row">
            ${fanButtons.map((button) => `
              <button
                ...
                data-action="${BrunoQuartoCasalSubview._escapeAttr(button.action)}"
                data-mode="${BrunoQuartoCasalSubview._escapeAttr(button.mode)}"
                ${button.mode && climateConfigured ? '' : 'disabled'}
              >${BrunoQuartoCasalSubview._escape(button.label)}</button>
            `).join('')}
          </div>
        </div>
      </section>
  ...
  static _escapeAttr(value) {
    return BrunoQuartoCasalSubview._escape(value).replace(/'/g, '&#39;');
  }
}

if (!customElements.get(BRUNO_QUARTO_CASAL_SUBVIEW_TAG)) {
  customElements.define(BRUNO_QUARTO_CASAL_SUBVIEW_TAG, BrunoQuartoCasalSubview);
}

window.customCards = window.customCards || [];
window.customCards.push({
  type: BRUNO_QUARTO_CASAL_SUBVIEW_TAG,
