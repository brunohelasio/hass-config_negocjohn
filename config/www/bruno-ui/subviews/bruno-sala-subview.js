...
const BRUNO_SALA_SUBVIEW_DEFAULT_CONFIG = {
  ...
  fallback_background: '/local/images/sala_estar.jpg?v=20260702-all-images-1',
  ...
  tv_apps: [
    { key: 'netflix', label: 'Netflix', image: '/local/images/netflix_bg.jpg?v=20260702-all-images-1', script: 'script.sala_tv_open_netflix' },
    { key: 'prime', label: 'Prime Video', image: '/local/images/prime_video_tile.png?v=20260702-all-images-1', script: 'script.sala_tv_open_prime' },
    { key: 'disney', label: 'Disney+', image: '/local/images/dp_bg.jpg?v=20260702-all-images-1', script: 'script.sala_tv_open_disney' },
    { key: 'max', label: 'Max', image: '/local/images/HBOMax_bg.jpg?v=20260702-all-images-1', script: 'script.sala_tv_open_hbo' },
  ],
  room_nav: [
  ...
  entities: {
    ...
    ps5_image: '/local/images/ps5.png?v=20260702-all-images-1',
    lights: [
      ...
      // NOVO (grade 2-col): placeholder removido da lista — não entra na grade.
      // ANTERIOR:
      // { entity: '', name: 'Led Fita TV', icon_type: 'ledstrip', placeholder: true, zone: 'sala' },
      // ANTERIOR (rollback): icon_type: 'light_flush'
      { entity: 'light.sala_2_switch_2', name: 'Luz principal', icon_type: 'ledstrip', zone: 'varanda' },
...
if (action === 'toggle-lights-panel') {
  ...
  _clampExpandedLights() {
    ...
    requestAnimationFrame(() => {
      ...
      // reset antes de medir (mede o conteúdo natural)
      ...
      // espaço reservado pelos blocos DEPOIS do card na coluna (ex.: A/C)
      let afterCard = 0;
      ...
      // dentro do card: zonas FECHADAS abaixo da grade expandida (só acordeão)
      let belowInCard = 0;
      if (expandedZone) {
      ...
      if (rowsThatFit < totalRows) {
        const visRows = Math.max(2, rowsThatFit); // mínimo 2 faixas, resto por scroll
    ...
    return `
      <section class="light-section">
        <div class="section-head">
          ...
          ${showZoneAction
            ? `<button type="button" class="zone-off" data-action="zone-off" data-zone="${zoneKey}">Apagar ${BrunoSalaSubview._escape(zone.name.toLowerCase())}</button>`
            : ''}
        </div>
  ...
  _renderLightCell(light, index, isOdd) {
    ...
    return `
      <button type="button" class="${cls}"
        ...
        data-entity="${BrunoSalaSubview._escapeAttr(light.entity)}"
        role="switch" aria-checked="${active ? 'true' : 'false'}"
        aria-label="${BrunoSalaSubview._escapeAttr(light.name)}">
        <span class="lc-icon">${BrunoSalaSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <span class="lc-name">${BrunoSalaSubview._escape(light.name)}</span>
  ...
  _renderLights(model) {
    ...
    // ANTERIOR (rollback): varanda: 'mdi:string-lights' (luminaria pisca-pisca, nao representava varanda)
    const zoneIcons = { sala: 'mdi:sofa-outline', varanda: 'bruno:balcony', ...(this._config.light_zone_icons || {}) };
  ...
  // Nome distinto de _renderLightTile (mantido acima para rollback do layout 2x2).
  _renderZoneTile(light, wide) {
    ...
    return `
      <button type="button"
        ...
        role="switch" aria-checked="${active ? 'true' : 'false'}"
        aria-label="${BrunoSalaSubview._escapeAttr(light.name)}">
        <span class="zl-icon">${BrunoSalaSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <span class="zl-switch" aria-hidden="true"><span class="zl-knob"></span></span>
        <span class="zl-name">${BrunoSalaSubview._escape(light.name)}</span>
      </button>
    `;
    ...
    const controlMarkup = controls.map((control) => {
      ...
      return `
        <button
          ...
          title="${BrunoSalaSubview._escapeAttr(ariaLabel)}"
        >
          <bruno-icon icon="${BrunoSalaSubview._escapeAttr(control.icon || 'mdi:toggle-switch-outline')}"></bruno-icon>
  ...
  _renderMediaHubLegacy(model) {
    ...
    const mediaActionButton = ({
      ...
      <button
        ...
        ${disabled ? 'disabled' : ''}
      >
        <bruno-icon icon="${BrunoSalaSubview._escapeAttr(icon)}"></bruno-icon>
      </button>
    `;
    ...
    const standbyImage = (src, className, fallbackIcon) => (
      src
        ...
        : `<bruno-icon icon="${BrunoSalaSubview._escapeAttr(fallbackIcon)}"></bruno-icon>`
    );
      ...
      return `
        ...
        >
          ${image
            ...
            : `<bruno-icon icon="${BrunoSalaSubview._escapeAttr(app.icon || 'mdi:apps')}"></bruno-icon>`}
        </button>
      `;
    }).join('');
        ...
        <div class="ps5-body ps5-minimal">
          ...
          <div class="ps5-footer">
            ...
            <div class="ps5-actions">
              <button type="button" class="primary-button" data-action="toggle-ps5" ${ps5.configured ? '' : 'disabled'}>${ps5.active ? 'Desligar' : 'Ligar'}</button>
              <button type="button" class="control-button" data-action="more-info" data-entity="${BrunoSalaSubview._escapeAttr(ps5.entityId || '')}" ${ps5.configured ? '' : 'disabled'}><bruno-icon icon="mdi:dots-horizontal"></bruno-icon></button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="glass-card spotify-card${spotify.active ? ' is-active' : ''}">
        ...
        <div class="spotify-body">
          <div class="spotify-art${artwork ? ' has-art' : ''}">
            ${artwork ? `<img src="${BrunoSalaSubview._escapeAttr(artwork)}" alt="">` : '<bruno-icon icon="mdi:music-note"></bruno-icon>'}
          </div>
  ...
  _styles() {
    return `
      ...
      * {
      ...
      .curtain-svg {
        ...
        /* NOVO (ANTERIOR: 2.1) — espessura optica, mesmo peso visual dos icones
           da rail (ver comentario em .zl-icon acima). Exibido a 16-17px aqui:
      ...
      */
      ...
      /* NOVO: espessura optica — mesmo peso visual dos icones da rail
         (ver comentario em .zl-icon acima). Exibido a 44px: 1.227 * 24/44 ≈ 0.67. */
      ...
      */
      ...
      @media (max-width: 800px) {
        ...
        /* Rede e Zigbee nao possuem entidade nesta subview; no phone a faixa
           resume apenas os estados operacionais do comodo.
           ANTERIOR (rollback 2026-07-29): seleção POSICIONAL
             .topband-badges .tb-badge:nth-child(n + 4),
           NOVO: seleção por ATRIBUTO. A badge de Presença entrou no início da
           faixa e deslocaria o nth-child, que passaria a esconder a Umidade. Com
           a marcação explícita o phone continua exibindo exatamente as mesmas
           três badges de hoje (Luzes, Temperatura, Umidade) — a Presença fica
           oculta no phone, como o rodapé já ficava. */
        ...
        /* _clampExpandedLights pode aplicar estilos inline no desktop. O
           !important garante que ele nao recrie um scroller dentro da Shell. */
        .lights-zones,
        .zone-lights,
        .office-light-list {
          flex: 0 0 auto;
        ...
        /* NOVO (2026-07-22) — a segunda camera volta a ser claramente
           secundaria no mobile, preservando o feed principal integral. */
        .camera-pip-feed {
          right: 16px;
          bottom: 16px;
