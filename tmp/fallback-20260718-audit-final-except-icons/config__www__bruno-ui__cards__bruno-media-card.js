...
const BRUNO_MEDIA_LAST_VALID_KEY = 'bruno-ui:last-valid-media:v2';
...
class BrunoMediaCard extends HTMLElement {
  ...
  setConfig(config) {
    ...
    if (this._lastValidMedia === undefined) this._lastValidMedia = this._readLastValidMedia();
    this._safeRender();
  }
  ...
  _readLastValidMedia() {
    try {
      const raw = globalThis.localStorage?.getItem(BRUNO_MEDIA_LAST_VALID_KEY);
  ...
  _storeLastValidMedia(snapshot) {
    ...
    if (comparable(this._lastValidMedia) === comparable(snapshot)) return;
    this._lastValidMedia = snapshot;
    try {
      globalThis.localStorage?.setItem(BRUNO_MEDIA_LAST_VALID_KEY, JSON.stringify(snapshot));
    } catch (_error) {
      // Storage can be unavailable in private WebViews; the in-memory snapshot remains valid.
    }
  }
  ...
  _playbackPriority(entityId) {
    ...
    const hasMetadata = Boolean(
      ...
      || this._cleanText(attrs.app_name)
      || this._cleanText(attrs.source)
      || (attrs.entity_picture && !this._isStandbyImage(attrs.entity_picture))
    );
    return hasMetadata && this._isVideoPlayer(entityId, config, contentType, attrs.app_name, attrs.source) ? 3 : 0;
  }
  ...
  _focusModel() {
    ...
    const persisted = hasPlayback ? null : this._lastValidMedia;
    const persistedEntity = persisted?.entity ? this._state(persisted.entity) : null;
    const refreshedPersistedImage = persistedEntity?.attributes?.entity_picture || '';
    const displayImage = hasPlayback
      ? image
      : ((!this._isStandbyImage(refreshedPersistedImage) && refreshedPersistedImage) || persisted?.image || '');
  ...
  _hasPlayback(state, title, image, appName, source, entityId = '', config = {}, contentType = '') {
    ...
    return hasMetadata && this._isVideoPlayer(entityId, config, contentType, appName, source);
  }
