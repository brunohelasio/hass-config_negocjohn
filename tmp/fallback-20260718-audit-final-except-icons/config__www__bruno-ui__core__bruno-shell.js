...
class BrunoShell extends HTMLElement {
          ...
          <div class="config-menu-list">
            <button class="config-menu-item" type="button" data-config-action="open-section" data-section="themes">
              <span class="config-menu-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M12 3a9 9 0 0 0 0 18c1.3 0 2-1.4 1.2-2.4-.6-.8 0-2 1-2h2.1A4.7 4.7 0 0 0 21 12 9 9 0 0 0 12 3Z"/><circle cx="8" cy="9" r=".8"/><circle cx="11" cy="6.8" r=".8"/><circle cx="15" cy="8" r=".8"/></svg>
              </span>
            ...
            <button class="config-menu-item" type="button" data-config-action="open-section" data-section="wallpaper">
              <span class="config-menu-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="16" rx="2"/><circle cx="8.5" cy="9" r="1.5"/><path d="m4 17 5-5 3.5 3.5 2.5-2.5 5 5"/></svg>
              </span>
            ...
            <button class="config-menu-item" type="button" data-config-action="open-section" data-section="updates">
              <span class="config-menu-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><path d="m7 10 5 5 5-5M12 15V3"/></svg>
              </span>
  ...
  _renderThemesChild() {
    ...
    return `
      <section class="config-panel config-child-panel" role="dialog" aria-modal="true" aria-label="Themes">
        <header class="config-header">
          <span class="config-icon" aria-hidden="true">
            <svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"/><path d="M12 3a9 9 0 0 0 0 18c1.3 0 2-1.4 1.2-2.4-.6-.8 0-2 1-2h2.1A4.7 4.7 0 0 0 21 12 9 9 0 0 0 12 3Z"/></svg>
          </span>
      ...
      <section class="config-panel config-child-panel wallpaper-panel" role="dialog" aria-modal="true" aria-label="Wallpaper">
        ...
        <div class="config-section wallpaper-content">
          <label class="wallpaper-field">
            <span>Area</span>
            <select data-config-action="wallpaper-section">
              ${sections.map((section) => `<option value="${BrunoShell._escapeAttr(section.key)}" ${section.key === selected.key ? 'selected' : ''}>${BrunoShell._escape(section.label)}</option>`).join('')}
            </select>
          </label>
          <label class="wallpaper-field">
            <span>Imagem</span>
          ...
          <small class="wallpaper-help">Use /local/..., /api/... ou uma URL http(s). Salvar renova automaticamente a versao da imagem.</small>
  ...
  _handleConfigClick(event) {
    ...
    if (spotifyTarget) {
      ...
      event.stopPropagation();
      if (spotifyTarget.dataset.spotifyAction === 'close') this._closeConfigPanel();
      return;
    }
  ...
  _handleConfigChange(event) {
    const target = event.target?.closest?.('[data-config-action="wallpaper-section"]');
    if (!target) return;
    this._wallpaperSection = target.value || 'home';
    this._wallpaperMessage = '';
    this._renderConfigPanel();
  }
