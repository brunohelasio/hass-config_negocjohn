...
const BRUNO_QUARTO_MARINA_SUBVIEW_DEFAULT_CONFIG = {
  ...
  fallback_background: '/local/images/quarto_marina.jpg?v=20260702-all-images-1',
  ...
  entities: {
    ...
    ps5_image: '',
    lights: [
      { entity: 'light.quarto_marina_switch_4', name: 'Luz principal', icon_type: 'light_flush', zone: 'sala' },
      { entity: 'light.quarto_marina_switch_1', name: 'Arandela', icon_type: 'sconce', zone: 'sala' },
      { entity: 'light.quarto_marina_switch_2', name: 'Estante', icon_type: 'light_flush', zone: 'sala' },
      { entity: 'light.quarto_marina_switch_3', name: 'Luz cortineiro', icon_type: 'light_flush', zone: 'sala' },
...
const BRUNO_QUARTO_MARINA_SUBVIEW_CLIMATE_ON_STATES = ['cool', 'heat', 'fan_only', 'dry', 'heat_cool', 'auto'];
const BRUNO_QUARTO_MARINA_SUBVIEW_CLIMATE_ACTIVE_ACTIONS = ['cooling', 'heating', 'drying', 'fan', 'preheating'];
const BRUNO_QUARTO_MARINA_SUBVIEW_CLIMATE_INACTIVE_ACTIONS = ['off', 'idle'];
const BRUNO_QUARTO_MARINA_SUBVIEW_TV_ON_STATES = ['on', 'playing', 'paused', 'idle'];
const BRUNO_QUARTO_MARINA_SUBVIEW_MEDIA_ON_STATES = ['playing', 'paused', 'on', 'idle'];
const BRUNO_QUARTO_MARINA_SUBVIEW_CAMERA_ONLINE_STATES = ['streaming', 'recording', 'idle', 'on'];
const BRUNO_QUARTO_MARINA_SUBVIEW_UNAVAILABLE_STATES = ['unavailable', 'unknown', '', 'none', 'null'];
const BRUNO_QUARTO_MARINA_SUBVIEW_TV_ICON_ANIMATION_MS = 1000;

class BrunoQuartoMarinaSubview extends HTMLElement {
  ...
  _normalizeConfig(config) {
    const entities = {
      ...BRUNO_QUARTO_MARINA_SUBVIEW_DEFAULT_CONFIG.entities,
      ...(config.entities || {}),
    };
    ...
    return {
      ...BRUNO_QUARTO_MARINA_SUBVIEW_DEFAULT_CONFIG,
      ...config,
      refresh_interval: Math.max(4000, Number(config.refresh_interval) || BRUNO_QUARTO_MARINA_SUBVIEW_DEFAULT_CONFIG.refresh_interval),
      room_nav: Array.isArray(config.room_nav) ? config.room_nav : BRUNO_QUARTO_MARINA_SUBVIEW_DEFAULT_CONFIG.room_nav,
      tv_apps: Array.isArray(config.tv_apps) ? config.tv_apps : BRUNO_QUARTO_MARINA_SUBVIEW_DEFAULT_CONFIG.tv_apps,
      entities,
    };
  }
  ...
  _refreshCameraImages() {
    ...
    this.shadowRoot.querySelectorAll('img[data-camera-src-base]').forEach((image) => {
      ...
      const nextSrc = BrunoQuartoMarinaSubview._withCacheBust(baseSrc, stamp);
  ...
  _cameraState(camera) {
    ...
    const online = !unavailable && BRUNO_QUARTO_MARINA_SUBVIEW_CAMERA_ONLINE_STATES.includes(state);
    ...
    return {
      ...
      online,
      image,
      imageUrl: this._loadedCameraUrls[camera.entity] || BrunoQuartoMarinaSubview._withCacheBust(image, this._refreshSeed),
  ...
  _tvModel() {
    ...
    const active = BRUNO_QUARTO_MARINA_SUBVIEW_TV_ON_STATES.includes(state);
    return {
      entity,
      state,
      active,
  ...
  _climateEntityId() {
    const configured = this._config?.entities?.climate;
    const candidates = (Array.isArray(configured) ? configured : [configured])
      .filter(Boolean)
      .map((entityId) => String(entityId));
    return candidates.find((entityId) => this._state(entityId)) || candidates[0] || '';
  }
  ...
  _climateIsActive(entity) {
    ...
    if (BRUNO_QUARTO_MARINA_SUBVIEW_CLIMATE_ACTIVE_ACTIONS.includes(hvacAction)) return true;
    if (BRUNO_QUARTO_MARINA_SUBVIEW_CLIMATE_INACTIVE_ACTIONS.includes(hvacAction)) return false;
    return BRUNO_QUARTO_MARINA_SUBVIEW_CLIMATE_ON_STATES.includes(entity.state || '');
  }

  _climateModel() {
    const entityId = this._climateEntityId();
    const entity = this._state(entityId);
    const attrs = entity?.attributes || {};
  ...
  _setClimateTarget(temperature) {
    ...
    const entityId = model.entityId || this._climateEntityId();
    if (!entityId || this._isUnavailable(model.entity)) return;
...
if (action === 'toggle-lights-panel') {
    ...
    if (action === 'toggle-climate' && !this._cooldown('climate', 1800)) {
      const model = this._climateModel();
      if (!model.entityId || this._isUnavailable(model.entity)) return;
      this._selectedClimatePanel = '';
      this._callService(model.active ? 'climate.turn_off' : 'climate.turn_on', { entity_id: model.entityId });
    }
    if (action === 'climate-mode') {
      const hvacMode = target.dataset.mode;
      const climate = this._climateModel();
      if (hvacMode && climate.entityId && !this._isUnavailable(climate.entity)) {
        this._selectedClimatePanel = '';
    ...
    if (action === 'fan-mode') {
      ...
      const climate = this._climateModel();
      if (fanMode && climate.entityId && !this._isUnavailable(climate.entity)) {
        this._selectedClimatePanel = '';
    ...
    if (action === 'swing-mode') {
      ...
      const climate = this._climateModel();
      if (swingMode && climate.entityId && !this._isUnavailable(climate.entity)) {
        this._selectedClimatePanel = '';
      ...
      <div class="glass-card status-rail">
        ${status.map((item) => `
          <div class="status-item">
            ...
            <div>
              <strong>${BrunoQuartoMarinaSubview._escape(item.value)}</strong>
              <span>${BrunoQuartoMarinaSubview._escape(item.label)}</span>
            </div>
          </div>
        `).join('')}
      </div>
  ...
  _renderLightZoneRail(lights, selectedZone) {
    ...
    return `
      <aside
        ...
        data-zone="${BrunoQuartoMarinaSubview._escapeAttr(selectedZone)}"
        data-dimmer-entity="${BrunoQuartoMarinaSubview._escapeAttr(dimmerTarget?.entity || '')}"
        data-dimmer-level="${BrunoQuartoMarinaSubview._escapeAttr(String(fillPercent))}"
      ...
      >
        <span class="rail-zone">${BrunoQuartoMarinaSubview._escape(label)}</span>
  ...
  _renderLightTile(light) {
    ...
    return `
      <button type="button" class="light-tile${active ? ' is-on' : ''}${disabled ? ' is-placeholder' : ''}" ${disabled ? 'disabled' : `data-action="toggle-light" data-entity="${BrunoQuartoMarinaSubview._escapeAttr(light.entity)}"`}>
        <span class="light-icon">${BrunoQuartoMarinaSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <strong>${BrunoQuartoMarinaSubview._escape(light.name)}</strong>
    ...
    return `
      <section class="light-section">
        <div class="section-head">
          ...
          ${showZoneAction
            ? `<button type="button" class="zone-off" data-action="zone-off" data-zone="${zoneKey}">Apagar ${BrunoQuartoMarinaSubview._escape(zone.name.toLowerCase())}</button>`
            : ''}
        </div>
  ...
  _renderLightCell(light, index, isOdd) {
    ...
    return `
      <button type="button" class="${cls}"
        ...
        data-entity="${BrunoQuartoMarinaSubview._escapeAttr(light.entity)}"
        role="switch" aria-checked="${active ? 'true' : 'false'}"
        aria-label="${BrunoQuartoMarinaSubview._escapeAttr(light.name)}">
        <span class="lc-icon">${BrunoQuartoMarinaSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <span class="lc-name">${BrunoQuartoMarinaSubview._escape(light.name)}</span>
  ...
  _renderZoneTile(light, wide) {
    ...
    return `
      <button type="button"
        ...
        role="switch" aria-checked="${active ? 'true' : 'false'}"
        aria-label="${BrunoQuartoMarinaSubview._escapeAttr(light.name)}">
        <span class="zl-icon">${BrunoQuartoMarinaSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <span class="zl-switch" aria-hidden="true"><span class="zl-knob"></span></span>
        <span class="zl-name">${BrunoQuartoMarinaSubview._escape(light.name)}</span>
      </button>
    `;
    ...
    const controlMarkup = controls.map((control) => {
      ...
      return `
        <button
          ...
          title="${BrunoQuartoMarinaSubview._escapeAttr(ariaLabel)}"
        >
          <bruno-icon icon="${BrunoQuartoMarinaSubview._escapeAttr(control.icon || 'mdi:toggle-switch-outline')}"></bruno-icon>
  ...
  _cameraFrame(camera) {
    ...
    return `
      <div class="camera-row-image">
        ${image ? `<img src="${BrunoQuartoMarinaSubview._escapeAttr(image)}" data-camera-src-base="${BrunoQuartoMarinaSubview._escapeAttr(base)}" data-camera-entity="${BrunoQuartoMarinaSubview._escapeAttr(camera.entity)}" alt="">` : ''}
        <div class="camera-placeholder" aria-hidden="true"></div>
      </div>
    `;
  }
  ...
  _renderMediaHubLegacy(model) {
    ...
    if (tvStateChanged) {
      this._mediaTvAnimationUntil = now + BRUNO_QUARTO_MARINA_SUBVIEW_TV_ICON_ANIMATION_MS;
    ...
    const renderMeta = (value) => (
      value
        ? `<small>${BrunoQuartoMarinaSubview._escape(value)}</small>`
    ...
    const mediaActionButton = ({
      ...
      <button
        ...
        data-action="${BrunoQuartoMarinaSubview._escapeAttr(action)}"
        title="${BrunoQuartoMarinaSubview._escapeAttr(label)}"
        aria-label="${BrunoQuartoMarinaSubview._escapeAttr(label)}"
        ${attrs}
        ${disabled ? 'disabled' : ''}
      >
        <bruno-icon icon="${BrunoQuartoMarinaSubview._escapeAttr(icon)}"></bruno-icon>
      </button>
    `;
    const mediaIdentityCell = (type, active = false, options = {}) => `
      <span class="media-identity-cell is-${BrunoQuartoMarinaSubview._escapeAttr(type)}${active ? ' is-active' : ''}" aria-hidden="true">
        ${BrunoQuartoMarinaSubview._tplMediaIcon(type, { active, animate: Boolean(options.animate) })}
      </span>
    `;
    const standbyImage = (src, className, fallbackIcon) => (
      src
        ? `<img class="media-standby-image ${className}" src="${BrunoQuartoMarinaSubview._escapeAttr(src)}" alt="">`
        : `<bruno-icon icon="${BrunoQuartoMarinaSubview._escapeAttr(fallbackIcon)}"></bruno-icon>`
    );
    const tvAppButtons = (this._config.tv_apps || []).slice(0, 4).map((app) => {
      const label = app.label || 'App';
      const image = app.image ? BrunoQuartoMarinaSubview._escapeAttr(app.image) : '';
      const keyClass = app.key ? ` app-${BrunoQuartoMarinaSubview._escapeAttr(app.key)}` : '';
      return `
        <button
          type="button"
          class="media-action-button media-image-button${keyClass}"
          data-action="tv-app"
          data-script="${BrunoQuartoMarinaSubview._escapeAttr(app.script || '')}"
          title="${BrunoQuartoMarinaSubview._escapeAttr(label)}"
          aria-label="${BrunoQuartoMarinaSubview._escapeAttr(label)}"
        ...
        >
          ${image
            ...
            : `<bruno-icon icon="${BrunoQuartoMarinaSubview._escapeAttr(app.icon || 'mdi:apps')}"></bruno-icon>`}
        </button>
      `;
    }).join('');
    ...
    const sources = {
      tv: {
        ...
        visual: tvPoster
          ? `<img src="${BrunoQuartoMarinaSubview._escapeAttr(tvPoster)}" alt="">`
      ...
      spotify: {
        ...
        visual: spotifyArtwork
          ? `<img src="${BrunoQuartoMarinaSubview._escapeAttr(spotifyArtwork)}" alt="">`
    ...
    return `
      <section class="glass-card media-hub-card is-${BrunoQuartoMarinaSubview._escapeAttr(current.key)}">
          ...
          <div class="media-tabs" role="tablist" aria-label="Fonte de midia">
            ${tabs.map((source) => `
              ...
              >
                ...
                <span>${BrunoQuartoMarinaSubview._escape(source.label)}</span>
                <small>${BrunoQuartoMarinaSubview._escape(source.state)}</small>
              </button>
            `).join('')}
          </div>
        </div>
        ...
        <div class="media-hub-body">
          ...
          <div class="media-hub-content">
            <div class="media-details">
              <strong>${BrunoQuartoMarinaSubview._escape(current.title)}</strong>
              ${current.meta}
            </div>
  ...
  _renderTV(model) {
    const tv = model.tv;
    const poster = tv.poster ? BrunoQuartoMarinaSubview._resolvePicture(tv.poster) : '';
      ...
      <section class="glass-card tv-card${tv.active ? ' is-active' : ''}">
        ...
        <div class="tv-body">
          ...
          <div class="poster-card${poster ? ' has-poster' : ''}">
            ${poster ? `<img src="${BrunoQuartoMarinaSubview._escapeAttr(poster)}" alt="">` : '<span>Poster</span>'}
          </div>
        </div>
      </section>
    `;
        ...
        <div class="ps5-body ps5-minimal">
          ...
          <div class="ps5-footer">
            ...
            <div class="ps5-actions">
              <button type="button" class="primary-button" data-action="toggle-ps5" ${ps5.configured ? '' : 'disabled'}>${ps5.active ? 'Desligar' : 'Ligar'}</button>
              <button type="button" class="control-button" data-action="more-info" data-entity="${BrunoQuartoMarinaSubview._escapeAttr(ps5.entityId || '')}" ${ps5.configured ? '' : 'disabled'}><bruno-icon icon="mdi:dots-horizontal"></bruno-icon></button>
            </div>
          </div>
        </div>
      </section>
    ...
    return `
      <section class="glass-card spotify-card${spotify.active ? ' is-active' : ''}">
        <div class="module-head">
          ...
          <span class="state-chip"><span></span>${spotify.playing ? 'Playing' : BrunoQuartoMarinaSubview._escape(spotify.state)}</span>
        </div>

        <div class="spotify-body">
          <div class="spotify-art${artwork ? ' has-art' : ''}">
            ${artwork ? `<img src="${BrunoQuartoMarinaSubview._escapeAttr(artwork)}" alt="">` : '<bruno-icon icon="mdi:music-note"></bruno-icon>'}
          </div>
  ...
  _renderClimateRing(climate, target, current, minTarget, maxTarget, dialMode) {
    ...
    const labelMarkup = labels.map((label) => {
      ...
      return `<text x="${p.x.toFixed(3)}" y="${p.y.toFixed(3)}" text-anchor="middle" dominant-baseline="middle" class="icg-label ${label.cls}">${BrunoQuartoMarinaSubview._escape(label.text)}</text>`;
    }).join('');

    return `
      <div class="icg-root">
        <div class="icg-shell">
          <svg class="icg-svg" viewBox="0 0 720 460" role="img" aria-label="${BrunoQuartoMarinaSubview._escapeAttr(`Temperatura alvo ${targetLabel}°. Ambiente ${ambientLabel}°.`)}">
            <defs>
            ...
            <text x="${cx}" y="260" text-anchor="middle" dominant-baseline="middle" class="icg-center-mode">${BrunoQuartoMarinaSubview._escape(modeLabel)}</text>
            <text x="${cx}" y="328" text-anchor="middle" dominant-baseline="middle" class="icg-center-temp">${BrunoQuartoMarinaSubview._escape(targetLabel)}°</text>
            ...
            <text x="${cx}" y="432" text-anchor="middle" dominant-baseline="middle" class="icg-ambient">Ambient ${BrunoQuartoMarinaSubview._escape(ambientLabel)}°</text>
          </svg>
        </div>
      </div>
    `;
  ...
  _renderAC(model) {
    ...
    const climateEntity = BrunoQuartoMarinaSubview._escapeAttr(climate.entityId || '');
  ...
  _styles() {
    return `
      ...
      */
      ...
      /* ANTERIOR (rollback): .zone-lights { display: flex; flex-direction: column; padding: 0 6px 6px; } */
  ...
  static _escapeAttr(value) {
    return BrunoQuartoMarinaSubview._escape(value).replace(/'/g, '&#39;');
  }
}

if (!customElements.get(BRUNO_QUARTO_MARINA_SUBVIEW_TAG)) {
  customElements.define(BRUNO_QUARTO_MARINA_SUBVIEW_TAG, BrunoQuartoMarinaSubview);
}
