...
const BRUNO_OFFICE_SUBVIEW_DEFAULT_CONFIG = {
  ...
  fallback_background: '/local/images/office.jpg?v=20260702-all-images-1',
  ...
  pc_image: '/local/images/office_pc.png?v=20260702-all-images-1',
  ...
  entities: {
    ...
    lights: [
      ...
      { entity: 'light.office_switch_2', name: 'Luz ambiente', icon_type: 'light_flush', zone: 'office' },
...
_mediaSourceFromAction(action) {
  ...
  _selectedMedia(model) {
    const pcActive = Boolean(model.pc?.active);
    const spotifyActive = Boolean(model.spotify?.active);
    const activeKeys = [pcActive ? 'pc' : '', spotifyActive ? 'spotify' : ''].filter(Boolean);
    const previous = this._lastActiveMediaKeys || [];
    const spotifyJustActivated = spotifyActive && !previous.includes('spotify');
    this._lastActiveMediaKeys = activeKeys;

    if (spotifyJustActivated) this._selectedMediaSource = 'spotify';
    if (!pcActive && this._selectedMediaSource === 'pc' && spotifyActive) {
      this._selectedMediaSource = 'spotify';
    }

    if (['pc', 'spotify'].includes(this._selectedMediaSource)) return this._selectedMediaSource;
    if (spotifyActive) return 'spotify';
    return 'pc';
  }
    ...
    return `
      <section class="light-section">
        <div class="section-head">
          ...
          ${showZoneAction
            ? `<button type="button" class="zone-off" data-action="zone-off" data-zone="${zoneKey}">Apagar ${BrunoOfficeSubview._escape(zone.name.toLowerCase())}</button>`
            : ''}
        </div>
  ...
  _renderLightCell(light, index, isOdd) {
    ...
    return `
      <button type="button" class="${cls}"
        ...
        data-entity="${BrunoOfficeSubview._escapeAttr(light.entity)}"
        role="switch" aria-checked="${active ? 'true' : 'false'}"
        aria-label="${BrunoOfficeSubview._escapeAttr(light.name)}">
        <span class="lc-icon">${BrunoOfficeSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <span class="lc-name">${BrunoOfficeSubview._escape(light.name)}</span>
  ...
  _renderZoneTile(light, wide) {
    ...
    return `
      <button type="button"
        ...
        role="switch" aria-checked="${active ? 'true' : 'false'}"
        aria-label="${BrunoOfficeSubview._escapeAttr(light.name)}">
        <span class="zl-icon">${BrunoOfficeSubview._tplLightIcon(light.icon_type || light.icon, active)}</span>
        <span class="zl-switch" aria-hidden="true"><span class="zl-knob"></span></span>
        <span class="zl-name">${BrunoOfficeSubview._escape(light.name)}</span>
      </button>
    `;
    ...
    const controlMarkup = controls.map((control) => {
      ...
      return `
        <button
          ...
          title="${BrunoOfficeSubview._escapeAttr(ariaLabel)}"
        >
          <bruno-icon icon="${BrunoOfficeSubview._escapeAttr(control.icon || 'mdi:toggle-switch-outline')}"></bruno-icon>
      ...
      <section class="glass-card spotify-card${spotify.active ? ' is-active' : ''}">
        ...
        <div class="spotify-body">
          <div class="spotify-art${artwork ? ' has-art' : ''}">
            ${artwork ? `<img src="${BrunoOfficeSubview._escapeAttr(artwork)}" alt="">` : '<bruno-icon icon="mdi:music-note"></bruno-icon>'}
          </div>
  ...
  _renderAC(model) {
    ...
    const renderOption = ({ action, mode, label, icon, active }) => `
      <button
        ...
        role="menuitem"
      >
        <bruno-icon icon="${BrunoOfficeSubview._escapeAttr(icon)}"></bruno-icon>
    ...
    return `
      ...
      */
      ...
      /* NOVO (grade 2-col): mesma grade da Sala. 3 luzes (ímpar) => a 1ª (Luz
         central) ocupa a linha inteira; as 2 restantes formam um par. Altura
