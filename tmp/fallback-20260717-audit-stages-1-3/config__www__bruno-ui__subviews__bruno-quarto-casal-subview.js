...
const BRUNO_QUARTO_CASAL_CURTAIN_CALIBRATION = [
  { visual: 0, position: 0 },
...
const BRUNO_QUARTO_CASAL_CURTAIN_PRESETS = [
...
const BRUNO_QUARTO_CASAL_CURTAIN_TRAVEL_MS = 30000;
const BRUNO_QUARTO_CASAL_CURTAIN_MIN_MOTION_MS = 1200;
const BRUNO_QUARTO_CASAL_CURTAIN_MOTION_TICK_MS = 350;
const BRUNO_QUARTO_CASAL_CURTAIN_TARGET_TOLERANCE = 2;
...
class BrunoQuartoCasalSubview extends HTMLElement {
  ...
  _interpolateCurtainPercent(value, fromKey, toKey) {
    ...
    const points = BRUNO_QUARTO_CASAL_CURTAIN_CALIBRATION;
  ...
  _curtainMotionDuration(startClosed, targetClosed) {
    ...
    return Math.max(BRUNO_QUARTO_CASAL_CURTAIN_MIN_MOTION_MS, BRUNO_QUARTO_CASAL_CURTAIN_TRAVEL_MS * (distance / 100));
  }
  ...
  _curtainMotionClosedPosition(motion = this._curtainMotion, now = Date.now()) {
    ...
    const duration = Math.max(1, Number(motion.duration) || BRUNO_QUARTO_CASAL_CURTAIN_MIN_MOTION_MS);
  ...
  _curtainMotionDisplayPosition(entityId, state, reportedClosed) {
    ...
    const reportedAtTarget = reported != null && Math.abs(reported - motion.targetClosed) <= BRUNO_QUARTO_CASAL_CURTAIN_TARGET_TOLERANCE;
  ...
  _startCurtainMotionTimer() {
    ...
    this._curtainMotionTimer = globalThis.setInterval(() => {
      ...
      this._safeRender();
    }, BRUNO_QUARTO_CASAL_CURTAIN_MOTION_TICK_MS);
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <main class="quarto-casal-subview">
  ...
  _renderCurtain(model) {
    ...
    const curtainMarks = BRUNO_QUARTO_CASAL_CURTAIN_PRESETS.map((preset) => `
      <button
        type="button"
      ...
      <div class="curtain-dock curtain-overlay${curtain.available ? '' : ' is-disabled'}" style="--curtain-position: ${curtainVisualPosition}%;">
          <div class="curtain-control-row">
            ...
            <div class="curtain-status" aria-live="polite">
              <span class="curtain-status-text">${BrunoQuartoCasalSubview._escape(curtain.status)}</span>
    ...
    return `
      <header class="subview-topband">
        ...
        <div class="topband-clock" aria-label="Data e hora">
          ...
          <small>${BrunoQuartoCasalSubview._escape(this._dateLine())}</small>
        </div>
      </header>
    `;
  }
  ...
  _renderFrameTop() {
    return `
      <header class="subview-topbar">
        <span class="subview-room">${BrunoQuartoCasalSubview._escape(this._config.title)}</span>
  ...
  _renderRoomSidebar() {
    ...
    const homeButton = `
      <button
        ...
        data-path="${BrunoQuartoCasalSubview._escapeAttr(this._config.navigation_path)}"
        title="Home"
        aria-label="Home"
      >
        ${BrunoQuartoCasalSubview._roomNavIcon('home')}
      ...
      <nav class="room-sidebar" aria-label="Navegacao de comodos">
        ...
        ${items.map((item) => `
          ...
          >
            ...
            <span class="room-nav-label">${BrunoQuartoCasalSubview._escape(item.name)}</span>
          </button>
        `).join('')}
      </nav>
    `;
  ...
  _renderCameraFeed(camera, options = {}) {
    ...
    const content = `
      ...
      <div class="camera-row-copy">
        <strong>${BrunoQuartoCasalSubview._escape(cameraName)}</strong>
      </div>
    `;
    ...
    if (pip && camera?.entity) {
      return `
        <button
          ...
          data-entity="${BrunoQuartoCasalSubview._escapeAttr(camera.entity)}"
          aria-label="Mostrar câmera ${BrunoQuartoCasalSubview._escapeAttr(cameraName)}"
        >
          ${content}
        </button>
      `;
    ...
    return `<div class="${classes}" aria-label="Câmera ${BrunoQuartoCasalSubview._escapeAttr(cameraName)}">${content}</div>`;
  }
  ...
  _renderCameraControls(camera) {
    ...
    const controlMarkup = controls.map((control) => {
      ...
      return `
        <button
          ...
          ${control.unavailable ? 'disabled' : `data-action="toggle-camera-control" data-entity="${BrunoQuartoCasalSubview._escapeAttr(control.entity)}"`}
    ...
    return `
      <div class="camera-control-strip" aria-label="Controles da câmera ${BrunoQuartoCasalSubview._escapeAttr(cameraName)}">
  ...
  _cameraFrame(camera) {
    ...
    return `
      <div class="camera-row-image">
        ...
        ${camera?.entity ? `<div class="camera-live" data-camera-live-mount="${BrunoQuartoCasalSubview._escapeAttr(camera.entity)}"></div>` : ''}
        <div class="camera-placeholder" aria-hidden="true"></div>
      </div>
    `;
  }
  ...
  _renderMediaHub(model) {
    ...
    const ps5 = model.ps5;
    const esc = (v) => BrunoQuartoCasalSubview._escape(v);
    const escA = (v) => BrunoQuartoCasalSubview._escapeAttr(v);
      ...
      ps5: {
        ...
        primaryActions: `
          ...
          ${mediaActionButton({
            ...
            label: 'Mais detalhes',
            attrs: `data-entity="${BrunoQuartoCasalSubview._escapeAttr(ps5.entityId || '')}"`,
  ...
  _renderPS5(model) {
    const ps5 = model.ps5;
    const image = ps5.image ? BrunoQuartoCasalSubview._escapeAttr(ps5.image) : '';
  ...
  _renderAC(model) {
    ...
    const climateEntity = BrunoQuartoCasalSubview._escapeAttr(this._config.entities.climate || '');
  ...
  _styles() {
    return `
      :host {
        --qcasal-gap: 10px;
        --qcasal-radius: var(--bruno-liquid-card-radius, 18px);
        --qcasal-radius-small: var(--bruno-liquid-card-radius-compact, 16px);
        --qcasal-cell-radius: var(--bruno-liquid-cell-radius, 16px);
      ...
      * {
      ...
      .side-panel { grid-area: side; min-width: 0; min-height: 0; display: grid; grid-template-rows: 72px minmax(0, 1fr); gap: var(--qcasal-gap); }
      ...
      .glass-card {
        ...
        min-width: 0;
        min-height: 0;
        border-radius: var(--qcasal-radius);
        overflow: hidden;
        color: var(--text-main);
      ...
      .glass-card::before {
        ...
        z-index: 0;
        pointer-events: none;
        border-radius: calc(var(--qcasal-radius) - 1px);
      ...
      .light-tile {
        ...
        padding: 13px 16px;
        text-align: left;
        border-radius: var(--qcasal-cell-radius);
        color: rgba(255,255,255,0.86);
      ...
      .lights-zone-rail::before {
        ...
        inset: 1px;
        pointer-events: none;
        border-radius: calc(var(--qcasal-cell-radius) - 1px);
        background:
      ...
      */
      ...
      .climate-trend {
        ...
        border-radius: 0 0 calc(var(--qcasal-radius) - 1px) calc(var(--qcasal-radius) - 1px);
        overflow: hidden;
        background: transparent;
      }
...
window.customCards.push({
  ...
  name: 'Bruno Q. Casal Subview',
  preview: false,
  description: 'Q. Casal subview rebuilt as an isolated Bruno Liquid Glass Web Component.',
});
