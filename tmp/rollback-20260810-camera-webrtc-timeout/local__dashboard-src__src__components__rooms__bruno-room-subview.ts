...
const PRAZO_MODO_AO_VIVO = 10000;
...
export class BrunoRoomSubview extends LitElement {
  ...
  private _cuidarDoAoVivo(entityPalco: string | undefined): void {
    ...
    if (!this._liveEl || this._liveEntity !== alvo) {
      ...
      if (!el) {
        // Sem o componente registrado, nao cair em HLS: a foto continua.
        this._aoVivoSuspenso = true;
        return;
      }
      this._liveEl = el;
      ...
      // connectedCallback. Primeiro conecta; somente depois muda entityid e
      // dispara a negociacao. O patch anterior fazia na ordem inversa.
      slot.appendChild(el);
      el.entityid = alvo;
  ...
  private _criarPlayer(): PlayerAoVivo | undefined {
    if (customElements.get('ha-web-rtc-player')) {
      const el = document.createElement('ha-web-rtc-player') as PlayerAoVivo;
  ...
  private _falharAoVivo(motivo: 'sem video' | 'prazo'): void {
    const entityId = this._liveEntity;
    if (!entityId) return;
    const nome = entityId.split('.')[1] ?? entityId;
    requisicaoManual(
  ...
  private _maisInfo(entityId: string | undefined): void {
    ...
    if (usaWebRtc(entityId)) {
      // O more-info precisa receber a unica sessao da camera. O tile volta para
      // instantaneos ate o usuario sair e entrar novamente no comodo.
      this._aoVivoSuspenso = true;
      this._pararAoVivo();
      this._sincronizarCameras();
    }
