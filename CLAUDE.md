...
| M3 | Sala 2x1 horizontal real | ✅ IMPLEMENTADO 2026-05-10 — mobile_sala_card.yaml reescrito do zero |
...
| M5 | Top badges scroll | ✅ IMPLEMENTADO 2026-05-10 — mobile_top_badges reescrito sem shadow DOM hack |
...
## Registro de Implementacao — Mobile V3 Revisao (2026-05-10)

### Contexto

Primeiro teste visual da V3 revelou 5 problemas que bloqueavam a experiencia mobile.
O usuario acessou `/ngocjohn-main/mobile-casa` manualmente (alterando URL no desktop)
e identificou os defeitos. Todos corrigidos nesta sessao.

### Problemas corrigidos e causa-raiz

#### F1 — Slug errado na pill navbar

**Sintoma:** Botoes da navbar navegavam para `/lovelace/mobile-*` (dashboard padrao do HA).
**Causa:** Slug confirmado anteriormente como "lovelace", mas URL real e `ngocjohn-main`.
**Arquivo:** `shared/mobile/mobile_pill_nav.yaml`
**Fix:** Substituicao de `/lovelace/` por `/ngocjohn-main/` nos 5 `navigation_path`.
**Rollback:** Reverter os 5 `navigation_path` para `/lovelace/mobile-*`.

---

#### F2 — Scroll lateral global na view

**Sintoma:** Toda a view (exceto navbar fixa) fazia scroll horizontal.
**Causa:** `min-width: max-content` do `mobile_top_badges` vazava para o container pai
da view porque o seletor Shadow DOM `hui-card$` nao alcancava o elemento correto
(`custom:layout-card` renderiza como `hui-layout-card`, nao `hui-card`).
**Arquivo:** `views/mobile/mobile-casa.yaml`
**Fix:** `overflow-x: hidden; max-width: 100vw` adicionados ao `hui-grid-layout` via card_mod.
**Rollback:** Remover as duas linhas adicionadas no card_mod da view.

---

#### F3 — mobile_top_badges com shadow DOM hack quebrado

**Sintoma:** Badges nao scrollavam internamente (o scroll era global, ver F2).
**Causa:** O seletor `hui-card$` nao atingia `hui-grid-layout` dentro de `custom:layout-card`.
O `bento_top_badges.yaml` desktop usa `minmax(0, 1fr)` que contrai o conteudo em vez de
deixar transbordar.
**Arquivo:** `shared/mobile/mobile_top_badges.yaml`
**Fix:** Arquivo reescrito do zero. Novo container: `custom:mod-card` com `overflow-x: auto`
no `ha-card` + `custom:layout-card` filho com `grid-auto-flow: column; grid-auto-columns: max-content`.
4 badges inline (Security, Lights, Media, Climate) sem incluir o desktop `bento_top_badges.yaml`.
**Rollback:** Comentar bloco `# NOVO` e descomentar bloco `# ANTERIOR` no proprio arquivo.

**Entidades monitoradas:**
- Security: `lock.nuki_neklanka_byt_lock`
- Lights: `light.grupo_luzes_sala`, `light.grupo_luzes_quarto_casal`, `light.grupo_luzes_quarto_marina`, `light.grupo_luzes_quarto_miguel`, `light.corredor_switch_1`
- Media: `media_player.android_tv_192_168_3_17`, `media_player.spotifyplus_bruno_helasio`, `media_player.echo_show`
- Climate: `climate.sl_ar_condicionado`, `climate.ac_office`, `climate.ac_quarto_miguel`

---

#### F4 — Sala em formato vertical (bento_sala.yaml incompativel com 2x1 horizontal)

**Sintoma:** Card da Sala no mobile era vertical (hero em cima, strip TV/AC em baixo),
ocupando mais espaco que o desejado e nao seguindo o formato 2x1 horizontal.
**Causa:** `mobile_sala_card.yaml` incluia `bento_sala.yaml` via `stack-in-card`. O
`bento_sala.yaml` tem `grid-template-rows: 1fr auto` (vertical). Tambem causava o aviso
"You are doing it wrong!" do card-mod por empilhar `mod-card` dentro de `stack-in-card`.
**Arquivo:** `shared/mobile/mobile_sala_card.yaml`
**Fix:** Arquivo reescrito do zero como `custom:layout-card` com `grid-template-columns: 1fr 140px`:
- Coluna esquerda (`info`): button-card com SVG da sala, nome, temp/umidade, contador de luzes, dots de status.
- Coluna direita (`controls`): `vertical-stack` com 4 pill buttons (Corredor / TV / A/C / PS5).
Toda a logica foi extraida diretamente do `bento_sala.yaml` — sem duplicacao de entidades.
**Rollback:** Comentar bloco `NOVO` e descomentar bloco `ANTERIOR` no proprio arquivo.

---

#### F5 — mod-card duplo no rail de comodos (warnings + estilos quebrados)

**Sintoma:** Console mostrava "You are doing it wrong! mod-card should NEVER be used
with a card that already has a ha-card element, such as stack-in-card". Glass dos cards
nao era aplicado corretamente.
**Causa:** `mobile_rooms_rail.yaml` envolvia cada `mobile_card_<comodo>.yaml`
(stack-in-card) num `mod-card` extra para aplicar scroll-snap. card-mod nao consegue
estilizar o stack-in-card quando envolto por mod-card.
**Arquivos alterados:**
- `shared/mobile/mobile_rooms_rail.yaml`: removidos wrappers `mod-card` externos. Cards
  incluidos diretamente no `custom:layout-card` filho.
- `shared/mobile/mobile_card_office.yaml`, `mobile_card_cozinha.yaml`,
  `mobile_card_lavabo.yaml`, `mobile_card_quarto_casal.yaml`,
  `mobile_card_quarto_marina.yaml`, `mobile_card_quarto_miguel.yaml`:
  adicionados `scroll-snap-align: start; min-width: calc(50vw - 22px); width: calc(50vw - 22px)`
  diretamente no `:host` do `stack-in-card`.
**Rollback:** Descomentar bloco `ANTERIOR` em `mobile_rooms_rail.yaml` e remover as 3 linhas
de scroll-snap do `:host` em cada `mobile_card_*.yaml`.

---

#### F6 — Auto-redirect nao disparava na bento-lab

**Sintoma:** Usuario precisava mudar URL manualmente. O card `zz-mobile-redirect.yaml`
existia mas nao estava incluido em `bento_main.yaml`.
**Arquivo:** `views/bento_main.yaml`
**Fix:** Adicionado `- !include main-grid/zz-mobile-redirect.yaml` como primeiro card.
O JS do redirect usa `parts[0]` da URL para extrair o slug dinamicamente — funciona
corretamente com `ngocjohn-main`.
**Rollback:** Comentar a linha `!include main-grid/zz-mobile-redirect.yaml`.

### Resumo dos arquivos alterados

| Arquivo | Tipo de alteracao |
|---------|------------------|
| `shared/mobile/mobile_pill_nav.yaml` | 5 `navigation_path` corrigidos |
| `views/mobile/mobile-casa.yaml` | overflow-x: hidden + max-width: 100vw na view |
| `shared/mobile/mobile_top_badges.yaml` | Reescrito (standalone, sem shadow DOM hack) |
| `shared/mobile/mobile_sala_card.yaml` | Reescrito (2x1 horizontal, standalone) |
| `shared/mobile/mobile_rooms_rail.yaml` | Removidos wrappers mod-card externos |
| `shared/mobile/mobile_card_office.yaml` | scroll-snap adicionado ao :host |
| `shared/mobile/mobile_card_cozinha.yaml` | scroll-snap adicionado ao :host |
| `shared/mobile/mobile_card_lavabo.yaml` | scroll-snap adicionado ao :host |
| `shared/mobile/mobile_card_quarto_casal.yaml` | scroll-snap adicionado ao :host |
| `shared/mobile/mobile_card_quarto_marina.yaml` | scroll-snap adicionado ao :host |
| `shared/mobile/mobile_card_quarto_miguel.yaml` | scroll-snap adicionado ao :host |
| `views/bento_main.yaml` | !include zz-mobile-redirect.yaml adicionado |

### Comportamento esperado apos esta revisao

- Ao abrir `bento-lab` em viewport <= 800px, redireciona automaticamente para `/ngocjohn-main/mobile-casa`.
- View mobile sem scroll horizontal global — apenas badges e rail de comodos scrollam internamente.
- Sala exibida em layout 2x1 horizontal (info esquerda, controles direita).
- Pill navbar com slugs corretos — todos os 5 botoes navegam para o dashboard certo.
- Cards dos comodos no rail renderizam com glass correto (sem mod-card duplo).
- O que fica fixo na tela: hero, sala, quick actions, pill navbar.
- O que tem scroll interno: badges (horizontal), rail de comodos (horizontal).
