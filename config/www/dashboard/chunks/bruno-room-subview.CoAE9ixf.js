import { i as w, a as sa, M as la, C as ca, r as O, e as f, b as k, O as G, p as da, m as R, c as pa, s as ma, R as ha, d as Y, f as ua, g as ga, h as ba, o as I, j as xa, k as N, l as fa, n as va, q as wa, A as x, t as l, u as Q, v as _a, w as ya, x as qa, y as ka, z as za, B as Aa, D as Ca, E as F } from "./main.uiioeD72.js";
const Sa = {
  sala: {
    title: "Sala",
...
class j extends sa {
  ...
  _alvoEcoDiferente(a) {
    return !this._alvoOtimista || this._alvoAntesDoPedido == null ? !1 : Math.abs(a - this._alvoAntesDoPedido) > 0.05;
  }
  setConfig(a) {
    ...
    return l`
      ...
      (n) => l`
              <div
                ...
                data-phone-hide=${n.ocultarNoTelefone ? "" : x}
                style="--tone: ${n.tone};"
              >
      ...
      return l`
        <section class="light-section">
          ...
          <div class="light-grid">
            ${s.luzes.map((m, c) => this._renderCelulaDeLuz(m, c, d))}
          </div>
        </section>
      `;
    });
  ...
  _renderFeed(a, o) {
    const e = a?.shortName || a?.name || "Câmera", t = !!this._controleCamera(a, "privacy")?.ativo, r = !a || a.indisponivel, n = [
      ...
      c.classList.add("is-loaded"), c.closest(".camera-main")?.classList.add("has-loaded-image"), this._marcarQuadroNaTela(a.entity);
    }}
              @error=${(m) => {
      const c = m.currentTarget;
      c.classList.remove("is-loaded"), c.closest(".camera-main")?.classList.remove("has-loaded-image"), this._socorrerCamera(a.entity);
    }}
            />` : x}
    ...
    return l`
      ...
      this._controlesCameraAbertos = !this._controlesCameraAbertos, this.requestUpdate();
        ...
        <div class="camera-stage camera-pip-stage ${r ? "is-controls-open" : ""}">
          ...
          ${t ? this._renderFeed(t, !0) : x}
          ${r ? this._renderControlesCamera(i) : x}
        </div>
      </div>
    `;
  }
  ...
  _corpoTv() {
    ...
    const n = Array.isArray(this._sub?.tvApps) ? this._sub.tvApps : [], p = this._appsTvAbertos && n.length ? l`<div class="mh-btn-row mh-btn-row-5">
      ...
      }, { soIcone: !0, desabilitado: !s.script, imagem: s.image ?? "" })
    )}
    ...
    return l`
      <div class="mh-left">
        <div class="mh-info">
          <small>Ligada</small>${r ? l`<em>${r}</em>` : x}
        </div>
  ...
  _corpoSpotify() {
    ...
    return l`
      <div class="mh-left">
        <div class="mh-info">
          <small>${a.titulo}</small>${a.artista ? l`<em>${a.artista}</em>` : x}
  ...
  _renderMediaHub() {
    ...
    ], n = Object.fromEntries(r.map((c) => [c.chave, c.ativo])), p = !a && r.some((c) => c.tocando) ? Object.fromEntries(r.map((c) => [c.chave, !!c.tocando])) : n, s = this._fonteAberta(r.map((c) => c.chave), p), d = !!r.find((c) => c.chave === s)?.tocando, m = [
      "glass-card",
      "media-hub-card",
    ...
    return l`
      ...
      this._menuMidiaAberto = !this._menuMidiaAberto, this.requestUpdate();
        ...
        ${this._menuMidiaAberto ? this._renderMenuMidia() : x}
        <div class="mh-sources">
          ${r.map((c) => {
      const u = c.chave === s, b = ["mh-source", u ? "is-open" : "", c.ativo ? "is-active" : ""].filter(Boolean).join(" "), _ = [
        "mh-source-body",
        `mh-source-body-${c.chave}`,
        c.ativo ? "is-source-active" : "is-source-idle",
        c.atmosfera ? "has-atmosphere" : ""
      ].filter(Boolean).join(" ");
      return l`
              <div class=${b}>
                <button
                  ...
                  @click=${() => this._selecionarFonteMidia(c.chave)}
                >
                  <bruno-icon
                    class="mh-src-icon ${c.chave === "spotify" ? "mh-icon-spotify" : ""}"
                    icon=${c.icone}
                  ></bruno-icon>
                  <span class="mh-src-name">${c.rotulo}</span>
                  <span class="mh-src-summary">${c.resumo}</span>
                  ${u ? x : l`<bruno-icon class="mh-src-chevron" icon="mdi:chevron-right"></bruno-icon>`}
                </button>
                ${u ? l`<div class=${_}>
                      ${this._estaNoTelefone() && c.atmosfera ? l`<img class="mh-now-atmosphere" src=${c.atmosfera} alt="" aria-hidden="true" />` : x}
                      ${c.corpo()}
                    </div>` : x}
              </div>
            `;
      ...
      return l`
          <article class=${_}>
            <button
              ...
              aria-label=${i}
              ?disabled=${u}
              @click=${() => !u && this._alternarAparelho(r)}
            >
              <div class="appliance-visual" data-image-wrapper>
                ${t ? l`<img src=${t} alt="" loading="lazy" decoding="async" />` : x}
              </div>
      ...
      <div class="icg-root">
        <div class="icg-shell">
          ...
          >
            ...
            <path d=${_(330, -180, 0)} class="icg-track-shadow"></path>
    ...
    return l`
      <div class="climate-stepper" role="group" aria-label="Temperatura alvo">
        <button
          ...
          ?disabled=${r || a.alvo != null && a.alvo <= a.minima}
          @click=${() => n(-1)}
        >
          −
        </button>
        ...
        <button
          ...
          ?disabled=${r || a.alvo != null && a.alvo >= a.maxima}
          @click=${() => n(1)}
        >
          +
        </button>
    ...
    const a = this._entidadeClimate(), o = this._modeloClimate(), e = o.swing.toLowerCase(), i = ["on", "ativo", "ativada", "enabled"].includes(e) || e.includes("ativ") && !e.includes("desativ"), t = (d) => [...new Set(d.filter(Boolean))], r = this._painelClima, n = {
      ...
      return m.length ? l`<div class="ac-popover" role="menu">
        ...
        (c) => l`
            <button
              type="button"
              class="ac-popover-option ${c.ativo ? "is-active" : ""}"
              role="menuitem"
              @click=${() => {
          this._painelClima = "", a && this._servico("climate", c.servico, { entity_id: a, [c.campo]: c.modo }), this.requestUpdate();
        }}
            >
              <bruno-icon icon=${c.icone}></bruno-icon><span>${c.rotulo}</span>
            </button>
          `
      )}
      ...
      this._painelClima = this._painelClima === d ? "" : d, this.requestUpdate();
        ...
        >
          ...
          <span class="ac-action-text"><small>${c}</small><strong>${u}</strong></span>
        </button>
        ${p(d)}
      </div>
    `;
...
customElements.get("bruno-room-subview") || customElements.define("bruno-room-subview", j);
const D = window;
D.customCards = D.customCards ?? [];
D.customCards.some((g) => g.type === "bruno-room-subview") || D.customCards.push({
...
//# sourceMappingURL=bruno-room-subview.CoAE9ixf.js.map
