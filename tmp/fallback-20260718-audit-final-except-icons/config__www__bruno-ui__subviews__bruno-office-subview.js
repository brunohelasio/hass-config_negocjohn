...
_mediaSourceFromAction(action) {
  ...
  _selectedMedia(model) {
    ...
    if (!spotifyActive && this._selectedMediaSource === 'spotify') {
      this._selectedMediaSource = pcActive ? 'pc' : '';
    }
  ...
  _cameraFrame(camera) {
    ...
    return `
      <div class="camera-row-image">
        ...
        ${camera?.entity ? `<div class="camera-live" data-camera-live-mount="${BrunoOfficeSubview._escapeAttr(camera.entity)}"></div>` : ''}
        <div class="camera-placeholder" aria-hidden="true"></div>
      </div>
    `;
  }
