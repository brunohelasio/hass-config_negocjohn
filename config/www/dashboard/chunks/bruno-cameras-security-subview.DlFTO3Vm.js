const u = "bruno-cameras-security-subview", m = {
...
}, f = ["streaming", "recording", "idle", "on"], h = ["unavailable", "unknown", ""], v = /* @__PURE__ */ new Set([
...
class o extends HTMLElement {
  ...
  constructor() {
    super(), this._refreshSeed = Date.now(), this._lastCameraImages = {}, this._lastClock = o._clock(), this._boundClick = (e) => this._handleClick(e), this._boundKeydown = (e) => this._handleKeydown(e), this._boundClock = () => this._updateClock(), this._liveEl = null, this._liveEntity = "", this._liveReady = "", this._liveTimer = null, this._mobileLiveWatchTimer = null, this._mobileLiveQuarantined = "", this._liveState = "idle", this._liveBlockedEntity = "", this._liveLoadToken = 0, this._cameraMenuOpen = !1, this._boundDialogClosed = (e) => this._handleDialogClosed(e);
  }
  connectedCallback() {
  ...
  _normalizeConfig(e = {}) {
    return {
      ...m,
      ...e,
      refresh_interval: Number(e.refresh_interval) > 0 ? Number(e.refresh_interval) : m.refresh_interval,
      cameras: Array.isArray(e.cameras) && e.cameras.length ? e.cameras : m.cameras
    };
  }
  _state(e) {
  ...
  _cameraState(e) {
    const a = this._state(e.entity), i = a?.state || "", t = !a || h.includes(i), r = !t && f.includes(i), s = a?.attributes?.entity_picture || "";
  ...
  _cameraGroup(e) {
    return e?.group === "social" || e?.group === "intimate" ? e.group : v.has(e?.entity) ? "social" : (x.has(e?.entity), "intimate");
  }
  _motionEvent(e) {
  ...
  _motionCount(e) {
    ...
    return (e || []).filter((i) => {
      ...
      return t && a - t.timestamp >= 0 && a - t.timestamp <= _;
    }).length;
  }
  _controlState(e, a) {
  ...
  _model() {
    const e = this._config || m, a = e.cameras.map((l) => this._cameraState(l)), i = this._state(e.active_entity)?.state, t = a[0]?.entity || "", r = this._localActiveCamera || (a.some((l) => l.entity === i) ? i : t), s = a.find((l) => l.entity === r) || a[0], n = a.filter((l) => l.entity !== s?.entity);
    return {
      activeCamera: s,
      activeId: r,
  ...
  _swapActive(e) {
    ...
    if (l && (l.textContent = o._displayName(n, !1, !0)), this._cameraMenuOpen) {
      ...
      const p = a.querySelector(".main-feed .feed-head-menu");
      p && (p.classList.remove("is-active"), p.setAttribute("aria-expanded", "false"));
    }
    ...
    const b = !!a.querySelector(".security-subview.is-desktop, .security-subview.is-mobile-v2");
    return r.setAttribute(
      "aria-label",
      o._displayName(t, !0, b)
    ), this._syncDesktopGroups(), this._renderedSignature = o._signatureFromModel(this._model()), !0;
  }
  _syncDesktopGroups() {
    const e = this.shadowRoot;
    if (!e?.querySelector(".security-subview.is-desktop, .security-subview.is-mobile-v2")) return;
    const a = this._model();
    [
  ...
  _replaceCameraControls(e) {
    ...
    a && (a.innerHTML = this._cameraControls(e), this._marcarAusenciaDeControles(e));
  }
  /**
   ...
   */
  _marcarAusenciaDeControles(e) {
    const a = this.shadowRoot?.querySelector(".security-subview");
    a && a.classList.toggle("sem-controles", !this._cameraControls(e));
  }
  ...
  _updateStage(e, a) {
    ...
    r || (r = document.createElement("img"), r.className = "camera-image is-hidden", r.alt = "", e.insertBefore(r, e.firstChild), this._bindImageElement(r)), r.dataset.cameraSrcBase = i, r.dataset.cameraEntity = a.entity, this._isMobileLayout() ? this._carregarSnapshotMobileValidado(r, s, a.entity) : r.src = s;
  }
  /**
   * Valida a troca manual antes de substituir o bitmap visivel no telefone.
   *
   * O token impede uma resposta atrasada de uma camera anterior de vencer uma
   * segunda troca rapida. O mesmo URL validado tende a vir do cache ao ser
   * entregue ao elemento visivel; o listener de load faz uma segunda barreira.
   */
  _carregarSnapshotMobileValidado(e, a, i) {
    if (!e || !a || !globalThis.Image) return;
    const t = `${i}:${Date.now()}:${Math.random()}`;
    e.dataset.mobileLoadToken = t;
    const r = new globalThis.Image();
    r.onload = () => {
      if (!(e.dataset.mobileLoadToken !== t || e.dataset.cameraEntity !== i)) {
        if (globalThis.BrunoCameraLive?.pareceQuadroVerde?.(r)) {
          globalThis.BrunoCameraLive?.marcar?.(i, "snapshot mobile verde rejeitado", 0, !1), this._solicitarSnapshotSeguro(i);
          return;
        }
        e.src = a;
      }
    }, r.onerror = () => {
      e.dataset.mobileLoadToken === t && this._solicitarSnapshotSeguro(i);
    }, r.src = a;
  }
  _solicitarSnapshotSeguro(e) {
    e && (this._sincronizarMotorCameras(), this._motorCameras?.buscarAgora?.(e));
  }
  ...
  _startRefreshTimer() {
    ...
    const e = Math.max(3e3, Number(this._config?.refresh_interval) || m.refresh_interval);
  ...
  _bindImageElement(e) {
    const a = () => {
      const i = e.dataset.cameraEntity || "";
      if (this._isMobileLayout() && globalThis.BrunoCameraLive?.pareceQuadroVerde?.(e)) {
        e.classList.add("is-hidden"), globalThis.BrunoCameraLive?.marcar?.(i, "imagem mobile verde rejeitada", 0, !1), this._solicitarSnapshotSeguro(i);
        return;
      }
      e.dataset.hasLoaded = "true", e.classList.remove("is-hidden");
    };
    this._isMobileLayout() && e.classList.add("is-hidden"), e.addEventListener("load", a), e.addEventListener("error", () => {
      e.dataset.hasLoaded !== "true" && e.classList.add("is-hidden");
    }), e.complete && e.naturalWidth > 0 && globalThis.queueMicrotask?.(a);
  }
  _render() {
    ...
    try {
      ...
      const i = this._isMobileLayout(), t = this._cameraControls(a), r = i ? `
          <main class="security-subview is-mobile-v2${t ? "" : " sem-controles"}">
            <header class="cameras-phone-head">
              <h1>Câmeras</h1>
              <p>
                <span>${e.onlineCount}/${e.totalCount} online</span>
                <span class="overview-sep" aria-hidden="true">·</span>
                <span>${e.motionCount} movimento</span>
              </p>
            </header>

            <section class="main-feed" aria-label="Câmera principal">
              ${o._mainFeed(a, !0, t, this._cameraMenuOpen)}
            </section>
        ...
        `;
      this.shadowRoot.innerHTML = `<style>${this._styles()}</style>${r}`, this.shadowRoot.removeEventListener("click", this._boundClick), this.shadowRoot.removeEventListener("keydown", this._boundKeydown), this.shadowRoot.addEventListener("click", this._boundClick), this.shadowRoot.addEventListener("keydown", this._boundKeydown), this._bindImages(), this._syncCameraControls(), this._syncPrivacySurfaces(), this._updateDesktopInformational(e), this._mountLiveFeed(e.activeId), this._renderedSignature = o._signatureFromModel(e);
    } catch (e) {
      this._renderError(e);
    }
  }
  ...
  _promoverLiveFeed(e) {
    if (!(!e || e !== this._liveEntity || !this._liveEl) && (this._liveTimer = null, this._liveReady !== e)) {
      if (this._isMobileLayout() && this._midiaLiveMobileEstaVerde()) {
        this._colocarLiveMobileEmQuarentena(e, "quadro verde no prazo");
        return;
      }
      this._liveReady = e, this._liveState = "live", this._liveEl.classList.add("is-ready"), this._sincronizarMotorCameras();
    }
  }
  ...
  _acharImagemAoVivo(e, a = 0) {
    if (!e || a > 4) return null;
    const i = [...e.querySelectorAll?.("img") ?? []].find((t) => t.complete && t.naturalWidth > 0);
    if (i) return i;
    for (const t of e.querySelectorAll?.("*") ?? []) {
      if (!t.shadowRoot) continue;
      const r = this._acharImagemAoVivo(t.shadowRoot, a + 1);
      if (r) return r;
    }
    return null;
  }
  _midiaLiveMobile() {
    const e = this._liveEl?.shadowRoot;
    if (!e) return null;
    const a = this._acharVideoAoVivo(e);
    return a && a.readyState >= 2 ? a : this._acharImagemAoVivo(e);
  }
  _midiaLiveMobileEstaVerde() {
    const e = this._midiaLiveMobile();
    return !!(e && globalThis.BrunoCameraLive?.pareceQuadroVerde?.(e));
  }
  /**
   * Quarentena exclusiva do telefone para o hui-image ja montado.
   *
   * Retirar is-ready revela o ultimo snapshot validado que esta por baixo. O
   * player continua conectado e volta sozinho quando os pixels normalizam.
   */
  _colocarLiveMobileEmQuarentena(e, a = "quadro verde eventual rejeitado") {
    const i = this._liveEl;
    if (!i || !e || e !== this._liveEntity) return;
    const t = this._mobileLiveQuarantined !== e;
    this._mobileLiveQuarantined = e, this._liveReady = "", i.classList.remove("is-ready"), i.setAttribute("data-bruno-quadro-verde", ""), t && (globalThis.BrunoCameraLive?.marcar?.(e, a, 0, !1), this._solicitarSnapshotSeguro(e));
  }
  _recuperarLiveMobile(e) {
    const a = this._liveEl;
    !a || !e || e !== this._liveEntity || (a.removeAttribute("data-bruno-quadro-verde"), a.classList.add("is-ready"), this._mobileLiveQuarantined = "", this._liveReady = e, this._liveState = "live", globalThis.BrunoCameraLive?.marcar?.(e, "stream mobile recuperado apos quadro verde"), this._sincronizarMotorCameras());
  }
  _iniciarVigiaLiveMobile() {
    if (!this._isMobileLayout() || this._mobileLiveWatchTimer) return;
    const e = () => {
      this._mobileLiveWatchTimer = null;
      const a = this._liveEl, i = this._liveEntity;
      if (!this.isConnected || !a || !i || !a.isConnected) return;
      const t = this._midiaLiveMobile();
      t && (!!globalThis.BrunoCameraLive?.pareceQuadroVerde?.(t) ? this._colocarLiveMobileEmQuarentena(i) : this._mobileLiveQuarantined === i && this._recuperarLiveMobile(i)), this._mobileLiveWatchTimer = globalThis.setTimeout(e, 500);
    };
    this._mobileLiveWatchTimer = globalThis.setTimeout(e, 500);
  }
  _pararVigiaLiveMobile() {
    this._mobileLiveWatchTimer && globalThis.clearTimeout(this._mobileLiveWatchTimer), this._mobileLiveWatchTimer = null, this._mobileLiveQuarantined = "";
  }
  _markLiveReady() {
    ...
    if (!(!e || !a || !i || i.readyState < 2 || this._liveReady === a)) {
      ...
      this._liveReady = a, this._liveState = "live", this._mobileLiveQuarantined === a && (this._mobileLiveQuarantined = "", e.removeAttribute("data-bruno-quadro-verde"), globalThis.BrunoCameraLive?.marcar?.(a, "stream mobile recuperado apos quadro verde")), this._liveGreenTimer && globalThis.clearTimeout(this._liveGreenTimer), this._liveGreenTimer = null, e.classList.add("is-ready"), this._liveTimer && globalThis.clearTimeout(this._liveTimer), this._liveTimer = null, this._sincronizarMotorCameras(), this._iniciarVigiaLiveMobile();
    }
  }
  ...
  _stopLiveFeed() {
    this._pararVigiaLiveMobile(), this._liveTimer && globalThis.clearTimeout(this._liveTimer), this._liveTimer = null, this._liveGreenTimer && globalThis.clearTimeout(this._liveGreenTimer), this._liveGreenTimer = null;
    ...
    return `
      ...
      * {
      ...
      @media (max-width: 800px) {
        ...
        /* PRIVACIDADE NO TELEFONE (2026-08-25).

           ANTERIOR (rollback): estas regras existiam somente dentro de
           min-width 801px. O estado e a superficie eram sincronizados no DOM,
           mas sem pintura no telefone: a ultima imagem apenas congelava.

           O escopo is-mobile-v2 evita qualquer mudanca no tablet. O z-index 2
           cobre imagem e player, mas preserva o cabecalho e o menu no nivel 3
           para que a privacidade possa ser desligada pelo mesmo controle. */
        .security-subview.is-mobile-v2 .camera-state-surface {
          position: absolute;
          inset: 0;
          z-index: 2;
          display: grid;
          ...
          text-align: center;
          background:
            radial-gradient(circle at 50% 42%, rgba(96,165,250,0.11), transparent 58%),
            rgba(5,8,14,0.82);
        ...
        .security-subview.is-mobile-v2 .camera-state-surface bruno-icon {
          display: none;
        }

        .security-subview.is-mobile-v2 .camera-state-surface span {
          font-size: clamp(11px, 3.1vw, 14px);
          font-weight: 720;
          line-height: 1.2;
        }

        .security-subview.is-mobile-v2 .camera-tile .camera-state-surface span {
          font-size: clamp(8px, 2.35vw, 10px);
        }

        .security-subview.is-mobile-v2 .main-feed-card.is-private .camera-image,
        .security-subview.is-mobile-v2 .main-feed-card.is-private .camera-live,
        .security-subview.is-mobile-v2 .camera-tile.is-private .camera-image {
          opacity: 0;
        }
      }
    `;
...
customElements.get(u) || customElements.define(u, o);
...
//# sourceMappingURL=bruno-cameras-security-subview.DlFTO3Vm.js.map
